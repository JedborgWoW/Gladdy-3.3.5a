local setmetatable = setmetatable

local GetLocale = GetLocale

local L = {}

-- Classes (English defaults, localized below)
L["Druid"] = "Druid"
L["Deathknight"] = "Death Knight"
L["Hunter"] = "Hunter"
L["Mage"] = "Mage"
L["Paladin"] = "Paladin"
L["Priest"] = "Priest"
L["Rogue"] = "Rogue"
L["Shaman"] = "Shaman"
L["Warlock"] = "Warlock"
L["Warrior"] = "Warrior"

-- Races (English defaults, localized below)
L["Human"] = "Human"
L["Orc"] = "Orc"
L["Dwarf"] = "Dwarf"
L["Night Elf"] = "Night Elf"
L["NightElf"] = "Night Elf"
L["Undead"] = "Undead"
L["Tauren"] = "Tauren"
L["Gnome"] = "Gnome"
L["Troll"] = "Troll"
L["Blood Elf"] = "Blood Elf"
L["BloodElf"] = "Blood Elf"
L["Draenei"] = "Draenei"

--Specs
--[[
L["Balance"] = BALANCE
L["Combat"] = COMBAT_LABEL
L["Fire"] = STRING_SCHOOL_FIRE
L["Arcane"] = STRING_SCHOOL_ARCANE
L["Shadow"] = STRING_SCHOOL_SHADOW
L["Holy"] = STRING_SCHOOL_HOLY
L["Elemental"] = STRING_SCHOOL_ELEMENTAL


--Modules
--L["Announcements"] = CHAT_ANNOUNCE
L["Auras"] = COMBAT_TEXT_SHOW_AURAS_TEXT
L["Cast Bar"] = SHOW_ENEMY_CAST
L["Buffs and Debuffs"] = BUFFOPTIONS_LABEL
--L["Class Icon"] = CLASS .. " " .. EMBLEM_SYMBOL
--L["Clicks"] =
L["Cooldowns"] = CAPACITANCE_SHIPMENT_COOLDOWN:gsub(": %%s", "")
--L["Export Import"] =
--L["Healthbar"]
L["Highlight"] = HIGHLIGHTING:gsub(":", "")
L["Pet"] = PET_TYPE_PET
--L["Racial"] = RACE .. " " .. ABILITIES
--L["Range Check"] =
L["Trinket"] = TRINKET0SLOT
--]]

if (GetLocale() == "ruRU") then
    -- Specs
    L["Balance"] = "Баланс"
    L["Feral"] = "Сила зверя"
    L["Restoration"] = "Исцеление"
    L["Beast Mastery"] = "Повелитель зверей"
    L["Marksmanship"] = "Стрельба"
    L["Survival"] = "Выживание"
    L["Arcane"] = "Тайная магия"
    L["Fire"] = "Огонь"
    L["Frost"] = "Лед"
    L["Holy"] = "Свет"
    L["Protection"] = "Защита"
    L["Retribution"] = "Возмездие"
    L["Discipline"] = "Послушание"
    L["Shadow"] = "Тьма"
    L["Assassination"] = "Ликвидация"
    L["Combat"] = "Бой"
    L["Subtlety"] = "Скрытность"
    L["Elemental"] = "Cтихии"
    L["Enhancement"] = "Совершенствование"
    L["Affliction"] = "Колдовство"
    L["Demonology"] = "Демонология"
    L["Destruction"] = "Разрушение"
    L["Arms"] = "Оружие"
    L["Fury"] = "Неистовство"

    -- Gladdy.lua
    L["Welcome to Gladdy!"] = "Вас приветствует Gladdy!"
    L["First run has been detected, displaying test frame."] = "Обнаружен первый запуск, показываем тестовое окно."
    L["Valid slash commands are:"] = "Валидные команды:"
    L["If this is not your first run please lock or move the frame to prevent this from happening."] = "Если это не первый запуск, переместите, либо закрепите окно во избежание показа окна."

    -- Frame.lua
    L["Gladdy - drag to move"] = "Gladdy - тащите для перемещения"

    -- Options.lua
    L["Announcements"] = "Оповещения"
    L["Announcement settings"] = "Настройки оповещения"
    L["Auras"] = "Ауры"
    L["Auras settings"] = "Настройки аур"
    L["Castbar"] = "Полоса применения"
    L["Castbar settings"] = "Настройки полосы применения"
    L["Classicon"] = "Иконка класса"
    L["Classicon settings"] = "Настройки иконки класса"
    L["Clicks"] = "Клики"
    L["Clicks settings"] = "Настройки кликов"
    L["Diminishings"] = "Диминишинги"
    L["Diminishings settings"] = "Настройки диминишингов"
    L["Healthbar"] = "Полоса жизней"
    L["Healthbar settings"] = "Настройки полосы жизней"
    L["Highlight"] = "Подсветка"
    L["Highlight settings"] = "Настройки подсветки"
    L["Nameplates"] = "Индикаторы здоровья"
    L["Nameplates settings"] = "Настройки индикаторов здоровья "
    L["Powerbar"] = "Полоса маны"
    L["Powerbar settings"] = "Настройки полосы маны"
    L["Score"] = "Счет"
    L["Score settings"] = "Настройки счета"
    L["Trinket"] = "Тринкет"
    L["Trinket settings"] = "Настройки тринкета"
    L["Reset module"] = "Сбросить настройки"
    L["Reset module to defaults"] = "Сбросить настройки модуля к начальным"
    L["No settings"] = "Нет настроек"
    L["Module has no settings"] = "Модуль не имеет настроек"
    L["General"] = "Основные"
    L["General settings"] = "Основные настройки"
    L["Lock frame"] = "Закрепить фрейм"
    L["Toggle if frame can be moved"] = "Включите, если фрейм можно двигать"
    L["Grow frame upwards"] = "Расти вверх"
    L["If enabled the frame will grow upwards instead of downwards"] = "Если включено, то фрейм будет расти вверх, а не вниз"
    L["Frame scale"] = "Масштаб фрейма"
    L["Scale of the frame"] = "Масштаб фрейма"
    L["Frame padding"] = "Отступы фрейма"
    L["Padding of the frame"] = "Отступы фрейма"
    L["Frame color"] = "Цвет фрейма"
    L["Color of the frame"] = "Цвет фрейма"
    L["Bar width"] = "Ширина полос"
    L["Width of the bars"] = "Ширина полос"
    L["Bottom margin"] = "Нижняя граница полосы"
    L["Margin between each button"] = "Отступ до следующей полосы"

    -- Announcements.lua
    L["RESURRECTING: %s (%s)"] = "ВОСКРЕШАЕТ: %s (%s)"
    L["SPEC DETECTED: %s - %s (%s)"] = "СПЕЦИАЛИЗАЦИЯ ОПРЕДЕЛЕНА: %s - %s (%s)"
    L["LOW HEALTH: %s (%s)"] = "МАЛО ЗДОРОВЬЯ: %s (%s)"
    L["TRINKET USED: %s (%s)"] = "ТРИНКЕТ ИСПОЛЬЗОВАН: %s (%s)"
    L["TRINKET READY: %s (%s)"] = "ТРИНКЕТ ГОТОВ: %s (%s)"
    L["DRINKING: %s (%s)"] = "ПЬЁТ: %s (%s)"
    L["Self"] = "Сам"
    L["Party"] = "Группа"
    L["Raid Warning"] = "Объявление рейду"
    L["Blizzard's Floating Combat Text"] = "Стандартный текст боя"
    L["MikScrollingBattleText"] = "MikScrollingBattleText"
    L["Scrolling Combat Text"] = "Scrolling Combat Text"
    L["Parrot"] = "Parrot"
    L["Drinking"] = "Питьё"
    L["Announces when enemies sit down to drink"] = "Оповещать, когда вражеский игрок начинает пить"
    L["Resurrection"] = "Воскрешение"
    L["Announces when an enemy tries to resurrect a teammate"] = "Оповещать, когда вражеский игрок начинает воскрешать союзника"
    L["New enemies"] = "Новые враги"
    L["Announces when new enemies are discovered"] = "Оповещать о найденных вражеских игроках"
    L["Spec Detection"] = "Обнаружение специализации"
    L["Announces when the spec of an enemy was detected"] = "Оповещать, когда обнаружена специализация вражеского игрока"
    L["Low health"] = "Мало жизней"
    L["Announces when an enemy drops below a certain health threshold"] = "Оповещать, когда уровень здоровья вражеского игрока падает ниже определенного количества"
    L["Low health threshold"] = "Процент малого количества здоровья"
    L["Choose how low an enemy must be before low health is announced"] = "Укажите процент здоровья вражеского игрока, когда нужно показать оповещение"
    L["Trinket used"] = "Использование пвп-тринкета"
    L["Announce when an enemy's trinket is used"] = "Оповещать об использовании пвп-тринкета вражеским игроком"
    L["Trinket ready"] = "Готовность пвп-тринкета"
    L["Announce when an enemy's trinket is ready again"] = "Оповещать, когда пвп-тринкет вражеского игрока готов"
    L["Destination"] = "Вариант оповещений"
    L["Choose how your announcements are displayed"] = "Выберать, как показывать оповещения"

    -- Auras.lua
    L["Font color"] = "Цвет текста"
    L["Color of the text"] = "Цвет текста"
    L["Font size"] = "Размер текста"
    L["Size of the text"] = "Размер текста"

    -- Castbar.lua
    L["Bar height"] = "Высота полосы"
    L["Height of the bar"] = "Высота полосы"
    L["Bar texture"] = "Текстура полосы"
    L["Texture of the bar"] = "Текстура полосы"
    L["Bar color"] = "Цвет полосы"
    L["Color of the cast bar"] = "Цвет полосы применений"
    L["Background color"] = "Цвет фона полосы"
    L["Color of the cast bar background"] = "Цвет фона полосы применений"
    L["Icon position"] = "Расположение значка трансляции"

    -- Diminishings.lua
    L["DR Cooldown position"] = "Позиция ДР таймеров"
    L["Position of the cooldown icons"] = "Позиция ДР таймеров"
    L["Left"] = "Слева"
    L["Right"] = "Справа"
    L["Icon Size"] = "Размер иконок"
    L["Size of the DR Icons"] = "Размер ДР иконок"

    -- Healthbar.lua
    L["Show the actual health"] = "Показывать текущее здоровье"
    L["Show the actual health on the health bar"] = "Показывать текущее здоровье на полосе жизней"
    L["Show max health"] = "Показывать максимальное здоровье"
    L["Show max health on the health bar"] = "Показывать максимальное здоровье на полосе жизней"
    L["Show health percentage"] = "Показывать здоровье в процентах"
    L["Show health percentage on the health bar"] = "Показывать здоровье в процентах на полосе жизней"

    -- Highlight.lua
    L["Border size"] = "Размер границы"
    L["Target border color"] = "Цвет контура цели"
    L["Color of the selected targets border"] = "Цвет контура цели"
    L["Focus border color"] = "Цвет контура фокуса"
    L["Color of the focus border"] = "Цвет контура фокуса"
    L["Raid leader border color"] = "Цвет контура цели рейд лидера"
    L["Color of the raid leader border"] = "Цвет контура цели рейд лидера"
    L["Highlight target"] = "Подсвечивать цель"
    L["Toggle if the selected target should be highlighted"] = "Включите, если необходима подсветка цели"
    L["Show border around target"] = "Показывать контур вокруг цели"
    L["Toggle if a border should be shown around the selected target"] = "Включите, если необходимо показывать контур вокруг цели"
    L["Show border around focus"] = "Показывать контур вокруг фокуса"
    L["Toggle of a border should be shown around the current focus"] = "Включите, если необходимо показывать контур вокруг фокуса"
    L["Show border around raid leader"] = "Показывать контур вокруг цели рейд лидера"
    L["Toggle if a border should be shown around the raid leader"] = "Включите, если необходимо показывать контур вокруг цели рейд лидера"

    -- Powerbar.lua
    L["Show the actual power"] = "Показывать текущую ману"
    L["Show the actual power on the power bar"] = "Показывать текущую ману на полосе маны"
    L["Show max power"] = "Показывать максимальную ману"
    L["Show max power on the power bar"] = "Показывать максимальную ману на полосе маны"
    L["Show power percentage"] = "Показывать ману в процентах"
    L["Show power percentage on the power bar"] = "Показывать ману в процентах на полосе маны"
    L["Color of the status bar background"] = "Цвет фона строки состояния"

    -- Trinket.lua
    L["No cooldown count (OmniCC)"] = "Не показывать кулдаун (OmniCC)"
    L["Disable cooldown timers by addons (reload UI to take effect)"] = "Отключить таймер кулдаунов для аддонов (необходима перезагрузка интерфейса)"

    -- Options.lua - General
    L["Unlock frame"] = "Разблокировать фрейм"
    L["Show Mover"] = "Показать перемещение"
    L["Hide Mover"] = "Скрыть перемещение"
    L["Toggle to show Mover Frames"] = "Переключить отображение фреймов перемещения"
    L["Test"] = "Тест"
    L["Show Test frames"] = "Показать тестовые фреймы"
    L["Hide"] = "Скрыть"
    L["Hide frames"] = "Скрыть фреймы"
    L["ReloadUI"] = "Перезагрузить UI"
    L["Reloads the UI"] = "Перезагружает интерфейс"
    L["Gladdy Tooltip round"] = "Gladdy Tooltip круглый"
    L["Gladdy Tooltip squared"] = "Gladdy Tooltip квадратный"
    L["Gloss (black border)"] = "Глянец (черная граница)"
    L["Grow Middle"] = "Расти от центра"
    L["Frames expand along a centric anchor"] = "Фреймы расширяются от центральной точки"
    L["Pixel Perfect Scale"] = "Идеальный масштаб пикселей"
    L["Enables Pixel Perfect Scale - disables manual "] = "Включает идеальный масштаб пикселей - отключает ручной "
    L["OmniCC Enabled"] = "OmniCC включен"
    L["Timer Format"] = "Формат таймера"
    L["Frame width"] = "Ширина фрейма"
    L["Background color"] = "Цвет фона"
    L["Background Color of the frame"] = "Цвет фона фрейма"
    L["Cooldown General"] = "Общие кулдауны"
    L["Font General"] = "Общий шрифт"
    L["General Font"] = "Основной шрифт"
    L["Font color text"] = "Цвет текста шрифта"
    L["Font color timer"] = "Цвет таймера шрифта"
    L["Color of the timers"] = "Цвет таймеров"
    L["Icons General"] = "Общие иконки"
    L["Icon border style"] = "Стиль границы иконки"
    L["This changes the border style of all icons"] = "Изменяет стиль границы всех иконок"
    L["Icon border color"] = "Цвет границы иконки"
    L["This changes the border color of all icons"] = "Изменяет цвет границы всех иконок"
    L["Statusbar General"] = "Общие полосы состояния"
    L["Statusbar texture"] = "Текстура полосы состояния"
    L["This changes the texture of all statusbar frames"] = "Изменяет текстуру всех полос состояния"
    L["Statusbar border style"] = "Стиль границы полосы состояния"
    L["This changes the border style of all statusbar frames"] = "Изменяет стиль границы всех полос состояния"
    L["Statusbar border offset divider (smaller is higher offset)"] = "Делитель смещения границы полосы состояния (меньше = больше смещение)"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "Смещение границы относительно полосы состояния (если полоса выходит за границу)"
    L["Statusbar border color"] = "Цвет границы полосы состояния"
    L["This changes the border color of all statusbar frames"] = "Изменяет цвет границы всех полос состояния"
    L["Hide Blizzard"] = "Скрыть Blizzard"
    L["Arena only"] = "Только арена"
    L["Never"] = "Никогда"
    L["Always"] = "Всегда"
    L["Grow Direction"] = "Направление роста"
    L["Down"] = "Вниз"
    L["Up"] = "Вверх"
    L["Load configuration"] = "Загрузить конфигурацию"
    L["Load configuration options"] = "Загрузить опции конфигурации"

    -- Nameplates.lua
    L["Show important auras on nameplates"] = "Показывать важные ауры на индикаторах здоровья"
    L["Zoomed Icon"] = "Увеличенная иконка"
    L["Zoomes the icon to remove borders"] = "Увеличивает иконку для удаления границ"
    L["Icon size"] = "Размер иконки"
    L["Size of each aura icon"] = "Размер каждой иконки ауры"
    L["Icon Width Factor"] = "Коэффициент ширины иконки"
    L["Stretches the icon"] = "Растягивает иконку"
    L["Max Icons"] = "Максимум иконок"
    L["Maximum number of aura icons to show"] = "Максимальное количество иконок аур для отображения"
    L["Icon Padding"] = "Отступ иконок"
    L["Space between icons"] = "Пространство между иконками"
    L["Horizontal offset"] = "Горизонтальное смещение"
    L["Vertical offset"] = "Вертикальное смещение"
    L["Center Icons"] = "Центрировать иконки"
    L["Sort Order"] = "Порядок сортировки"
    L["How to sort aura icons"] = "Как сортировать иконки аур"
    L["Time"] = "Время"
    L["Time Left"] = "Осталось времени"
    L["Priority"] = "Приоритет"
    L["Frame Strata and Level"] = "Слой и уровень фрейма"
    L["Frame Strata"] = "Слой фрейма"
    L["Frame Level"] = "Уровень фрейма"
    L["Aura List"] = "Список аур"
    L["Configure which auras to show on nameplates"] = "Настроить какие ауры показывать на индикаторах здоровья"
    L["Note: Uses the same aura list as the Auras module. Configure auras in the Auras module settings."] = "Примечание: Использует тот же список аур, что и модуль Ауры. Настройте ауры в настройках модуля Ауры."

    -- Diminishings.lua
    L["Show when Aura applied"] = "Показывать при применении ауры"
    L["Group"] = "Группа"
    L["DR Icon"] = "Иконка ДР"
    L["Group direction"] = "Направление группы"
    L["Cooldown Font"] = "Шрифт кулдауна"
    L["Enable DR Colors as Font Color"] = "Включить цвета ДР как цвет шрифта"
    L["Shows the current DR Level on the DR icon."] = "Показывает текущий уровень ДР на иконке ДР."
    L["Cooldown number alpha"] = "Прозрачность числа кулдауна"
    L["Font Outline"] = "Контур шрифта"
    L["X Offset"] = "Смещение X"
    L["Y Offset"] = "Смещение Y"
    L["DR Font"] = "Шрифт ДР"
    L["Enable DR Font"] = "Включить шрифт ДР"
    L["DR Font color"] = "Цвет шрифта ДР"
    L["Color of the font"] = "Цвет шрифта"
    L["Font of the DR Font"] = "Шрифт для шрифта ДР"
    L["DR Colors"] = "Цвета ДР"
    L["DR Texts"] = "Тексты ДР"
    L["Text on half DR"] = "Текст на половине ДР"
    L["Text on quarter DR"] = "Текст на четверти ДР"
    L["Text on immune DR"] = "Текст на иммунитете ДР"
    L["Enable DR Colors as Border Color"] = "Включить цвета ДР как цвет границы"
    L["Colors borders of DRs in respective DR Colors"] = "Окрашивает границы ДР в соответствующие цвета ДР"
    L["Border color"] = "Цвет границы"
    L["DR Grow Direction"] = "Направление роста ДР"
    L["Grow Direction of the dr icons"] = "Направление роста иконок ДР"
    L["Enabled DR module"] = "Включен модуль ДР"
    L["Change the DR Duration in seconds (DR is dynamic between 15-20s)"] = "Изменить длительность ДР в секундах (ДР динамичен между 15-20с)"
    L["DR Duration"] = "Длительность ДР"
    L["Space between Icons"] = "Пространство между иконками"
    L["Scale of the text"] = "Масштаб текста"
    L["Scale of the font"] = "Масштаб шрифта"

    -- Clicks.lua
    L["Action #%d"] = "Действие #%d"
    L["Left Click Enemy 1"] = "Левый клик по врагу 1"
    L["Left Click Enemy 2"] = "Левый клик по врагу 2"
    L["Left Click Enemy 3"] = "Левый клик по врагу 3"
    L["Left Click Enemy 4"] = "Левый клик по врагу 4"
    L["Left Click Enemy 5"] = "Левый клик по врагу 5"
    L["Right Click Enemy 1"] = "Правый клик по врагу 1"
    L["Right Click Enemy 2"] = "Правый клик по врагу 2"
    L["Right Click Enemy 3"] = "Правый клик по врагу 3"
    L["Right Click Enemy 4"] = "Правый клик по врагу 4"
    L["Right Click Enemy 5"] = "Правый клик по врагу 5"
    L["Middle Click Enemy 1"] = "Средний клик по врагу 1"
    L["Middle Click Enemy 2"] = "Средний клик по врагу 2"
    L["Middle Click Enemy 3"] = "Средний клик по врагу 3"
    L["Middle Click Enemy 4"] = "Средний клик по врагу 4"
    L["Middle Click Enemy 5"] = "Средний клик по врагу 5"
    L["Button4 Click Enemy 1"] = "Кнопка4 клик по врагу 1"
    L["Button4 Click Enemy 2"] = "Кнопка4 клик по врагу 2"
    L["Button4 Click Enemy 3"] = "Кнопка4 клик по врагу 3"
    L["Button4 Click Enemy 4"] = "Кнопка4 клик по врагу 4"
    L["Button4 Click Enemy 5"] = "Кнопка4 клик по врагу 5"
    L["Button5 Click Enemy 1"] = "Кнопка5 клик по врагу 1"
    L["Button5 Click Enemy 2"] = "Кнопка5 клик по врагу 2"
    L["Button5 Click Enemy 3"] = "Кнопка5 клик по врагу 3"
    L["Button5 Click Enemy 4"] = "Кнопка5 клик по врагу 4"
    L["Button5 Click Enemy 5"] = "Кнопка5 клик по врагу 5"
    L["Left button"] = "Левая кнопка"
    L["Right button"] = "Правая кнопка"
    L["Middle button"] = "Средняя кнопка"
    L["Button 4"] = "Кнопка 4"
    L["Button 5"] = "Кнопка 5"
    L["None"] = "Нет"
    L["All"] = "Все"
    L["CTRL"] = "CTRL"
    L["SHIFT"] = "SHIFT"
    L["ALT"] = "ALT"
    L["Cast Spell"] = "Применить заклинание"
    L["Name"] = "Имя"
    L["Select the name of the click option"] = "Выберите имя опции клика"
    L["Button"] = "Кнопка"
    L["Select which mouse button to use"] = "Выберите какую кнопку мыши использовать"
    L["Modifier"] = "Модификатор"
    L["Select which modifier to use"] = "Выберите какой модификатор использовать"
    L["Action"] = "Действие"
    L["Select what action this mouse button does"] = "Выберите какое действие выполняет эта кнопка мыши"
    L["Cast Spell / Macro"] = "Применить заклинание / Макрос"
    L["Target"] = "Цель"
    L["Focus"] = "Фокус"

    -- Additional missing keys
    L["Frame"] = "Фрейм"
    L["Enabled"] = "Включено"
    L["Icon"] = "Иконка"
    L["Position"] = "Позиция"
    L["Cooldown"] = "Кулдаун"
    L["Font"] = "Шрифт"
    L["Font scale"] = "Масштаб шрифта"
    L["Font color"] = "Цвет шрифта"
    L["Border"] = "Граница"
    L["Border style"] = "Стиль границы"
    L["Buff color"] = "Цвет баффа"
    L["Debuff color"] = "Цвет дебаффа"
    L["Check All"] = "Выбрать все"
    L["Uncheck All"] = "Снять все"
    L["Force Icon"] = "Принудительная иконка"
    L["Icon of the DR"] = "Иконка ДР"
    L["Categories"] = "Категории"
    L["Immune"] = "Иммунитет"
    L["Half"] = "Половина"
    L["Quarter"] = "Четверть"
    L["Top"] = "Верх"
    L["Bottom"] = "Низ"
    
    -- Additional missing keys for ruRU
    L["No Pet"] = "Нет питомца"
    L["0%"] = "0%"
    L["100%"] = "100%"
    L["50%"] = "50%"
    L["Above"] = "Выше"
    L["Add Aura"] = "Добавить ауру"
    L["Add Buff"] = "Добавить бафф"
    L["Add Debuff"] = "Добавить дебафф"
    L["All enemy totem border color"] = "Цвет границы всех вражеских тотемов"
    L["All friendly totem border color"] = "Цвет границы всех дружественных тотемов"
    L["Alpha of the icon when cooldown active"] = "Прозрачность иконки при активном кулдауне"
    L["Announce Spell School"] = "Оповещать о школе заклинаний"
    L["Announces Spell School like \"Holy\" instead of Spell Name"] = "Оповещает школу заклинаний, например \"Свет\", вместо названия заклинания"
    L["Announces when enemies' spells are interrupted"] = "Оповещает, когда заклинания врагов прерваны"
    L["Attach To TotemPlate"] = "Прикрепить к табличке тотема"
    L["Aura Detached"] = "Аура отсоединена"
    L["Aura X Offset"] = "Смещение ауры по X"
    L["Aura Y Offset"] = "Смещение ауры по Y"
    L["Aura size"] = "Размер ауры"
    L["Aura width factor"] = "Коэффициент ширины ауры"
    L["Background"] = "Фон"
    L["Bar Color"] = "Цвет полосы"
    L["Bar Size"] = "Размер полосы"
    L["Bar horizontal"] = "Горизонтальная полоса"
    L["Bar vertical"] = "Вертикальная полоса"
    L["BeastMastery"] = "Повелитель зверей"
    L["Below"] = "Ниже"
    L["Blood"] = "Кровь"
    L["Brewmaster"] = "Пивовар"
    L["Buff Lists"] = "Списки баффов"
    L["By Arena"] = "По арене"
    L["By Order"] = "По порядку"
    L["COOLDOWN USED: %s (%s) used %s - %s sec. cooldown"] = "ИСПОЛЬЗОВАН КУЛДАУН: %s (%s) использовал %s - %s сек. кулдаун"
    L["Class Colored Bars"] = "Полосы цвета класса"
    L["Class Icon Enabled"] = "Иконка класса включена"
    L["Class colored"] = "Цвет класса"
    L["Colored trinket"] = "Цветной тринкет"
    L["Colored trinket CD"] = "Цветной тринкет с кулдауном"
    L["Colored trinket No CD"] = "Цветной тринкет без кулдауна"
    L["Custom Colors"] = "Пользовательские цвета"
    L["Custom Tags"] = "Пользовательские теги"
    L["Custom Tags Enabled"] = "Пользовательские теги включены"
    L["Default Aura"] = "Аура по умолчанию"
    L["Delete"] = "Удалить"
    L["Desaturate Icon"] = "Обесцветить иконку"
    L["Detached Aura"] = "Отсоединенная аура"
    L["Detached Interrupt"] = "Отсоединенное прерывание"
    L["Detached Mode"] = "Режим отсоединения"
    L["Dialog"] = "Диалог"
    L["Dispel Icon"] = "Иконка рассеивания"
    L["Dispel Icon Enabled"] = "Иконка рассеивания включена"
    L["Dr Border Colors Enabled"] = "Цвета границ ДР включены"
    L["Dynamic Icon"] = "Динамическая иконка"
    L["Enable"] = "Включить"
    L["Enable Aura to be displayed"] = "Включить отображение ауры"
    L["Enable Custom Colors"] = "Включить пользовательские цвета"
    L["Enable Interrupt Spell School Colors"] = "Включить цвета школ заклинаний для прерываний"
    L["Enabled Buffs and Debuffs module"] = "Модуль баффов и дебаффов включен"
    L["Enabled Racials"] = "Расовые способности включены"
    L["Enabled showing racial"] = "Включено отображение расовых способностей"
    L["Enables Pets module"] = "Включает модуль питомцев"
    L["Enables Targets module"] = "Включает модуль целей"
    L["Enemy"] = "Враг"
    L["Enrage"] = "Исступление"
    L["Export"] = "Экспорт"
    L["Export your current profile to share with others or your various accounts."] = "Экспортируйте ваш текущий профиль для обмена с другими или вашими различными аккаунтами."
    L["Fade"] = "Затухание"
    L["Font Enabled"] = "Шрифт включен"
    L["Font of the bar"] = "Шрифт полосы"
    L["Font of the castbar"] = "Шрифт полосы применения"
    L["Font of the cooldown"] = "Шрифт кулдауна"
    L["Font of the custom totem name"] = "Шрифт пользовательского имени тотема"
    L["Font size left text"] = "Размер шрифта левого текста"
    L["Font size right text"] = "Размер шрифта правого текста"
    L["Form"] = "Форма"
    L["Friendly"] = "Дружественный"
    L["Fullscreen"] = "Полноэкранный"
    L["Fullscreen Dialog"] = "Полноэкранный диалог"
    L["Group Class Icons"] = "Групповые иконки классов"
    L["Group Mode"] = "Режим группы"
    L["Group Pets"] = "Групповые питомцы"
    L["Group Targets"] = "Групповые цели"
    L["Guardian"] = "Страж"
    L["Health Bar"] = "Полоса здоровья"
    L["Health Bar Text"] = "Текст полосы здоровья"
    L["Health Values"] = "Значения здоровья"
    L["Health color"] = "Цвет здоровья"
    L["Health font size"] = "Размер шрифта здоровья"
    L["Height"] = "Высота"
    L["Hide Disabled Totem Plates"] = "Скрыть отключенные таблички тотемов"
    L["High"] = "Высокий"
    L["Higher is darker"] = "Чем выше, тем темнее"
    L["Highlight mouseover"] = "Подсветка при наведении"
    L["Horizontal Grow Direction"] = "Горизонтальное направление роста"
    L["Horizontal Grow Direction of the cooldown icons"] = "Горизонтальное направление роста иконок кулдауна"
    L["Icon Enabled"] = "Иконка включена"
    L["Icon border"] = "Граница иконки"
    L["If test is running, type \"/gladdy test\" again"] = "Если тест запущен, введите \"/gladdy test\" снова"
    L["Import"] = "Импорт"
    L["Interrupt Detached"] = "Отсоединенное прерывание"
    L["Interrupt Spells School Colors"] = "Цвета школ заклинаний для прерываний"
    L["Interrupt X Offset"] = "Смещение прерывания по X"
    L["Interrupt Y Offset"] = "Смещение прерывания по Y"
    L["Interrupt size"] = "Размер прерывания"
    L["Interrupt width factor"] = "Коэффициент ширины прерывания"
    L["Interrupts"] = "Прерывания"
    L["INTERRUPTED: %s (%s)"] = "ПРЕРВАНО: %s (%s)"
    L["Layer Group"] = "Группа слоя"
    L["Layer Group and Level"] = "Группа и уровень слоя"
    L["Layer Level"] = "Уровень слоя"
    L["Left Font Outline"] = "Контур левого шрифта"
    L["Left Text"] = "Левый текст"
    L["Left Text Horizontal Offset"] = "Горизонтальное смещение левого текста"
    L["Left Text Vertical Offset"] = "Вертикальное смещение левого текста"
    L["Left font size"] = "Размер левого шрифта"
    L["Locked"] = "Заблокировано"
    L["Low"] = "Низкий"
    L["Max"] = "Макс"
    L["Max Icons per row"] = "Максимум иконок в ряду"
    L["Medium"] = "Средний"
    L["Mistweaver"] = "Ткач туманов"
    L["Nature"] = "Природа"
    L["No Cooldown Circle"] = "Без круга кулдауна"
    L["Cooldowncircle"] = "Круг кулдауна"
    L["Only numbers, comma or space is allowed. \n Example: 124, 51243, 4121, 41111"] = "Разрешены только числа, запятая или пробел. \n Пример: 124, 51243, 4121, 41111"
    L["Out of Range Darkening Level"] = "Уровень затемнения вне диапазона"
    L["Power Bar"] = "Полоса силы"
    L["Power Bar Text"] = "Текст полосы силы"
    L["Power Texts"] = "Тексты силы"
    L["Power font size"] = "Размер шрифта силы"
    L["Race font size"] = "Размер шрифта расы"
    L["Range Check"] = "Проверка расстояния"
    L["Remaining"] = "Осталось"
    L["Reset Glow"] = "Сбросить свечение"
    L["Reset Glow Color"] = "Сбросить цвет свечения"
    L["Reset Time"] = "Сбросить время"
    L["Reset time in seconds"] = "Время сброса в секундах"
    L["Reverse"] = "Обратный"
    L["Right Font Outline"] = "Контур правого шрифта"
    L["Right Text"] = "Правый текст"
    L["Right Text Horizontal Offset"] = "Горизонтальное смещение правого текста"
    L["Right Text Vertical Offset"] = "Вертикальное смещение правого текста"
    L["Right font size"] = "Размер правого шрифта"
    L["Scale"] = "Масштаб"
    L["Scourge"] = "Проклятые"
    L["Show 1-5 as name instead"] = "Показывать 1-5 вместо имени"
    L["Show Arena1-5 as name instead"] = "Показывать Арена1-5 вместо имени"
    L["Show CC"] = "Показывать контроль"
    L["Show Highlight border inside of frame"] = "Показывать границу подсветки внутри фрейма"
    L["Show Spec Icon"] = "Показывать иконку специализации"
    L["Show Spell Name"] = "Показывать название заклинания"
    L["Show Time"] = "Показывать время"
    L["Show border colors by spell school"] = "Показывать цвета границ по школе заклинаний"
    L["Show dynamic color on cooldown numbers"] = "Показывать динамический цвет на числах кулдауна"
    L["Show enemy"] = "Показывать врагов"
    L["Show friendly"] = "Показывать друзей"
    L["Show name text"] = "Показывать текст имени"
    L["Show race"] = "Показывать расу"
    L["Show spec"] = "Показывать специализацию"
    L["Show the units name"] = "Показывать имя юнита"
    L["Show two timers"] = "Показывать два таймера"
    L["Shows a solid colored icon when off/off CD."] = "Показывает сплошную цветную иконку, когда выключено/без кулдауна."
    L["Shows all debuffs, which are displayed on the ClassIcon as well"] = "Показывает все дебаффы, которые также отображаются на иконке класса"
    L["Size"] = "Размер"
    L["Size & Padding"] = "Размер и отступы"
    L["Size of each cd icon"] = "Размер каждой иконки кулдауна"
    L["Size of the health text"] = "Размер текста здоровья"
    L["Size of the left text"] = "Размер левого текста"
    L["Size of the power text"] = "Размер текста силы"
    L["Size of the race text"] = "Размер текста расы"
    L["Size of the right text"] = "Размер правого текста"
    L["Spark"] = "Искра"
    L["Spark enabled"] = "Искра включена"
    L["Spell ID used for texture"] = "ID заклинания, используемый для текстуры"
    L["Spell IDs"] = "ID заклинаний"
    L["Spell School Colors"] = "Цвета школ заклинаний"
    L["Spell School Colors Enabled"] = "Цвета школ заклинаний включены"
    L["Spells"] = "Заклинания"
    L["Start Time"] = "Время начала"
    L["Start time in seconds"] = "Время начала в секундах"
    L["Status Bar border"] = "Граница полосы состояния"
    L["Status Bar border color"] = "Цвет границы полосы состояния"
    L["Stealth Color"] = "Цвет скрытности"
    L["Style"] = "Стиль"
    L["Test With Talents"] = "Тест с талантами"
    L["Test Without Talents"] = "Тест без талантов"
    L["Text"] = "Текст"
    L["Thick Outline"] = "Толстый контур"
    L["Thin Outline"] = "Тонкий контур"
    L["This changes positions of the trinket"] = "Это изменяет позиции тринкета"
    L["This changes positions with trinket"] = "Это изменяет позиции с тринкетом"
    L["This will overwrite your current profile!"] = "Это перезапишет ваш текущий профиль!"
    L["Tooltip"] = "Подсказка"
    L["Totem Plates"] = "Таблички тотемов"
    L["Totem Pulse"] = "Пульс тотема"
    L["Totem Pulse Enabled"] = "Пульс тотема включен"
    L["Turn on/off"] = "Включить/выключить"
    L["Turns countdown before the start of an arena match on/off."] = "Включает/выключает обратный отсчет перед началом матча на арене."
    L["Turns totem icons instead of nameplates on or off."] = "Включает/выключает иконки тотемов вместо табличек имен."
    L["Unholy"] = "Нечестивость"
    L["Unknown"] = "Неизвестно"
    L["Vertical Bar"] = "Вертикальная полоса"
    L["Vertical Grow Direction"] = "Вертикальное направление роста"
    L["Vertical Grow Direction of the cooldown icons"] = "Вертикальное направление роста иконок кулдауна"
    L["Width"] = "Ширина"
    L["Will use Debuff Color if disabled"] = "Будет использовать цвет дебаффа, если отключено"
    L["Windwalker"] = "Танцующий с ветром"
    L["XiconProfiles"] = "Профили Xicon"
    L["yards"] = "ярдов"
    L["changes "] = "изменения "
elseif GetLocale() == "deDE" then
    -- Announcements.lua
    L["Announcements"] = "Meldungen"
    L["RESURRECTING: %s (%s)"] = "Wiederbeleben: %s (%s) "
    L["SPEC DETECTED: %s - %s (%s)"] = "Talenspezalisierung entdeckt: %s - %s (%s)"
    L["LOW HEALTH: %s (%s)"] = "Niedriges Leben: %s (%s)"
    L["TRINKET USED: %s (%s)"] = "Insiginie benutzt: %s (%s)"
    L["TRINKET READY: %s (%s)"] = "Insignie bereit: %s (%s)"
    L["DRINKING: %s (%s)"] = "Trinken: %s (%s)"
    L["Self"] = "Selbst"
    L["Party"] = "Gruppe"
    L["Raid Warning"] = "Schlachtzugwarnung"
    L["Blizzard's Floating Combat Text"] = "Blizzard Kampftext"
    L["Trinket used"] = "Insignie benutzt"
    L["Announce when an enemy's trinket is used"] = "Warnt, wenn ein Gegner seine Insignie benutzt"
    L["Trinket ready"] = "Insignie bereit"
    L["Announce when an enemy's trinket is ready again"] = "Warnt wenn die Insignie eines Gegner wieder bereit ist"
    L["Drinking"] = "Trinken"
    L["Announces when enemies sit down to drink"] = "Warnt wenn Gegner sich zum Trinken hinsetzen"
    L["Resurrection"] = "Wiederbelebung"
    L["Announces when an enemy tries to resurrect a teammate"] = "Warnt wenn Gegner versuchen Teammitglieder wiederzubeleben"
    L["New enemies"] = "Neue Gegner"
    L["Announces when new enemies are discovered"] = "Gibt an, wenn neue Gegner entdeckt wurden"
    L["Spec Detection"] = "Talent Entdeckung"
    L["Announces when the spec of an enemy was detected"] = "Gibt an, wenn Talente eines Gegners entdeckt wurden"
    L["Low health"] = "Wenig Leben"
    L["Announces when an enemy drops below a certain health threshold"] = "Warnt, wenn das Leben eines Gegners unter einen bestimmten Prozentwert fällt"
    L["Low health threshold"] = "Prozentwert: Wenig Leben"
    L["Choose how low an enemy must be before low health is announced"] = "Bestimme wie wenig Leben ein Gegner haben muss, damit vor wenig Leben gewarnt wird"
    L["Destination"] = "Ziel"
    L["Choose how your announcements are displayed"] = "Bestimme wo Warnungen dargestellt werden"

    -- ArenaCountDown.lua
    L["Arena Countdown"] = "Arena Countdown"
    L["Turns countdown before the start of an arena match on/off."] = ""
    L["Size"] = "Größe"

    -- Auras.lua
    L["Auras"] = "Auren"
    L["Frame"] = "Frame"
    L["Cooldown"] = "Abklingzeit"
    L["No Cooldown Circle"] = "Verstecke Abklingzeitzirkel"
    L["Cooldown circle alpha"] = "Abklingzeitzirkel Alpha"
    L["Font"] = "Schrift"
    L["Font of the cooldown"] = "Schrift der Abklingzeit"
    L["Font scale"] = "Schriftskalierung"
    L["Scale of the text"] = "Skalierungfaktor des Texts"
    L["Font color"] = "Schriftfarbe"
    L["Color of the text"] = "Farbe des Texts"
    L["Border"] = "Rahmen"
    L["Border style"] = "Rahmen Stil"
    L["Buff color"] = "Buff Farbe"
    L["Debuff color"] = "Debuff Farbe"
    L["Check All"] = "Alle auswählen"
    L["Uncheck All"] = "Alle abwählen"
    L["Enabled"] = "Eingeschaltet"
    L["Priority"] = "Priorität"

    -- BuffsDebuffs.lua
    L["Buffs and Debuffs"] = "Buffs und Debuffs"
    L["Enabled Buffs and Debuffs module"] = "Buffs und Debuffs Modul einschalten"
    L["Show CC"] = "Zeige Crowdcontrol"
    L["Shows all debuffs, which are displayed on the ClassIcon as well"] = "Zeigt alle Buffs & Debuffs, die auch auf dem Klassensymbol dargestellt werden"
    L["Buffs"] = "Buffs"
    L["Size & Padding"] = "Größe und Abstand"
    L["Icon Size"] = "Symbol Größe"
    L["Size of the DR Icons"] = "Größe der DR Symbole"
    L["Icon Width Factor"] = "Symbol Breitenfaktor"
    L["Stretches the icon"] = "Streckt das Symbol"
    L["Icon Padding"] = "Symbol Abstand"
    L["Space between Icons"] = "Abstand zwischen den Symbolen"
    L["Position"] = "Position"
    L["Aura Position"] = "Aura Position"
    L["Position of the aura icons"] = "Position der Aura Symbole"
    L["Top"] = "Oben"
    L["Bottom"] = "Unten"
    L["Left"] = "Links"
    L["Right"] = "Rechts"
    L["Grow Direction"] = "Richtung"
    L["Grow Direction of the aura icons"] = "In welche Richtung die Symbole wachsen"
    L["Horizontal offset"] = "Horizontaler Offset"
    L["Vertical offset"] = "Vertikaler Offset"
    L["Alpha"] = "Alpha"
    L["Debuffs"] = "Debuffs"
    L["Dynamic Timer Color"] = "Dynamische Textfarbe"
    L["Show dynamic color on cooldown numbers"] = "Verändert die Farbe des Textes dynamisch"
    L["Color of the cooldown timer and stacks"] = "Farbe der Abklingzeit und Stapel"
    L["Spell School Colors"] = "Zauberart Farbe"
    L["Spell School Colors Enabled"] = "Zauberart Farbe Eingeschaltet"
    L["Show border colors by spell school"] = "Färbt den Rahmen entspechend der Zauberart"
    L["Curse"] = "Fluch"
    L["Color of the border"] = "Farbe des Rahmens"
    L["Magic"] = "Magie"
    L["Poison"] = "Gift"
    L["Physical"] = "Physisch"
    L["Immune"] = "Immun"
    L["Disease"] = "Erkrankung"
    L["Aura"] = "Aura"
    L["Form"] = "Form"

    -- Castbar.lua
    L["Cast Bar"] = "Zauberleiste"
    L["Bar"] = "Balken"
    L["Bar Size"] = "Balken Größe"
    L["Bar height"] = "Balken Höhe"
    L["Height of the bar"] = "Höhe des Balken"
    L["Bar width"] = "Balken Weite"
    L["Width of the bars"] = "Weite des Balken"
    L["Texture"] = "Textur"
    L["Bar texture"] = "Balken Textur"
    L["Texture of the bar"] = "Textur des Balken"
    L["Bar color"] = "Balken Farbe"
    L["Color of the cast bar"] = "Farbe des Balken"
    L["Background color"] = "Hintergrundfarbe"
    L["Color of the cast bar background"] = "Hinergrundfarbe des Zauberbalkens"
    L["Border size"] = "Rahmen Größe"
    L["Status Bar border"] = "Balken Rahmen"
    L["Status Bar border color"] = "Balken Rahmen Farbe"
    L["Icon"] = "Symbol"
    L["Icon size"] = "Symbolgröße"
    L["Icon border"] = "Symbolrahmen"
    L["Icon border color"] = "Farbe Symbolrahmen"
    L["Spark"] = "Funke"
    L["Spark enabled"] = "Funke eingeschaltet"
    L["Spark color"] = "Funkenfarbe"
    L["Color of the cast bar spark"] = "Farbe des Zauberleisten Funke"
    L["Font of the castbar"] = "Schriftart der Zauberleiste"
    L["Font size"] = "Schriftgröße"
    L["Size of the text"] = "Schriftgröße"
    L["Format"] = "Darstellung"
    L["Timer Format"] = "Zeitdarstellung"
    L["Remaining"] = "Verbleibend"
    L["Total"] = "Total"
    L["Both"] = "Beides"
    L["Castbar position"] = "Zauberleistenposition"
    L["Icon position"] = "Symbolposition"
    L["Offsets"] = "Offsets"

    -- Classicon.lua
    L["Class Icon"] = "Klassensymbol"
    L["Balance"] = "Gleichgewicht"
    L["Feral"] = "Wilder Kampf"
    L["Restoration"] = "Wiederherstellung"
    L["Beast Mastery"] = "Tierherrschaft"
    L["Marksmanship"] = "Treffsicherheit"
    L["Survival"] = "Überleben"
    L["Arcane"] = "Arkan"
    L["Fire"] = "Feuer"
    L["Frost"] = "Frost"
    L["Holy"] = "Heilig"
    L["Protection"] = "Schutz"
    L["Retribution"] = "Vergeltung"
    L["Discipline"] = "Disziplin"
    L["Shadow"] = "Schatten"
    L["Assassination"] = "Meucheln"
    L["Combat"] = "Kampf"
    L["Subtlety"] = "Täuschung"
    L["Elemental"] = "Elemental"
    L["Enhancement"] = "Verstärkung"
    L["Affliction"] = "Gebrechen"
    L["Demonology"] = "Demonologie"
    L["Destruction"] = "Zerstörung"
    L["Arms"] = "Waffen"
    L["Fury"] = "Furor"
    L["Show Spec Icon"] = "Zeige Spezialisierungssymbol"
    L["Shows Spec Icon once spec is detected"] = "Zeigt das Talentspezialisierungs Symbol sobald die Spezialisierung erkannt wurde"
    L["Icon width factor"] = "Symbol Breitenfaktor"
    L["This changes positions with trinket"] = "Das tauscht die Position mit dem Trinket, wenn auf der gleichen Seite."
    L["Border color"] = "Rahmenfarbe"

    --CombatIndicator.lua
    L["Combat Indicator"] = "Kampfindikator"
    L["Enable Combat Indicator icon"] = "Schalte Kampfindikator ein"
    L["Anchor"] = "Anker"
    L["This changes the anchor of the ci icon"] = "Dies ändert den Anker des Kampfindikatorsymbols"
    L["This changes position relative to its anchor of the ci icon"] = "Dies ändert die Position relativ zum Anker"

    -- Cooldowns.lua
    L["Cooldowns"] = "Abklingzeiten"
    L["Enabled cooldown module"] = "Включен модуль кулдаунов"
    L["Cooldown size"] = "Abklingzeit Größe"
    L["Size of each cd icon"] = "Größe eines einzelnen Symbols"
    L["Max Icons per row"] = "Maximale Anzahl an Symbolen pro Reihe"
    L["Scale of the font"] = "Skalierung der Schrift"
    L["Anchor of the cooldown icons"] = "Anker der Abklingzeiten Symbole"
    L["Grow Direction of the cooldown icons"] = "Richtung der Abklingzeiten Symbole"
    L["Offset"] = "Offset"

    -- Diminishings.lua
    L["Diminishings"] = "DR"
    L["Enabled DR module"] = "DR einschalten"
    L["DR Cooldown position"] = "DR Position"
    L["Position of the cooldown icons"] = "Position der Symbole"
    L["DR Border Colors"] = "DR Rahmen Farbe"
    L["Dr Border Colors Enabled"] = "DR Rahmen Farben eingeschaltet"
    L["Colors borders of DRs in respective DR-color below"] = "Färbt die Rahmen der DR Symbole je nach Stärke der Verminderung"
    L["Half"] = "Hälfte"
    L["Quarter"] = "Viertel"
    L["Categories"] = "Kategorien"
    L["Force Icon"] = "Erzwinge Symbol"
    L["Icon of the DR"] = "Symbol des DR"
    L["DR Duration"] = "DR Dauer"
    L["Change the DR Duration in seconds (DR is dynamic between 15-20s)"] = "Verändere die DR Dauer in Sekunden (DR ist dynamisch zwischen 15-20s)"

    -- ExportImport.lua
    L["Export Import"] = "Exportieren Importieren"
    L["Profile Export Import"] = "Profile Exportieren Importieren"

    -- Healthbar.lua
    L["Health Bar"] = "Lebensbalken"
    L["DEAD"] = "TOT"
    L["LEAVE"] = "VERLASSEN"
    L["General"] = "Allgemein"
    L["Color of the status bar background"] = "Farbe des Balkenhintergrunds"
    L["Font of the bar"] = "Schriftart des Balken"
    L["Name font size"] = "Schriftgröße des Namen"
    L["Size of the name text"] = "Schriftgröße des Namen"
    L["Health font size"] = "Schriftgröße der Gesundheit"
    L["Size of the health text"] = "Schriftgröße der Gesundheit"
    L["Size of the border"] = "Rahmengröße"
    L["Health Bar Text"] = "Lebensbalken Text"
    L["Show name text"] = "Namen zeigen"
    L["Show the units name"] = "Zeige den Namen des Gegners"
    L["Show ArenaX"] = "ArenaX zeigen"
    L["Show 1-5 as name instead"] = "Zeigt 1-5 anstatt des Namens"
    L["Show the actual health"] = "Zeige die momentane Gesundheit"
    L["Show the actual health on the health bar"] = "Zeigt die momentane Gesundheit"
    L["Show max health"] = "Zeige maximale Gesundheit"
    L["Show max health on the health bar"] = "Zeige maximale Gesundheit"
    L["Show health percentage"] = "Zeige Prozentwert"
    L["Show health percentage on the health bar"] = "Zeige Prozentwert der Gesundheit"

    -- Highlight.lua
    L["Highlight"] = "Hervorhebung"
    L["Show Inside"] = "Zeige innen"
    L["Show Highlight border inside of frame"] = "Zeige die Hervorhebung innerhalb des Frames"
    L["Colors"] = "Farben"
    L["Target border color"] = "Rahmenfarbe deines Ziels"
    L["Color of the selected targets border"] = "Rahmenfarbe deines momentanen Ziels"
    L["Focus border color"] = "Rahmenfarbe deines Fokus"
    L["Color of the focus border"] = "Rahmenfarbe deines momentanen Fokus"
    L["Highlight target"] = "Hervorhebung des Ziels"
    L["Toggle if the selected target should be highlighted"] = "Ziel hervorheben ein/ausschalten"
    L["Show border around target"] = "Zeige Rahmen um dein Ziel"
    L["Toggle if a border should be shown around the selected target"] = "Zeigt Rahmen um dein momentanes Ziel"
    L["Show border around focus"] = "Zeige Rahmen um dein Fokus"
    L["Toggle of a border should be shown around the current focus"] = "Zeigt Rahmen um dein Fokusziel"

    -- Pets.lua
    L["Pets"] = "Begleiter"
    L["Enables Pets module"] = "Schaltet das Begleiter Modul ein"
    L["Width of the bar"] = "Breite des Balkens"
    L["Health color"] = "Gesundheitsfarbe"
    L["Color of the status bar"] = "Farbe des Balkens"
    L["Portrait"] = "Portrait"
    L["Health Values"] = "Gesundheitswerte"

    -- Powerbar.lua
    L["Power Bar"] = "Mana/Energie Balken"
    L["Power Bar Text"] = "Mana/Energie Balken Text"
    L["Power Texts"] = "Mana/Energie Balken Texte"
    L["Show race"] = "Rasse zeigen"
    L["Show spec"] = "Spezialisierung zeigen"
    L["Show the actual power"] = "Zeige das momentane Mana"
    L["Show the actual power on the power bar"] = "Zeige das momentane Mana"
    L["Show max power"] = "Zeige das maximale Mana"
    L["Show max power on the power bar"] = "Zeige das maximale Mana"
    L["Show power percentage"] = "Zeige Prozentwert"
    L["Show power percentage on the power bar"] = "Zeige Prozentwert"

    -- Racial.lua
    L["Racial"] = "Rassenfertigkeit"
    L["Enable racial icon"] = "Rassenfertigkeit einschalten"
    L["This changes the anchor of the racial icon"] = "Dies ändert den Anker des Rassenfertigkeitssymbols"
    L["This changes position relative to its anchor of the racial icon"] = "Dies ändert doe Position relativ zu seinem Anker"

    -- TotemPlates.lua
    L["Totem Plates"] = "Totem Symbole"
    L["Customize Totems"] = "Individuelle Totemeinstellungen"
    L["Custom totem name"] = "Individueller Totem Name"
    L["Totem General"] = "Totems Allgemein"
    L["Turns totem icons instead of nameplates on or off. (Requires reload)"] = ""
    L["Show friendly"] = "Zeige für freundliche"
    L["Show enemy"] = "Zeige für feindliche"
    L["Totem size"] = "Totem Größe"
    L["Size of totem icons"] = "Größe der Totemsymbole"
    L["Font of the custom totem name"] = "Schriftart der benutzerdefinierten Totem Namen"
    L["Apply alpha when no target"] = "Wende den Alpha-Wert an, wenn kein Ziel anvisiert ist"
    L["Always applies alpha, even when you don't have a target. Else it is 1."] = "Alpha immer anwenden, auch wenn man kein Ziel anvisiert hat. Sonst ist der Alpha-Wert 1"
    L["Apply alpha when targeted"] = "Wende den Alpha-Wert an, wenn das Totem als Ziel anvisiert ist"
    L["Always applies alpha, even when you target the totem. Else it is 1."] = "Alpha immer anwenden, auch wenn das Totem als Ziel anvisiert ist. Sonst ist der Alpha-Wert 1"
    L["All totem border alphas (configurable per totem)"] = "Alpha aller Totems"
    L["Totem icon border style"] = "Totem Rahmenstil"
    L["All totem border color"] = "Rahmenfarbe aller Totems"

    -- Trinket.lua
    L["Trinket"] = "Insignie"
    L["Enable trinket icon"] = "Insignie einschalten"
    L["This changes positions of the trinket"] = "Dies ändert die Position der Insignie"

    -- XiconProfiles.lua
    L["Profile"] = "Profil"

    -- Frame.lua
    L["Gladdy - drag to move"] = "Gladdy - ziehe um zu bewegen"

    -- Gladdy.lua
    L["Welcome to Gladdy!"] = "Willkommen bei Gladdy!"
    L["First run has been detected, displaying test frame."] = "Erster Start wurde entdeckt, zeige Testbild an."
    L["Valid slash commands are:"] = "Gültige slash Befehle sind:"
    L["If this is not your first run please lock or move the frame to prevent this from happening."] = "Wenn dies nicht dein erster Start ist, sperre oder bewege das Bild um diese Meldung zu verhindern."

    -- Options.lua
    L["settings"] = "Einstellungen"
    L["Reset module"] = "Modul zurücksetzen"
    L["Reset module to defaults"] = "Setze das Modul auf seine Standardwerte zurück"
    L["No settings"] = "Keine Einstellungen"
    L["Module has no settings"] = "Modul hat keine Einstellungen"
    L["General settings"] = "Allgemeine Einstellungen"
    L["Lock frame"] = "Sperre Frame"
    L["Toggle if frame can be moved"] = "Aktivieren falls das Frame bewegt werden kann"
    L["Grow frame upwards"] = "Frame von unten nach oben aufbauen"
    L["If enabled the frame will grow upwards instead of downwards"] = "Falls aktiviert, wird das Frame von unten nach oben aufgebaut"
    L["Down"] = "Runter"
    L["Up"] = "Hoch"
    L["Frame General"] = "Frame Allgemein"
    L["Frame scale"] = "Frame Skalierung"
    L["Scale of the frame"] = "Skalierung des Frames"
    L["Frame padding"] = "Symbolabstand"
    L["Padding of the frame"] = "Abstand zwischen den Elementen des Frames"
    L["Frame width"] = "Frame Breite"
    L["Margin"] = "Frame Abstand"
    L["Margin between each button"] = "Abstand zwischen den Arena Einheiten"
    L["Cooldown General"] = "Abklingzeiten Allgemein"
    L["Font General"] = "Schriftart Allgemein"
    L["General Font"] = "Allgemeine Schriftart"
    L["Font color text"] = "Schriftfarbe von text"
    L["Font color timer"] = "Schriftfarbe von Abklingzeiten"
    L["Color of the timers"] = "Farbe der Abklingzeiten"
    L["Icons General"] = "Symbol Allgemein"
    L["Icon border style"] = "Rahmenstil"
    L["This changes the border style of all icons"] = "Dies ändert den Rahmenstil aller Symbole"
    L["This changes the border color of all icons"] = "Dies ändert die Rahmenfarbe aller Symbole"
    L["Statusbar General"] = "Balken Allgemein"
    L["Statusbar texture"] = "Balken Textur"
    L["This changes the texture of all statusbar frames"] = "Dies ändert die Textur aller Balken"
    L["Statusbar border style"] = "Balken Rahmenstil"
    L["This changes the border style of all statusbar frames"] = "Dies ändert den Rahmenstil aller Balken"
    L["Statusbar border offset divider (smaller is higher offset)"] = "Rahmenstil offset Quotient"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "Offset des Rahmens zur Statusbar (falls der Balken hinter dem Rahmen erscheint)"
    L["Statusbar border color"] = "Balken Rahmenfarbe"
    L["This changes the border color of all statusbar frames"] = "Dies ändert die Rahmenfarbe aller Balken"
    L["Gladdy Tooltip round"] = "Gladdy Tooltip rund"
    L["Gladdy Tooltip squared"] = "Gladdy Tooltip quadratisch"
    L["Gloss (black border)"] = "Glanz (schwarzer Rand)"
    L["Unlock frame"] = "Frame entsperren"
    L["Show Mover"] = "Mover anzeigen"
    L["Hide Mover"] = "Mover verstecken"
    L["Toggle to show Mover Frames"] = "Mover Frames ein/ausschalten"
    L["Test"] = "Test"
    L["Show Test frames"] = "Test Frames anzeigen"
    L["Hide"] = "Verstecken"
    L["Hide frames"] = "Frames verstecken"
    L["ReloadUI"] = "UI neu laden"
    L["Reloads the UI"] = "Lädt die UI neu"
    L["Grow Middle"] = "Von der Mitte wachsen"
    L["Frames expand along a centric anchor"] = "Frames erweitern sich entlang eines zentralen Ankers"
    L["Pixel Perfect Scale"] = "Pixel-Perfekte Skalierung"
    L["Enables Pixel Perfect Scale - disables manual "] = "Aktiviert Pixel-Perfekte Skalierung - deaktiviert manuelle "
    L["OmniCC Enabled"] = "OmniCC aktiviert"
    L["Frame width"] = "Frame Breite"
    L["Background color"] = "Hintergrundfarbe"
    L["Background Color of the frame"] = "Hintergrundfarbe des Frames"
    L["Cooldown General"] = "Abklingzeiten Allgemein"
    L["Font General"] = "Schriftart Allgemein"
    L["General Font"] = "Allgemeine Schriftart"
    L["Font color text"] = "Schriftfarbe von text"
    L["Font color timer"] = "Schriftfarbe von Abklingzeiten"
    L["Color of the timers"] = "Farbe der Abklingzeiten"
    L["Icons General"] = "Symbol Allgemein"
    L["Icon border style"] = "Rahmenstil"
    L["This changes the border style of all icons"] = "Dies ändert den Rahmenstil aller Symbole"
    L["Icon border color"] = "Rahmenfarbe aller Symbole"
    L["This changes the border color of all icons"] = "Dies ändert die Rahmenfarbe aller Symbole"
    L["Statusbar General"] = "Balken Allgemein"
    L["Statusbar texture"] = "Balken Textur"
    L["This changes the texture of all statusbar frames"] = "Dies ändert die Textur aller Balken"
    L["Statusbar border style"] = "Balken Rahmenstil"
    L["This changes the border style of all statusbar frames"] = "Dies ändert den Rahmenstil aller Balken"
    L["Statusbar border offset divider (smaller is higher offset)"] = "Rahmenstil offset Quotient"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "Offset des Rahmens zur Statusbar (falls der Balken hinter dem Rahmen erscheint)"
    L["Hide Blizzard"] = "Blizzard verstecken"
    L["Arena only"] = "Nur Arena"
    L["Never"] = "Niemals"
    L["Always"] = "Immer"
    L["Grow Direction"] = "Wachstumsrichtung"
    L["Down"] = "Runter"
    L["Up"] = "Hoch"
    L["Load configuration"] = "Konfiguration laden"
    L["Load configuration options"] = "Konfigurationsoptionen laden"

    -- Nameplates.lua
    L["Show important auras on nameplates"] = "Zeige wichtige Auren auf Namensplaketten"
    L["Zoomed Icon"] = "Vergrößertes Symbol"
    L["Zoomes the icon to remove borders"] = "Vergrößert das Symbol um Rahmen zu entfernen"
    L["Icon size"] = "Symbolgröße"
    L["Size of each aura icon"] = "Größe jedes Aura Symbols"
    L["Icon Width Factor"] = "Symbol Breitenfaktor"
    L["Stretches the icon"] = "Streckt das Symbol"
    L["Max Icons"] = "Max Symbole"
    L["Maximum number of aura icons to show"] = "Maximale Anzahl an Aura Symbolen zum Anzeigen"
    L["Icon Padding"] = "Symbol Abstand"
    L["Space between icons"] = "Abstand zwischen Symbolen"
    L["Horizontal offset"] = "Horizontaler Offset"
    L["Vertical offset"] = "Vertikaler Offset"
    L["Center Icons"] = "Symbole zentrieren"
    L["Sort Order"] = "Sortierreihenfolge"
    L["How to sort aura icons"] = "Wie Aura Symbole sortiert werden"
    L["Time"] = "Zeit"
    L["Time Left"] = "Verbleibende Zeit"
    L["Priority"] = "Priorität"
    L["Frame Strata and Level"] = "Frame Schicht und Level"
    L["Frame Strata"] = "Frame Schicht"
    L["Frame Level"] = "Frame Level"
    L["Aura List"] = "Aura Liste"
    L["Configure which auras to show on nameplates"] = "Konfiguriere welche Auren auf Namensplaketten angezeigt werden"
    L["Note: Uses the same aura list as the Auras module. Configure auras in the Auras module settings."] = "Hinweis: Verwendet die gleiche Aura Liste wie das Auren Modul. Konfiguriere Auren in den Auren Modul Einstellungen."

    -- Diminishings.lua
    L["Show when Aura applied"] = "Zeige wenn Aura angewendet"
    L["Group"] = "Gruppe"
    L["DR Icon"] = "DR Symbol"
    L["Group direction"] = "Gruppenrichtung"
    L["Cooldown Font"] = "Abklingzeit Schriftart"
    L["Enable DR Colors as Font Color"] = "Aktiviere DR Farben als Schriftfarbe"
    L["Shows the current DR Level on the DR icon."] = "Zeigt das aktuelle DR Level auf dem DR Symbol."
    L["Cooldown number alpha"] = "Abklingzeit Zahl Alpha"
    L["Font Outline"] = "Schriftumriss"
    L["X Offset"] = "X Offset"
    L["Y Offset"] = "Y Offset"
    L["DR Font"] = "DR Schriftart"
    L["Enable DR Font"] = "Aktiviere DR Schriftart"
    L["DR Font color"] = "DR Schriftfarbe"
    L["Color of the font"] = "Farbe der Schriftart"
    L["Font of the DR Font"] = "Schriftart der DR Schriftart"
    L["DR Colors"] = "DR Farben"
    L["DR Texts"] = "DR Texte"
    L["Text on half DR"] = "Text bei halbem DR"
    L["Text on quarter DR"] = "Text bei viertel DR"
    L["Text on immune DR"] = "Text bei immunem DR"
    L["Enable DR Colors as Border Color"] = "Aktiviere DR Farben als Rahmenfarbe"
    L["Colors borders of DRs in respective DR Colors"] = "Färbt Rahmen von DRs in entsprechenden DR Farben"
    L["Border color"] = "Rahmenfarbe"
    L["DR Grow Direction"] = "DR Wachstumsrichtung"
    L["Grow Direction of the dr icons"] = "Wachstumsrichtung der DR Symbole"
    L["Change the DR Duration in seconds (DR is dynamic between 15-20s)"] = "Ändere die DR Dauer in Sekunden (DR ist dynamisch zwischen 15-20s)"
    L["DR Duration"] = "DR Dauer"
    L["Space between Icons"] = "Abstand zwischen Symbolen"

    -- Clicks.lua
    L["Action #%d"] = "Aktion #%d"
    L["Left Click Enemy 1"] = "Linksklick Gegner 1"
    L["Left Click Enemy 2"] = "Linksklick Gegner 2"
    L["Left Click Enemy 3"] = "Linksklick Gegner 3"
    L["Left Click Enemy 4"] = "Linksklick Gegner 4"
    L["Left Click Enemy 5"] = "Linksklick Gegner 5"
    L["Right Click Enemy 1"] = "Rechtsklick Gegner 1"
    L["Right Click Enemy 2"] = "Rechtsklick Gegner 2"
    L["Right Click Enemy 3"] = "Rechtsklick Gegner 3"
    L["Right Click Enemy 4"] = "Rechtsklick Gegner 4"
    L["Right Click Enemy 5"] = "Rechtsklick Gegner 5"
    L["Middle Click Enemy 1"] = "Mittelklick Gegner 1"
    L["Middle Click Enemy 2"] = "Mittelklick Gegner 2"
    L["Middle Click Enemy 3"] = "Mittelklick Gegner 3"
    L["Middle Click Enemy 4"] = "Mittelklick Gegner 4"
    L["Middle Click Enemy 5"] = "Mittelklick Gegner 5"
    L["Button4 Click Enemy 1"] = "Taste4 Klick Gegner 1"
    L["Button4 Click Enemy 2"] = "Taste4 Klick Gegner 2"
    L["Button4 Click Enemy 3"] = "Taste4 Klick Gegner 3"
    L["Button4 Click Enemy 4"] = "Taste4 Klick Gegner 4"
    L["Button4 Click Enemy 5"] = "Taste4 Klick Gegner 5"
    L["Button5 Click Enemy 1"] = "Taste5 Klick Gegner 1"
    L["Button5 Click Enemy 2"] = "Taste5 Klick Gegner 2"
    L["Button5 Click Enemy 3"] = "Taste5 Klick Gegner 3"
    L["Button5 Click Enemy 4"] = "Taste5 Klick Gegner 4"
    L["Button5 Click Enemy 5"] = "Taste5 Klick Gegner 5"
    L["Left button"] = "Linke Taste"
    L["Right button"] = "Rechte Taste"
    L["Middle button"] = "Mittlere Taste"
    L["Button 4"] = "Taste 4"
    L["Button 5"] = "Taste 5"
    L["None"] = "Keine"
    L["All"] = "Alle"
    L["CTRL"] = "STRG"
    L["SHIFT"] = "UMSCHALT"
    L["ALT"] = "ALT"
    L["Cast Spell"] = "Zauber wirken"
    L["Name"] = "Name"
    L["Select the name of the click option"] = "Wähle den Namen der Klick Option"
    L["Button"] = "Taste"
    L["Select which mouse button to use"] = "Wähle welche Maustaste verwendet werden soll"
    L["Modifier"] = "Modifikator"
    L["Select which modifier to use"] = "Wähle welcher Modifikator verwendet werden soll"
    L["Action"] = "Aktion"
    L["Select what action this mouse button does"] = "Wähle welche Aktion diese Maustaste ausführt"
    L["Cast Spell / Macro"] = "Zauber wirken / Makro"
    L["Target"] = "Ziel"
    L["Focus"] = "Fokus"

    -- Additional missing keys
    L["Frame"] = "Frame"
    L["Enabled"] = "Eingeschaltet"
    L["Icon"] = "Symbol"
    L["Position"] = "Position"
    L["Cooldown"] = "Abklingzeit"
    L["Font"] = "Schrift"
    L["Font scale"] = "Schriftskalierung"
    L["Font color"] = "Schriftfarbe"
    L["Border"] = "Rahmen"
    L["Border style"] = "Rahmen Stil"
    L["Buff color"] = "Buff Farbe"
    L["Debuff color"] = "Debuff Farbe"
    L["Check All"] = "Alle auswählen"
    L["Uncheck All"] = "Alle abwählen"
    L["Force Icon"] = "Erzwinge Symbol"
    L["Icon of the DR"] = "Symbol des DR"
    L["Categories"] = "Kategorien"
    L["Immune"] = "Immun"
    L["Half"] = "Hälfte"
    L["Quarter"] = "Viertel"
    L["Top"] = "Oben"
    L["Bottom"] = "Unten"
    
    -- Additional missing keys for deDE
    L["No Pet"] = "Kein Begleiter"
    L["0%"] = "0%"
    L["100%"] = "100%"
    L["50%"] = "50%"
    L["Above"] = "Oberhalb"
    L["Add Aura"] = "Aura hinzufügen"
    L["Add Buff"] = "Buff hinzufügen"
    L["Add Debuff"] = "Debuff hinzufügen"
    L["All enemy totem border color"] = "Rahmenfarbe aller feindlichen Totems"
    L["All friendly totem border color"] = "Rahmenfarbe aller freundlichen Totems"
    L["Alpha of the icon when cooldown active"] = "Alpha des Symbols wenn Abklingzeit aktiv"
    L["Announce Spell School"] = "Zauberart ankündigen"
    L["Announces Spell School like \"Holy\" instead of Spell Name"] = "Kündigt Zauberart wie \"Heilig\" statt Zaubername an"
    L["Announces when enemies' spells are interrupted"] = "Kündigt an wenn Zauber der Gegner unterbrochen werden"
    L["Attach To TotemPlate"] = "An Totem-Tafel anhängen"
    L["Aura Detached"] = "Aura getrennt"
    L["Aura X Offset"] = "Aura X Versatz"
    L["Aura Y Offset"] = "Aura Y Versatz"
    L["Aura size"] = "Aura Größe"
    L["Aura width factor"] = "Aura Breitenfaktor"
    L["Background"] = "Hintergrund"
    L["Bar Color"] = "Balken Farbe"
    L["Bar horizontal"] = "Balken horizontal"
    L["Bar vertical"] = "Balken vertikal"
    L["BeastMastery"] = "Tierherrschaft"
    L["Below"] = "Unterhalb"
    L["Blood"] = "Blut"
    L["Brewmaster"] = "Braumeister"
    L["Buff Lists"] = "Buff Listen"
    L["By Arena"] = "Nach Arena"
    L["By Order"] = "Nach Reihenfolge"
    L["COOLDOWN USED: %s (%s) used %s - %s sec. cooldown"] = "ABKLINGZEIT VERWENDET: %s (%s) verwendete %s - %s Sek. Abklingzeit"
    L["Class Colored Bars"] = "Klassenfarbige Balken"
    L["Class Icon Enabled"] = "Klassensymbol aktiviert"
    L["Class colored"] = "Klassenfarbig"
    L["Colored trinket"] = "Farbige Insignie"
    L["Colored trinket CD"] = "Farbige Insignie mit CD"
    L["Colored trinket No CD"] = "Farbige Insignie ohne CD"
    L["Custom Colors"] = "Benutzerdefinierte Farben"
    L["Custom Tags"] = "Benutzerdefinierte Tags"
    L["Custom Tags Enabled"] = "Benutzerdefinierte Tags aktiviert"
    L["Default Aura"] = "Standard Aura"
    L["Delete"] = "Löschen"
    L["Desaturate Icon"] = "Symbol entsättigen"
    L["Detached Aura"] = "Getrennte Aura"
    L["Detached Interrupt"] = "Getrenntes Unterbrechen"
    L["Detached Mode"] = "Getrennter Modus"
    L["Dialog"] = "Dialog"
    L["Dispel Icon"] = "Bannen Symbol"
    L["Dispel Icon Enabled"] = "Bannen Symbol aktiviert"
    L["Dynamic Icon"] = "Dynamisches Symbol"
    L["Enable"] = "Aktivieren"
    L["Enable Aura to be displayed"] = "Aura zur Anzeige aktivieren"
    L["Enable Custom Colors"] = "Benutzerdefinierte Farben aktivieren"
    L["Enable Interrupt Spell School Colors"] = "Zauberart Farben für Unterbrechungen aktivieren"
    L["Enabled Racials"] = "Rassenfertigkeiten aktiviert"
    L["Enabled showing racial"] = "Anzeige der Rassenfertigkeit aktiviert"
    L["Enables Targets module"] = "Ziele Modul aktivieren"
    L["Enemy"] = "Feindlich"
    L["Enrage"] = "Wut"
    L["Export"] = "Exportieren"
    L["Export your current profile to share with others or your various accounts."] = "Exportiere dein aktuelles Profil um es mit anderen oder deinen verschiedenen Accounts zu teilen."
    L["Fade"] = "Verblassen"
    L["Font Enabled"] = "Schrift aktiviert"
    L["Font of the custom totem name"] = "Schriftart des benutzerdefinierten Totem Namens"
    L["Font size left text"] = "Schriftgröße linker Text"
    L["Font size right text"] = "Schriftgröße rechter Text"
    L["Form"] = "Form"
    L["Friendly"] = "Freundlich"
    L["Fullscreen"] = "Vollbild"
    L["Fullscreen Dialog"] = "Vollbild Dialog"
    L["Group Class Icons"] = "Gruppen Klassensymbole"
    L["Group Mode"] = "Gruppen Modus"
    L["Group Pets"] = "Gruppen Begleiter"
    L["Group Targets"] = "Gruppen Ziele"
    L["Guardian"] = "Wächter"
    L["Health Bar Text"] = "Lebensbalken Text"
    L["Health Values"] = "Gesundheitswerte"
    L["Height"] = "Höhe"
    L["Hide Disabled Totem Plates"] = "Deaktivierte Totem Tafeln verstecken"
    L["High"] = "Hoch"
    L["Higher is darker"] = "Höher ist dunkler"
    L["Highlight mouseover"] = "Hervorhebung bei Mausüber"
    L["Horizontal Grow Direction"] = "Horizontale Wachstumsrichtung"
    L["Horizontal Grow Direction of the cooldown icons"] = "Horizontale Wachstumsrichtung der Abklingzeit Symbole"
    L["Icon Enabled"] = "Symbol aktiviert"
    L["Icon border"] = "Symbol Rahmen"
    L["If test is running, type \"/gladdy test\" again"] = "Wenn Test läuft, tippe \"/gladdy test\" erneut"
    L["Import"] = "Importieren"
    L["Interrupt Detached"] = "Unterbrechung getrennt"
    L["Interrupt Spells School Colors"] = "Zauberart Farben für Unterbrechungen"
    L["Interrupt X Offset"] = "Unterbrechung X Versatz"
    L["Interrupt Y Offset"] = "Unterbrechung Y Versatz"
    L["Interrupt size"] = "Unterbrechung Größe"
    L["Interrupt width factor"] = "Unterbrechung Breitenfaktor"
    L["Interrupts"] = "Unterbrechungen"
    L["INTERRUPTED: %s (%s)"] = "UNTERBROCHEN: %s (%s)"
    L["Layer Group"] = "Schicht Gruppe"
    L["Layer Group and Level"] = "Schicht Gruppe und Level"
    L["Layer Level"] = "Schicht Level"
    L["Left Font Outline"] = "Linker Schriftumriss"
    L["Left Text"] = "Linker Text"
    L["Left Text Horizontal Offset"] = "Linker Text horizontaler Versatz"
    L["Left Text Vertical Offset"] = "Linker Text vertikaler Versatz"
    L["Left font size"] = "Linke Schriftgröße"
    L["Locked"] = "Gesperrt"
    L["Low"] = "Niedrig"
    L["Max"] = "Max"
    L["Medium"] = "Mittel"
    L["Mistweaver"] = "Nebelweber"
    L["Nature"] = "Natur"
    L["No Cooldown Circle"] = "Kein Abklingzeitzirkel"
    L["Cooldowncircle"] = "Abklingzeitzirkel"
    L["Only numbers, comma or space is allowed. \n Example: 124, 51243, 4121, 41111"] = "Nur Zahlen, Komma oder Leerzeichen erlaubt. \n Beispiel: 124, 51243, 4121, 41111"
    L["Out of Range Darkening Level"] = "Dunkelheitsstufe außerhalb der Reichweite"
    L["Power Bar Text"] = "Mana/Energie Balken Text"
    L["Power Texts"] = "Mana/Energie Balken Texte"
    L["Power font size"] = "Mana Schriftgröße"
    L["Race font size"] = "Rassen Schriftgröße"
    L["Range Check"] = "Reichweitenprüfung"
    L["Remaining"] = "Verbleibend"
    L["Reset Glow"] = "Leuchten zurücksetzen"
    L["Reset Glow Color"] = "Leuchten Farbe zurücksetzen"
    L["Reset Time"] = "Zeit zurücksetzen"
    L["Reset time in seconds"] = "Zurücksetzzeit in Sekunden"
    L["Reverse"] = "Umkehren"
    L["Right Font Outline"] = "Rechter Schriftumriss"
    L["Right Text"] = "Rechter Text"
    L["Right Text Horizontal Offset"] = "Rechter Text horizontaler Versatz"
    L["Right Text Vertical Offset"] = "Rechter Text vertikaler Versatz"
    L["Right font size"] = "Rechte Schriftgröße"
    L["Scale"] = "Skalierung"
    L["Scourge"] = "Geißel"
    L["Show Arena1-5 as name instead"] = "Zeige Arena1-5 als Name statt"
    L["Show CC"] = "Zeige Crowdcontrol"
    L["Show Spec Icon"] = "Zeige Spezialisierungssymbol"
    L["Show Spell Name"] = "Zeige Zaubername"
    L["Show Time"] = "Zeige Zeit"
    L["Show border colors by spell school"] = "Zeige Rahmenfarben nach Zauberart"
    L["Show dynamic color on cooldown numbers"] = "Zeige dynamische Farbe auf Abklingzeit Zahlen"
    L["Show name text"] = "Namen zeigen"
    L["Show race"] = "Rasse zeigen"
    L["Show spec"] = "Spezialisierung zeigen"
    L["Show the units name"] = "Zeige den Namen des Gegners"
    L["Show two timers"] = "Zeige zwei Timer"
    L["Shows a solid colored icon when off/off CD."] = "Zeigt ein einfarbiges Symbol wenn aus/ohne CD."
    L["Size"] = "Größe"
    L["Size of each cd icon"] = "Größe jedes CD Symbols"
    L["Size of the health text"] = "Schriftgröße der Gesundheit"
    L["Size of the left text"] = "Größe des linken Texts"
    L["Size of the power text"] = "Größe des Mana Texts"
    L["Size of the race text"] = "Größe des Rassen Texts"
    L["Size of the right text"] = "Größe des rechten Texts"
    L["Spark"] = "Funke"
    L["Spark enabled"] = "Funke aktiviert"
    L["Spell ID used for texture"] = "Zauber ID verwendet für Textur"
    L["Spell IDs"] = "Zauber IDs"
    L["Spell School Colors"] = "Zauberart Farbe"
    L["Spell School Colors Enabled"] = "Zauberart Farbe Eingeschaltet"
    L["Spells"] = "Zauber"
    L["Start Time"] = "Startzeit"
    L["Start time in seconds"] = "Startzeit in Sekunden"
    L["Status Bar border"] = "Balken Rahmen"
    L["Status Bar border color"] = "Balken Rahmen Farbe"
    L["Stealth Color"] = "Tarnung Farbe"
    L["Style"] = "Stil"
    L["Test With Talents"] = "Test mit Talenten"
    L["Test Without Talents"] = "Test ohne Talente"
    L["Text"] = "Text"
    L["Thick Outline"] = "Dicker Umriss"
    L["Thin Outline"] = "Dünner Umriss"
    L["This changes positions of the trinket"] = "Dies ändert die Position der Insignie"
    L["This changes positions with trinket"] = "Dies ändert die Positionen mit Insignie"
    L["This will overwrite your current profile!"] = "Dies wird dein aktuelles Profil überschreiben!"
    L["Tooltip"] = "Tooltip"
    L["Totem Pulse"] = "Totem Puls"
    L["Totem Pulse Enabled"] = "Totem Puls aktiviert"
    L["Turn on/off"] = "Ein/Ausschalten"
    L["Turns countdown before the start of an arena match on/off."] = "Schaltet den Countdown vor dem Start eines Arenakampfes ein/aus."
    L["Turns totem icons instead of nameplates on or off."] = "Schaltet Totemsymbole statt Namensplaketten ein/aus."
    L["Unholy"] = "Unheilig"
    L["Unknown"] = "Unbekannt"
    L["Vertical Bar"] = "Vertikaler Balken"
    L["Vertical Grow Direction"] = "Vertikale Wachstumsrichtung"
    L["Vertical Grow Direction of the cooldown icons"] = "Vertikale Wachstumsrichtung der Abklingzeit Symbole"
    L["Width"] = "Breite"
    L["Will use Debuff Color if disabled"] = "Verwendet Debuff Farbe wenn deaktiviert"
    L["Windwalker"] = "Windläufer"
    L["XiconProfiles"] = "Xicon Profile"
    L["yards"] = "Meter"
    L["changes "] = "Änderungen "
elseif GetLocale() == "zhTW" then
    -- Announcements.lua
    L["Announcements"] = "通報"
    L["RESURRECTING: %s (%s)"] = "復活: %s (%s) "
    L["SPEC DETECTED: %s - %s (%s)"] = "敵方天賦: %s - %s (%s)"
    L["LOW HEALTH: %s (%s)"] = "低生命值: %s (%s)"
    L["TRINKET USED: %s (%s)"] = "飾品已使用: %s (%s)"
    L["TRINKET READY: %s (%s)"] = "飾品就緒: %s (%s)"
    L["DRINKING: %s (%s)"] = "正在喝水: %s (%s)"
    L["Self"] = "玩家"
    L["Party"] = "隊伍"
    L["Raid Warning"] = "團隊警告"
    L["Blizzard's Floating Combat Text"] = "Blizzard 戰鬥浮動文字"
    L["Trinket used"] = "飾品已使用"
    L["Announce when an enemy's trinket is used"] = "當敵方使用飾品時發出通知"
    L["Trinket ready"] = "飾品就緒"
    L["Announce when an enemy's trinket is ready again"] = "當敵方飾品就緒時發出通報"
    L["Drinking"] = "正在喝水"
    L["Announces when enemies sit down to drink"] = "當敵方喝水時發出通報"
    L["Resurrection"] = "復活"
    L["Announces when an enemy tries to resurrect a teammate"] = "當敵方嘗試復活隊友時發出通報"
    L["New enemies"] = "新的敵人"
    L["Announces when new enemies are discovered"] = "當發現新的敵人時發出通報"
    L["Spec Detection"] = "天賦偵測"
    L["Announces when the spec of an enemy was detected"] = "當偵測到敵方天賦時發出通報"
    L["Low health"] = "低生命值"
    L["Announces when an enemy drops below a certain health threshold"] = "當敵方生命值低於一定條件時發出通報"
    L["Low health threshold"] = "低生命值門檻"
    L["Choose how low an enemy must be before low health is announced"] = "設定低生命值通報門檻"
    L["Destination"] = "發送通報至"
    L["Choose how your announcements are displayed"] = "選擇通報發送至哪個頻道"

    -- ArenaCountDown.lua
    L["Arena Countdown"] = "競技場計時"
    L["Turns countdown before the start of an arena match on/off."] = "在競技場開始前倒數剩餘秒數"
    L["Size"] = "大小"

    -- Auras.lua
    L["Auras"] = "光環"
    L["Frame"] = "框架"
    L["Cooldown"] = "冷卻時間"
    L["No Cooldown Circle"] = "取消圖示冷卻倒數陰影"
    L["Cooldown circle alpha"] = "冷卻倒數陰影alpha值"
    L["Font"] = "字型"
    L["Font of the cooldown"] = "設定冷卻時間字型"
    L["Font scale"] = "字體大小"
    L["Scale of the text"] = "設定字體大小"
    L["Font color"] = "字體顏色"
    L["Color of the text"] = "設定字體顏色"
    L["Border"] = "邊框"
    L["Border style"] = "邊框樣式"
    L["Buff color"] = "增益顏色"
    L["Debuff color"] = "減益顏色"
    L["Check All"] = "全選"
    L["Uncheck All"] = "取消全選"
    L["Enabled"] = "啟用"
    L["Priority"] = "優先"
    L["Interrupt Spells School Colors"] = "打斷法術分類顏色"
    L["Enable Interrupt Spell School Colors"] = "啟用"
    L["Will use Debuff Color if disabled"] = "若未啟用則使用一般減益顏色"
    L["Buffs"] = "增益" --Line 573
    L["Debuffs"] = "減益" --Line 566
    L["Interrupts"] = "斷法" --Line 580

    -- BuffsDebuffs.lua
    L["Buffs and Debuffs"] = "增益與減益"
    L["Enabled Buffs and Debuffs module"] = "啟用增益與減益模組"
    L["Show CC"] = "顯示控場"
    L["Shows all debuffs, which are displayed on the ClassIcon as well"] = "顯示所有減益效果，這些減益效果也顯示在職業圖示上"
    L["Buffs"] = "增益"
    L["Size & Padding"] = "大小與內距"
    L["Icon Size"] = "圖示大小"
    L["Size of the DR Icons"] = "遞減圖示大小"
    L["Icon Width Factor"] = "圖示寬度比例"
    L["Stretches the icon"] = "圖示寬度"
    L["Icon Padding"] = "圖示內距"
    L["Space between Icons"] = "圖示間距"
    L["Position"] = "位置"
    L["Aura Position"] = "光環位置"
    L["Position of the aura icons"] = "光環圖示位置"
    L["Top"] = "頂部"
    L["Bottom"] = "底部"
    L["Left"] = "左"
    L["Right"] = "右"
    --L["Grow Direction"] = ""
    L["Grow Direction of the aura icons"] = "光環圖示的延伸方向"
    L["Horizontal offset"] = "水平位移"
    L["Vertical offset"] = "垂直位移"
    L["Alpha"] = "Alpha值"
    L["Debuffs"] = "減益"
    L["Dynamic Timer Color"] = "動態計時條顏色"
    L["Show dynamic color on cooldown numbers"] = "冷卻時間數字以動態顏色顯示"
    L["Color of the cooldown timer and stacks"] = "Farbe der Abklingzeit und Stapel"
    L["Spell School Colors"] = "法術種類顏色"
    L["Spell School Colors Enabled"] = "啟用"
    L["Show border colors by spell school"] = "根據不同法術種類顯示不同邊框顏色"
    L["Curse"] = "詛咒"
    L["Color of the border"] = "邊框顏色"
    L["Magic"] = "魔法"
    L["Poison"] = "中毒"
    L["Physical"] = "物理"
    L["Immune"] = "免疫"
    L["Disease"] = "疾病"
    L["Aura"] = "光環"
    L["Form"] = "形態"
    L["Font"] = "字型" --Line 906
    L["Border"] = "邊框" --Line 949
    L["Debuff Lists"] = "減益列表" --Line 1036
    L["Buff Lists"] = "增益列表" --Line 1051

    -- Castbar.lua
    L["Cast Bar"] = "施法條"
    L["Bar"] = "施法條"
    L["Bar Size"] = "施法條大小"
    L["Bar height"] = "高度"
    L["Height of the bar"] = "計量條高度"
    L["Bar width"] = "寬度"
    L["Width of the bars"] = "計量條寬度"
    L["Texture"] = "材質"
    L["Bar texture"] = "施法條材質"
    L["Texture of the bar"] = "計量條材質"
    L["Bar color"] = "施法條顏色"
    L["Color of the cast bar"] = "計量條顏色"
    L["Background color"] = "背景顏色"
    L["Color of the cast bar background"] = "施法條背景顏色"
    L["Border size"] = "邊框大小"
    L["Status Bar border"] = "狀態列邊框"
    L["Status Bar border color"] = "狀態列邊框顏色"
    L["Icon"] = "圖示"
    L["Icon size"] = "圖示大小"
    L["Icon border"] = "圖示邊框"
    L["Icon border color"] = "圖示邊框顏色"
    L["If test is running, type \"/gladdy test\" again"] = "如果測試已經開始，調整此選項後請輸入/gladdy test以重新開始測試"
    L["Spark"] = "尾端發亮"
    L["Spark enabled"] = "啟用"
    L["Spark color"] = "顏色"
    L["Color of the cast bar spark"] = "計時條進度的尾端顏色"
    L["Font of the castbar"] = "施法條字型"
    L["Font size"] = "字體大小"
    L["Size of the text"] = "施法條字體大小"
    L["Format"] = "格式"
    L["Timer Format"] = "時間格式"
    L["Remaining"] = "剩餘時間"
    L["Total"] = "總時間"
    L["Both"] = "兩者"
    L["Castbar position"] = "施法條位置"
    L["Icon position"] = "圖示位置"
    L["Offsets"] = "位移"

    -- Classicon.lua
    L["Class Icon"] = "職業圖示"
    L["Balance"] = "平衡"
    L["Feral"] = "野性"
    L["Restoration"] = "恢復"
    L["Beast Mastery"] = "獸王"
    L["Marksmanship"] = "射擊"
    L["Survival"] = "生存"
    L["Arcane"] = "奧術"
    L["Fire"] = "火焰"
    L["Frost"] = "冰霜"
    L["Holy"] = "神聖"
    L["Protection"] = "防護"
    L["Retribution"] = "懲戒"
    L["Discipline"] = "戒律"
    L["Shadow"] = "暗影"
    L["Assassination"] = "刺殺"
    L["Combat"] = "戰鬥"
    L["Subtlety"] = "敏銳"
    L["Elemental"] = "元素"
    L["Enhancement"] = "增強"
    L["Affliction"] = "痛苦"
    L["Demonology"] = "惡魔"
    L["Destruction"] = "毀滅"
    L["Arms"] = "武器"
    L["Fury"] = "狂怒"
    L["Show Spec Icon"] = "顯示天賦圖示"
    L["Shows Spec Icon once spec is detected"] = "若偵測到天賦則顯示天賦圖示"
    L["Icon width factor"] = "圖示寬度比例"
    L["This changes positions with trinket"] = "調整職業圖示位置"
    L["Border color"] = "邊框顏色"

    --CombatIndicator.lua
    L["Combat Indicator"] = "戰鬥指示器"
    L["Enable Combat Indicator icon"] = "顯示是否進入戰鬥"
    L["Anchor"] = "定位"
    L["This changes the anchor of the ci icon"] = "調整戰鬥指示器圖示定位點"
    L["This changes position relative to its anchor of the ci icon"] = "調整戰鬥指示器位置"

    -- Constants.lua
    L["Physical"] = "物理" --Line 749
    L["Holy"] = "神聖" --Line 750
    L["Fire"] = "火焰" --Line 751
    L["Nature"] = "自然" --Line 752
    L["Frost"] = "冰霜" --Line 753
    L["Shadow"] = "暗影" --Line 754
    L["Arcane"] = "奧術" --Line 755
    L["Unknown"] = "未知" --Line 756

    -- Cooldowns.lua
    L["Cooldowns"] = "技能冷卻監控"
    L["Enabled cooldown module"] = "啟用冷卻時間監控模組"
    L["Cooldown size"] = "大小"
    L["Size of each cd icon"] = "冷卻時間圖示大小"
    L["Icon Width Factor"] = "寬度"
    L["Max Icons per row"] = "每行圖示數量"
    L["Scale of the font"] = "字體大小"
    L["Anchor of the cooldown icons"] = "冷卻圖示定位"
    L["Grow Direction of the cooldown icons"] = "冷卻圖示延伸方向"
    L["Offset"] = "位移"
    L["BloodElf"] = "血精靈"
    L["NightElf"] = "夜精靈"
    L["Scourge"] = "不死族"

    -- Diminishings.lua
    L["Diminishings"] = "控場遞減"
    L["Enabled DR module"] = "啟用遞減模組"
    L["DR Cooldown position"] = "遞減冷卻時間位置"
    L["Position of the cooldown icons"] = "遞減冷卻時間圖示位置"
    L["DR Border Colors"] = "DR邊框顏色"
    L["Dr Border Colors Enabled"] = "啟用"
    L["Colors borders of DRs in respective DR-color below"] = "邊框顏色依遞減設定為以下顏色"
    L["Half"] = "二分之一"
    L["Quarter"] = "四分之一"
    L["Categories"] = "法術列表"
    L["Force Icon"] = "使用自訂圖示"
    L["Icon of the DR"] = "選擇此區圖示取代原始技能圖示"

    -- ExportImport.lua
    L["Export Import"] = "匯出/匯入"
    L["Profile Export Import"] = "設定檔匯出/匯入"
    L["Export"] = "匯出" --Line 138
    L["Export your current profile to share with others or your various accounts."] = "匯出您目前的設定檔" --Line 139
    L["Import"] = "匯入" --Line 162
    L["This will overwrite your current profile!"] = "這將會覆蓋您目前的設定檔" --Line 163

    -- Healthbar.lua
    L["Health Bar"] = "血量條"
    L["DEAD"] = "死亡"
    L["LEAVE"] = "暫離"
    L["General"] = "一般"
    L["Color of the status bar background"] = "狀態列背景顏色"
    L["Font of the bar"] = "字型"
    L["Name font size"] = "名稱字體大小"
    L["Size of the name text"] = "設定名稱字體大小"
    L["Health font size"] = "生命值字體大小"
    L["Size of the health text"] = "設定生命值字體大小"
    L["Size of the border"] = "邊框大小"
    L["Health Bar Text"] = "血量條文字"
    L["Show name text"] = "顯示名稱"
    L["Show the units name"] = "顯示單位名稱"
    L["Show ArenaX"] = "顯示編號"
    L["Show 1-5 as name instead"] = "使用編號1-5代替角色名稱"
    L["Show the actual health"] = "顯示目前生命值"
    L["Show the actual health on the health bar"] = "在血量條上顯示目前生命值"
    L["Show max health"] = "顯示最大生命值"
    L["Show max health on the health bar"] = "在血量條上顯示最大生命值"
    L["Show health percentage"] = "顯示百分比"
    L["Show health percentage on the health bar"] = "在血量條上顯示生命值百分比"

    -- Highlight.lua
    L["Highlight"] = "高亮提示"
    L["Show Inside"] = "顯示在框架內"
    L["Show Highlight border inside of frame"] = "將高亮邊框顯示於框架內側"
    L["Colors"] = "邊框顏色"
    L["Target border color"] = "目標"
    L["Color of the selected targets border"] = "目標的邊框顏色"
    L["Focus border color"] = "專注"
    L["Color of the focus border"] = "專注目標邊框顏色"
    L["Highlight target"] = "高亮目標"
    L["Toggle if the selected target should be highlighted"] = "是否高亮當前目標"
    L["Show border around target"] = "顯示目標邊框"
    L["Toggle if a border should be shown around the selected target"] = "是否顯示當前目標的邊框"
    L["Show border around focus"] = "顯示專注邊框"
    L["Toggle of a border should be shown around the current focus"] = "是否顯示當前專注目標的邊框"

    -- Pets.lua
    L["Pets"] = "寵物"
    L["Enables Pets module"] = "啟用寵物模組"
    L["Width of the bar"] = "寵物列寬度"
    L["Health color"] = "生命值顏色"
    L["Color of the status bar"] = "狀態列顏色"
    L["Portrait"] = "頭像"
    L["Health Values"] = "生命值"

    -- Powerbar.lua
    L["Power Bar"] = "法力/能量條"
    L["Power Bar Text"] = "法力/能量條文字"
    L["Power Texts"] = "法力/能量條文字"
    L["Show race"] = "顯示種族"
    L["Show spec"] = "顯示天賦"
    L["Show the actual power"] = "顯示目前法力/能量"
    L["Show the actual power on the power bar"] = "在計量條中顯示目前法力/能量值"
    L["Show max power"] = "顯示最大法力/能量值"
    L["Show max power on the power bar"] = "在計量條中顯示最大法力/能量值"
    L["Show power percentage"] = "顯示法力/能量百分比"
    L["Show power percentage on the power bar"] = "在計量條中顯示目前法力/能量百分比"

    -- Racial.lua
    L["Racial"] = "種族"
    L["Enable racial icon"] = "啟用種族圖示"
    L["This changes the anchor of the racial icon"] = "調整種族圖示定位點"
    L["This changes position relative to its anchor of the racial icon"] = "調整種族圖示位置"

    -- TotemPlates.lua
    L["Totem Plates"] = "圖騰名條"
    L["Customize Totems"] = "自訂圖騰"
    L["Custom totem name"] = "自訂圖騰名稱"
    L["Totem General"] = "圖騰通用設定"
    L["Turns totem icons instead of nameplates on or off. (Requires reload)"] = "是否顯示圖騰名條(需重新載入)"
    L["Show friendly"] = "顯示友方圖騰"
    L["Show enemy"] = "顯示敵方圖騰"
    L["Totem size"] = "圖騰大小"
    L["Size of totem icons"] = "圖騰圖示大小"
    L["Font of the custom totem name"] = "自訂圖騰字型"
    L["Apply alpha when no target"] = "圖騰非目標時套用alpha值"
    L["Always applies alpha, even when you don't have a target. Else it is 1."] = "若圖騰未被選為目標，其圖示套用alpha值設定"
    L["Apply alpha when targeted"] = "圖騰為目標時套用alpha值"
    L["Always applies alpha, even when you target the totem. Else it is 1."] = "圖騰被選為目標時，其圖示套用alpha值設定"
    L["All totem border alphas (configurable per totem)"] = "圖騰 Alpha值 "
    L["Totem icon border style"] = "圖騰邊框樣式"
    L["All totem border color"] = "圖騰邊框顏色"

    -- Trinket.lua
    L["Trinket"] = "飾品"
    L["Enable trinket icon"] = "啟用飾品圖示"
    L["This changes positions of the trinket"] = "調整飾品圖示位置"

    -- XiconProfiles.lua
    L["Profile"] = "樣式"
    L["XiconProfiles"] = "框架外觀" --Line 4
    L[" No Pet"] = "(無寵物)" --Line 109, 119

    -- Frame.lua
    L["Gladdy - drag to move"] = "Gladdy - 拖曳移動"

    -- Gladdy.lua
    L["Welcome to Gladdy!"] = "歡迎使用 Gladdy!"
    L["First run has been detected, displaying test frame."] = "第一次使用時，顯示此測試框架。"
    L["Valid slash commands are:"] = "可用的指令為:"
    L["If this is not your first run please lock or move the frame to prevent this from happening."] = "若非第一次使用，請移動或鎖定框架以避免此提示再次出現。"

    -- Clicks.lua
    L["Action #%d"] = "動作 #%d"
    L["Target"] = "目標" --Line 15
    L["Focus"] = "專注" --Line 16
    L["Clicks"] = "點擊動作"
    L["Left button"] = "左鍵"
    L["Right button"] = "右鍵"
    L["Middle button"] = "中鍵"
    L["Button 4"] = "滑鼠按鍵4"
    L["Button 5"] = "滑鼠按鍵5"
    L["Select what action this mouse button does"] = "設定輸入按鍵後的欲執行的動作"
    L["Modifier"] = "修飾鍵"
    L["Select which modifier to use"] = "設定欲使用的修飾鍵"
    L["Button"] = "按鍵"
    L["Select which mouse button to use"] = "設定欲使用的滑鼠按鍵"
    L["Name"] = "名稱"
    L["Select the name of the click option"] = "設定動作名稱"
    L["Action"] = "動作"
    L["Cast Spell / Macro"] = "施放法術/巨集"

    --RangeCheck.lua
    L["Range Check"] = "距離檢查"
    L["Spells"] = "法術"
    L["Fade"] = "變暗"
    L["Out of Range Darkening Level"] = "超出距離時變暗程度"
    L["Higher is darker"] = "數值越高越暗"
    L["yards"] = " 碼" --Line 366, 388
    L["Changing the spellID only applies to your player class!\n\nExample: If you are a Paladin and wish to change your range check spell to Repentance, edit the Paladin spellID to 20066."] = "對應您的職業修改欲用於監控距離的技能。\n\n範例:若您為聖騎士且想以懺悔技能用於距離監控，請將聖騎士的法術ID改為20066。" --Line 352

    --ShadowsightTimer.lua
    L["Shadowsight Timer"] = "暗影視界計時"
    L["Locked"] = "鎖定"
    L["Announce"] = "通報"
    L["Scale"] = "大小"
    L["Shadowsight up in %ds"] = "暗影視界於%d秒後就緒"
    L["Shadowsight up!"] = "暗影視界已就緒"

    -- Options.lua
    L["settings"] = "設定"
    L["Reset module"] = "重設模組"
    L["Reset module to defaults"] = "將模組重設為預設值"
    L["No settings"] = "無設定"
    L["Module has no settings"] = "模組沒有設定"
    L["General settings"] = "通用設定"
    L["Lock frame"] = "鎖定框架"
    L["Toggle if frame can be moved"] = "調整框架是否可移動"
    L["Grow frame upwards"] = "框架向上延伸"
    L["If enabled the frame will grow upwards instead of downwards"] = "開啟此選項時框架向上延伸"
    L["Down"] = "下"
    L["Up"] = "上"
    L["Frame General"] = "框架"
    L["Frame scale"] = "框架大小"
    L["Scale of the frame"] = "框架的尺寸"
    L["Frame padding"] = "框架內距"
    L["Padding of the frame"] = "框架的內距"
    L["Frame width"] = "框架寬度"
    L["Margin"] = "框架間距"
    L["Margin between each button"] = "框架的間距"
    L["Cooldown General"] = "冷卻"
    L["Font General"] = "字型"
    L["General Font"] = "Allgemeine Schriftart"
    L["Font color text"] = "文字顏色"
    L["Font color timer"] = "計時器文字顏色"
    L["Color of the timers"] = "計時器顏色"
    L["Icons General"] = "圖示"
    L["Icon border style"] = "圖示邊框樣式"
    L["This changes the border style of all icons"] = "調整所有圖示的邊框樣式"
    L["This changes the border color of all icons"] = "調整所有圖示的邊框顏色"
    L["Statusbar General"] = "狀態列"
    L["Statusbar texture"] = "狀態列材質"
    L["This changes the texture of all statusbar frames"] = "調整所有狀態列的材質"
    L["Statusbar border style"] = "狀態列邊框樣式"
    L["This changes the border style of all statusbar frames"] = "調整所有狀態列的邊框樣式"
    L["Statusbar border offset divider (smaller is higher offset)"] = "狀態列邊框距離"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "調整狀態列邊框距離"
    L["Statusbar border color"] = "狀態列邊框顏色"
    L["This changes the border color of all statusbar frames"] = "調整所有狀態列的邊框顏色"
    L["Hide Blizzard"] = "隱藏暴雪框架"
    L["Grow Direction"] = "框架延伸方向"
    L["Arena only"] = "只在競技場中"
    L["Never"] = "從不"
    L["Always"] = "永遠"
    L["Load configuration"] = "設定選項" --Line 713
    L["Load configuration options"] = "載入設定選項" --Line 714
    L["Background Color of the frame"] = "框架的背景顏色"

    L["Gladdy"] = "Gladdy目標框架"  --Line 210, 709, 727
    L["Gladdy Tooltip round"] = "Gladdy 工具提示 圓形"
    L["Gladdy Tooltip squared"] = "Gladdy 工具提示 方形"
    L["Gloss (black border)"] = "光澤 (黑色邊框)"
    L["Unlock frame"] = "解鎖框架"
    L["Show Mover"] = "顯示移動器"
    L["Hide Mover"] = "隱藏移動器"
    L["Toggle to show Mover Frames"] = "切換顯示移動器框架"
    L["Test"] = "測試"
    L["Show Test frames"] = "顯示測試框架"
    L["Hide"] = "隱藏"
    L["Hide frames"] = "隱藏框架"
    L["ReloadUI"] = "重新載入UI"
    L["Reloads the UI"] = "重新載入使用者介面"
    L["Grow Middle"] = "從中心延伸"
    L["Frames expand along a centric anchor"] = "框架沿中心錨點擴展"
    L["Pixel Perfect Scale"] = "像素完美縮放"
    L["Enables Pixel Perfect Scale - disables manual "] = "啟用像素完美縮放 - 停用手動 "
    L["OmniCC Enabled"] = "啟用 OmniCC"
    L["Frame width"] = "框架寬度"
    L["Background color"] = "背景顏色"
    L["Background Color of the frame"] = "框架的背景顏色"
    L["Cooldown General"] = "冷卻"
    L["Font General"] = "字型"
    L["General Font"] = "通用字型"
    L["Font color text"] = "文字顏色"
    L["Font color timer"] = "計時器文字顏色"
    L["Color of the timers"] = "計時器顏色"
    L["Icons General"] = "圖示"
    L["Icon border style"] = "圖示邊框樣式"
    L["This changes the border style of all icons"] = "調整所有圖示的邊框樣式"
    L["Icon border color"] = "圖示邊框顏色"
    L["This changes the border color of all icons"] = "調整所有圖示的邊框顏色"
    L["Statusbar General"] = "狀態列"
    L["Statusbar texture"] = "狀態列材質"
    L["This changes the texture of all statusbar frames"] = "調整所有狀態列的材質"
    L["Statusbar border style"] = "狀態列邊框樣式"
    L["This changes the border style of all statusbar frames"] = "調整所有狀態列的邊框樣式"
    L["Statusbar border offset divider (smaller is higher offset)"] = "狀態列邊框距離"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "調整狀態列邊框距離"
    L["Statusbar border color"] = "狀態列邊框顏色"
    L["This changes the border color of all statusbar frames"] = "調整所有狀態列的邊框顏色"
    L["Hide Blizzard"] = "隱藏暴雪框架"
    L["Arena only"] = "只在競技場中"
    L["Never"] = "從不"
    L["Always"] = "永遠"
    L["Grow Direction"] = "框架延伸方向"
    L["Down"] = "下"
    L["Up"] = "上"
    L["Load configuration"] = "設定選項"
    L["Load configuration options"] = "載入設定選項"

    -- Nameplates.lua
    L["Show important auras on nameplates"] = "在名條上顯示重要光環"
    L["Zoomed Icon"] = "縮放圖示"
    L["Zoomes the icon to remove borders"] = "縮放圖示以移除邊框"
    L["Icon size"] = "圖示大小"
    L["Size of each aura icon"] = "每個光環圖示大小"
    L["Icon Width Factor"] = "圖示寬度比例"
    L["Stretches the icon"] = "圖示寬度"
    L["Max Icons"] = "最大圖示數"
    L["Maximum number of aura icons to show"] = "顯示的最大光環圖示數量"
    L["Icon Padding"] = "圖示內距"
    L["Space between icons"] = "圖示間距"
    L["Horizontal offset"] = "水平位移"
    L["Vertical offset"] = "垂直位移"
    L["Center Icons"] = "置中圖示"
    L["Sort Order"] = "排序順序"
    L["How to sort aura icons"] = "如何排序光環圖示"
    L["Time"] = "時間"
    L["Time Left"] = "剩餘時間"
    L["Frame Strata and Level"] = "框架層級和等級"
    L["Frame Strata"] = "框架層級"
    L["Frame Level"] = "框架等級"
    L["Aura List"] = "光環列表"
    L["Configure which auras to show on nameplates"] = "設定要在名條上顯示哪些光環"
    L["Note: Uses the same aura list as the Auras module. Configure auras in the Auras module settings."] = "注意：使用與光環模組相同的光環列表。在光環模組設定中設定光環。"

    -- Diminishings.lua
    L["Show when Aura applied"] = "應用光環時顯示"
    L["Group"] = "群組"
    L["DR Icon"] = "遞減圖示"
    L["Group direction"] = "群組方向"
    L["Cooldown Font"] = "冷卻字型"
    L["Enable DR Colors as Font Color"] = "啟用遞減顏色作為字型顏色"
    L["Shows the current DR Level on the DR icon."] = "在遞減圖示上顯示當前遞減等級。"
    L["Cooldown number alpha"] = "冷卻數字透明度"
    L["Font Outline"] = "字型輪廓"
    L["X Offset"] = "X 位移"
    L["Y Offset"] = "Y 位移"
    L["DR Font"] = "遞減字型"
    L["Enable DR Font"] = "啟用遞減字型"
    L["DR Font color"] = "遞減字型顏色"
    L["Color of the font"] = "字型顏色"
    L["Font of the DR Font"] = "遞減字型的字型"
    L["DR Colors"] = "遞減顏色"
    L["DR Texts"] = "遞減文字"
    L["Text on half DR"] = "半遞減文字"
    L["Text on quarter DR"] = "四分之一遞減文字"
    L["Text on immune DR"] = "免疫遞減文字"
    L["Enable DR Colors as Border Color"] = "啟用遞減顏色作為邊框顏色"
    L["Colors borders of DRs in respective DR Colors"] = "以對應的遞減顏色為遞減邊框著色"
    L["Border color"] = "邊框顏色"
    L["DR Grow Direction"] = "遞減延伸方向"
    L["Grow Direction of the dr icons"] = "遞減圖示的延伸方向"
    L["Change the DR Duration in seconds (DR is dynamic between 15-20s)"] = "以秒為單位變更遞減持續時間（遞減在15-20秒之間動態變化）"
    L["DR Duration"] = "遞減持續時間"
    L["Space between Icons"] = "圖示間距"
    L["Scale of the text"] = "設定字體大小"
    L["Scale of the font"] = "字型大小"

    -- Clicks.lua
    L["Action #%d"] = "動作 #%d"
    L["Left Click Enemy 1"] = "左鍵點擊敵人 1"
    L["Left Click Enemy 2"] = "左鍵點擊敵人 2"
    L["Left Click Enemy 3"] = "左鍵點擊敵人 3"
    L["Left Click Enemy 4"] = "左鍵點擊敵人 4"
    L["Left Click Enemy 5"] = "左鍵點擊敵人 5"
    L["Right Click Enemy 1"] = "右鍵點擊敵人 1"
    L["Right Click Enemy 2"] = "右鍵點擊敵人 2"
    L["Right Click Enemy 3"] = "右鍵點擊敵人 3"
    L["Right Click Enemy 4"] = "右鍵點擊敵人 4"
    L["Right Click Enemy 5"] = "右鍵點擊敵人 5"
    L["Middle Click Enemy 1"] = "中鍵點擊敵人 1"
    L["Middle Click Enemy 2"] = "中鍵點擊敵人 2"
    L["Middle Click Enemy 3"] = "中鍵點擊敵人 3"
    L["Middle Click Enemy 4"] = "中鍵點擊敵人 4"
    L["Middle Click Enemy 5"] = "中鍵點擊敵人 5"
    L["Button4 Click Enemy 1"] = "按鈕4點擊敵人 1"
    L["Button4 Click Enemy 2"] = "按鈕4點擊敵人 2"
    L["Button4 Click Enemy 3"] = "按鈕4點擊敵人 3"
    L["Button4 Click Enemy 4"] = "按鈕4點擊敵人 4"
    L["Button4 Click Enemy 5"] = "按鈕4點擊敵人 5"
    L["Button5 Click Enemy 1"] = "按鈕5點擊敵人 1"
    L["Button5 Click Enemy 2"] = "按鈕5點擊敵人 2"
    L["Button5 Click Enemy 3"] = "按鈕5點擊敵人 3"
    L["Button5 Click Enemy 4"] = "按鈕5點擊敵人 4"
    L["Button5 Click Enemy 5"] = "按鈕5點擊敵人 5"
    L["Left button"] = "左鍵"
    L["Right button"] = "右鍵"
    L["Middle button"] = "中鍵"
    L["Button 4"] = "滑鼠按鍵4"
    L["Button 5"] = "滑鼠按鍵5"
    L["None"] = "無"
    L["All"] = "全部"
    L["CTRL"] = "CTRL"
    L["SHIFT"] = "SHIFT"
    L["ALT"] = "ALT"
    L["Cast Spell"] = "施放法術"
    L["Name"] = "名稱"
    L["Select the name of the click option"] = "設定動作名稱"
    L["Button"] = "按鍵"
    L["Select which mouse button to use"] = "設定欲使用的滑鼠按鍵"
    L["Modifier"] = "修飾鍵"
    L["Select which modifier to use"] = "設定欲使用的修飾鍵"
    L["Action"] = "動作"
    L["Select what action this mouse button does"] = "設定輸入按鍵後的欲執行的動作"
    L["Cast Spell / Macro"] = "施放法術/巨集"
    L["Target"] = "目標"
    L["Focus"] = "專注"
    
    -- Additional missing keys for zhTW
    L["No Pet"] = "無寵物"
    L["0%"] = "0%"
    L["100%"] = "100%"
    L["50%"] = "50%"
    L["Above"] = "上方"
    L["Add Aura"] = "新增光環"
    L["Add Buff"] = "新增增益"
    L["Add Debuff"] = "新增減益"
    L["All enemy totem border color"] = "所有敵方圖騰邊框顏色"
    L["All friendly totem border color"] = "所有友方圖騰邊框顏色"
    L["Alpha of the icon when cooldown active"] = "冷卻中圖示的透明度"
    L["Announce Spell School"] = "通報法術分類"
    L["Announces Spell School like \"Holy\" instead of Spell Name"] = "通報法術分類如「神聖」而非法術名稱"
    L["Announces when enemies' spells are interrupted"] = "當敵方法術被打斷時通報"
    L["Attach To TotemPlate"] = "附加至圖騰名條"
    L["Aura Detached"] = "光環分離"
    L["Aura X Offset"] = "光環 X 位移"
    L["Aura Y Offset"] = "光環 Y 位移"
    L["Aura size"] = "光環大小"
    L["Aura width factor"] = "光環寬度比例"
    L["Background"] = "背景"
    L["Bar Color"] = "計量條顏色"
    L["Bar horizontal"] = "水平計量條"
    L["Bar vertical"] = "垂直計量條"
    L["BeastMastery"] = "獸王"
    L["Below"] = "下方"
    L["Blood"] = "鮮血"
    L["Brewmaster"] = "釀酒大師"
    L["Buff Lists"] = "增益列表"
    L["By Arena"] = "依競技場"
    L["By Order"] = "依順序"
    L["COOLDOWN USED: %s (%s) used %s - %s sec. cooldown"] = "冷卻已使用: %s (%s) 使用了 %s - %s 秒冷卻"
    L["Class Colored Bars"] = "職業顏色計量條"
    L["Class Icon Enabled"] = "職業圖示已啟用"
    L["Class colored"] = "職業顏色"
    L["Colored trinket"] = "彩色飾品"
    L["Colored trinket CD"] = "彩色飾品冷卻"
    L["Colored trinket No CD"] = "彩色飾品無冷卻"
    L["Custom Colors"] = "自訂顏色"
    L["Custom Tags"] = "自訂標籤"
    L["Custom Tags Enabled"] = "自訂標籤已啟用"
    L["Default Aura"] = "預設光環"
    L["Delete"] = "刪除"
    L["Desaturate Icon"] = "圖示去飽和"
    L["Detached Aura"] = "分離光環"
    L["Detached Interrupt"] = "分離斷法"
    L["Detached Mode"] = "分離模式"
    L["Dialog"] = "對話框"
    L["Dispel Icon"] = "驅散圖示"
    L["Dispel Icon Enabled"] = "驅散圖示已啟用"
    L["Dynamic Icon"] = "動態圖示"
    L["Enable"] = "啟用"
    L["Enable Aura to be displayed"] = "啟用光環顯示"
    L["Enable Custom Colors"] = "啟用自訂顏色"
    L["Enable Interrupt Spell School Colors"] = "啟用打斷法術分類顏色"
    L["Enabled Racials"] = "種族技能已啟用"
    L["Enabled showing racial"] = "已啟用顯示種族"
    L["Enables Targets module"] = "啟用目標模組"
    L["Enemy"] = "敵方"
    L["Enrage"] = "狂怒"
    L["Export"] = "匯出"
    L["Export your current profile to share with others or your various accounts."] = "匯出您目前的設定檔以與他人或您的其他帳號分享。"
    L["Fade"] = "淡出"
    L["Font Enabled"] = "字型已啟用"
    L["Font of the custom totem name"] = "自訂圖騰名稱字型"
    L["Font size left text"] = "左側文字字體大小"
    L["Font size right text"] = "右側文字字體大小"
    L["Form"] = "形態"
    L["Friendly"] = "友方"
    L["Fullscreen"] = "全螢幕"
    L["Fullscreen Dialog"] = "全螢幕對話框"
    L["Group Class Icons"] = "群組職業圖示"
    L["Group Mode"] = "群組模式"
    L["Group Pets"] = "群組寵物"
    L["Group Targets"] = "群組目標"
    L["Guardian"] = "守護者"
    L["Health Bar Text"] = "血量條文字"
    L["Health Values"] = "生命值"
    L["Height"] = "高度"
    L["Hide Disabled Totem Plates"] = "隱藏已停用的圖騰名條"
    L["High"] = "高"
    L["Higher is darker"] = "數值越高越暗"
    L["Highlight mouseover"] = "滑鼠指向高亮"
    L["Horizontal Grow Direction"] = "水平延伸方向"
    L["Horizontal Grow Direction of the cooldown icons"] = "冷卻圖示的水平延伸方向"
    L["Icon Enabled"] = "圖示已啟用"
    L["Icon border"] = "圖示邊框"
    L["If test is running, type \"/gladdy test\" again"] = "如果測試正在執行，請再次輸入 \"/gladdy test\""
    L["Import"] = "匯入"
    L["Interrupt Detached"] = "分離斷法"
    L["Interrupt X Offset"] = "斷法 X 位移"
    L["Interrupt Y Offset"] = "斷法 Y 位移"
    L["Interrupt size"] = "斷法大小"
    L["Interrupt width factor"] = "斷法寬度比例"
    L["Interrupts"] = "斷法"
    L["INTERRUPTED: %s (%s)"] = "已打斷: %s (%s)"
    L["Layer Group"] = "圖層群組"
    L["Layer Group and Level"] = "圖層群組和等級"
    L["Layer Level"] = "圖層等級"
    L["Left Font Outline"] = "左側字型輪廓"
    L["Left Text"] = "左側文字"
    L["Left Text Horizontal Offset"] = "左側文字水平位移"
    L["Left Text Vertical Offset"] = "左側文字垂直位移"
    L["Left font size"] = "左側字體大小"
    L["Locked"] = "已鎖定"
    L["Low"] = "低"
    L["Max"] = "最大"
    L["Medium"] = "中"
    L["Mistweaver"] = "織霧者"
    L["Nature"] = "自然"
    L["Cooldowncircle"] = "冷卻圓圈"
    L["Only numbers, comma or space is allowed. \n Example: 124, 51243, 4121, 41111"] = "僅允許數字、逗號或空格。\n 範例: 124, 51243, 4121, 41111"
    L["Out of Range Darkening Level"] = "超出距離時變暗程度"
    L["Power Bar Text"] = "法力/能量條文字"
    L["Power Texts"] = "法力/能量條文字"
    L["Power font size"] = "法力字體大小"
    L["Race font size"] = "種族字體大小"
    L["Range Check"] = "距離檢查"
    L["Remaining"] = "剩餘"
    L["Reset Glow"] = "重設發光"
    L["Reset Glow Color"] = "重設發光顏色"
    L["Reset Time"] = "重設時間"
    L["Reset time in seconds"] = "重設時間（秒）"
    L["Reverse"] = "反向"
    L["Right Font Outline"] = "右側字型輪廓"
    L["Right Text"] = "右側文字"
    L["Right Text Horizontal Offset"] = "右側文字水平位移"
    L["Right Text Vertical Offset"] = "右側文字垂直位移"
    L["Right font size"] = "右側字體大小"
    L["Scale"] = "大小"
    L["Scourge"] = "不死族"
    L["Show Arena1-5 as name instead"] = "顯示 Arena1-5 作為名稱"
    L["Show Spell Name"] = "顯示法術名稱"
    L["Show Time"] = "顯示時間"
    L["Show border colors by spell school"] = "根據法術分類顯示邊框顏色"
    L["Show dynamic color on cooldown numbers"] = "冷卻時間數字以動態顏色顯示"
    L["Show name text"] = "顯示名稱文字"
    L["Show race"] = "顯示種族"
    L["Show spec"] = "顯示天賦"
    L["Show the units name"] = "顯示單位名稱"
    L["Show two timers"] = "顯示兩個計時器"
    L["Shows a solid colored icon when off/off CD."] = "當關閉/無冷卻時顯示實色圖示。"
    L["Size"] = "大小"
    L["Size of each cd icon"] = "每個冷卻圖示大小"
    L["Size of the health text"] = "生命值文字大小"
    L["Size of the left text"] = "左側文字大小"
    L["Size of the power text"] = "法力文字大小"
    L["Size of the race text"] = "種族文字大小"
    L["Size of the right text"] = "右側文字大小"
    L["Spark"] = "尾端發亮"
    L["Spark enabled"] = "尾端發亮已啟用"
    L["Spell ID used for texture"] = "用於材質的法術 ID"
    L["Spell IDs"] = "法術 ID"
    L["Spell School Colors"] = "法術分類顏色"
    L["Spell School Colors Enabled"] = "法術分類顏色已啟用"
    L["Spells"] = "法術"
    L["Start Time"] = "開始時間"
    L["Start time in seconds"] = "開始時間（秒）"
    L["Status Bar border"] = "狀態列邊框"
    L["Status Bar border color"] = "狀態列邊框顏色"
    L["Stealth Color"] = "潛行顏色"
    L["Style"] = "樣式"
    L["Test With Talents"] = "使用天賦測試"
    L["Test Without Talents"] = "不使用天賦測試"
    L["Text"] = "文字"
    L["Thick Outline"] = "粗輪廓"
    L["Thin Outline"] = "細輪廓"
    L["This changes positions of the trinket"] = "調整飾品圖示位置"
    L["This changes positions with trinket"] = "調整與飾品的位置"
    L["This will overwrite your current profile!"] = "這將會覆蓋您目前的設定檔！"
    L["Tooltip"] = "工具提示"
    L["Totem Pulse"] = "圖騰脈衝"
    L["Totem Pulse Enabled"] = "圖騰脈衝已啟用"
    L["Turn on/off"] = "開啟/關閉"
    L["Turns countdown before the start of an arena match on/off."] = "在競技場開始前倒數剩餘秒數"
    L["Turns totem icons instead of nameplates on or off."] = "開啟/關閉圖騰圖示而非名條。"
    L["Unholy"] = "穢邪"
    L["Unknown"] = "未知"
    L["Vertical Bar"] = "垂直計量條"
    L["Vertical Grow Direction"] = "垂直延伸方向"
    L["Vertical Grow Direction of the cooldown icons"] = "冷卻圖示的垂直延伸方向"
    L["Width"] = "寬度"
    L["Will use Debuff Color if disabled"] = "若未啟用則使用減益顏色"
    L["Windwalker"] = "御風者"
    L["XiconProfiles"] = "框架外觀"
    L["yards"] = " 碼"
    L["changes "] = "變更 "
elseif GetLocale() == "zhCN" then
    -- Announcements.lua
    L["Announcements"] = "通知"
    L["RESURRECTING: %s (%s)"] = "复活: %s (%s) "
    L["SPEC DETECTED: %s - %s (%s)"] = "敌方天赋: %s - %s (%s)"
    L["LOW HEALTH: %s (%s)"] = "低生命值: %s (%s)"
    L["TRINKET USED: %s (%s)"] = "饰品已使用: %s (%s)"
    L["TRINKET READY: %s (%s)"] = "饰品就绪: %s (%s)"
    L["DRINKING: %s (%s)"] = "正在喝水: %s (%s)"
    L["Self"] = "玩家"
    L["Party"] = "队伍"
    L["Raid Warning"] = "团队警告"
    L["Blizzard's Floating Combat Text"] = "Blizzard 战斗浮动文字"
    L["Trinket used"] = "饰品已使用"
    L["Announce when an enemy's trinket is used"] = "当敌方使用饰品时发出通知"
    L["Trinket ready"] = "饰品就緒"
    L["Announce when an enemy's trinket is ready again"] = "当敌方饰品就绪时发出通报"
    L["Drinking"] = "正在喝水"
    L["Announces when enemies sit down to drink"] = "当敌方喝水时发出通报"
    L["Resurrection"] = "复活"
    L["Announces when an enemy tries to resurrect a teammate"] = "当敌方尝试复活队友时发出通报"
    L["New enemies"] = "新的敌人"
    L["Announces when new enemies are discovered"] = "当发现新的敌人时发出通报"
    L["Spec Detection"] = "天赋侦测"
    L["Announces when the spec of an enemy was detected"] = "当侦测到敌方天赋时发出通报"
    L["Low health"] = "低生命值"
    L["Announces when an enemy drops below a certain health threshold"] = "当敌方生命值低于一定条件时发出通报"
    L["Low health threshold"] = "低生命值门槛"
    L["Choose how low an enemy must be before low health is announced"] = "设定低生命值通报门槛"
    L["Destination"] = "发送通报至"
    L["Choose how your announcements are displayed"] = "选择通报发送至哪个频道"

    -- ArenaCountDown.lua
    L["Arena Countdown"] = "竞技场计时"
    L["Turns countdown before the start of an arena match on/off."] = "在竞技场开始前倒数剩余秒数"
    L["Size"] = "大小"

    -- Auras.lua
    L["Auras"] = "光环"
    L["Frame"] = "框架"
    L["Cooldown"] = "冷却时间"
    L["No Cooldown Circle"] = "取消图示冷却倒数阴影"
    L["Cooldown circle alpha"] = "冷却倒数阴影alpha值"
    L["Font"] = "字体"
    L["Font of the cooldown"] = "設定冷卻時間字型"
    L["Font scale"] = "字体大小"
    L["Scale of the text"] = "设定字体大小"
    L["Font color"] = "字体颜色"
    L["Color of the text"] = "设定字体颜色"
    L["Border"] = "边框"
    L["Border style"] = "边框样式"
    L["Buff color"] = "增益顏色"
    L["Debuff color"] = "减益顏色"
    L["Check All"] = "全选"
    L["Uncheck All"] = "取消全选"
    L["Enabled"] = "启用"
    L["Priority"] = "优先"
    L["Interrupt Spells School Colors"] = "打断法术分类颜色"
    L["Enable Interrupt Spell School Colors"] = "启用"
    L["Will use Debuff Color if disabled"] = "若未启用则使用一般减益颜色"
    L["Buffs"] = "增益" --Line 573
    L["Debuffs"] = "減益" --Line 566
    L["Interrupts"] = "打断" --Line 580

    -- BuffsDebuffs.lua
    L["Buffs and Debuffs"] = "增益与减益"
    L["Enabled Buffs and Debuffs module"] = "启用增益与减益模块"
    L["Show CC"] = "显示控场"
    L["Shows all debuffs, which are displayed on the ClassIcon as well"] = "显示所有减益效果，这些减益效果也显示在职业图标上"
    L["Buffs"] = "增益"
    L["Size & Padding"] = "大小与內距"
    L["Icon Size"] = "图标大小"
    L["Size of the DR Icons"] = "递减图标大小"
    L["Icon Width Factor"] = "图标宽度比例"
    L["Stretches the icon"] = "图标宽度"
    L["Icon Padding"] = "图标內距"
    L["Space between Icons"] = "图标间距"
    L["Position"] = "位置"
    L["Aura Position"] = "光环位置"
    L["Position of the aura icons"] = "光环图标位置"
    L["Top"] = "顶部"
    L["Bottom"] = "底部"
    L["Left"] = "左"
    L["Right"] = "右"
    --L["Grow Direction"] = ""
    L["Grow Direction of the aura icons"] = "光环图标的延伸方向"
    L["Horizontal offset"] = "水平偏移"
    L["Vertical offset"] = "垂直偏移"
    L["Alpha"] = "Alpha值"
    L["Debuffs"] = "减益"
    L["Dynamic Timer Color"] = "动态计时条颜色"
    L["Show dynamic color on cooldown numbers"] = "冷却时间数字以动态颜色显示"
    L["Color of the cooldown timer and stacks"] = "Farbe der Abklingzeit und Stapel"
    L["Spell School Colors"] = "法术种类颜色"
    L["Spell School Colors Enabled"] = "启用"
    L["Show border colors by spell school"] = "根据不同法术显示不同边框颜色"
    L["Curse"] = "诅咒"
    L["Color of the border"] = "边框颜色"
    L["Magic"] = "魔法"
    L["Poison"] = "中毒"
    L["Physical"] = "物理"
    L["Immune"] = "免疫"
    L["Disease"] = "疾病"
    L["Aura"] = "光环"
    L["Form"] = "形态"
    L["Font"] = "字体" --Line 906
    L["Border"] = "边框" --Line 949
    L["Debuff Lists"] = "减益列表" --Line 1036
    L["Buff Lists"] = "增益列表" --Line 1051

    -- Castbar.lua
    L["Cast Bar"] = "施法条"
    L["Bar"] = "施法条"
    L["Bar Size"] = "施法条大小"
    L["Bar height"] = "高度"
    L["Height of the bar"] = "计量条高度"
    L["Bar width"] = "宽度"
    L["Width of the bars"] = "计量条宽度"
    L["Texture"] = "材质"
    L["Bar texture"] = "施法条材质"
    L["Texture of the bar"] = "计量条材质"
    L["Bar color"] = "施法条颜色"
    L["Color of the cast bar"] = "计量条颜色"
    L["Background color"] = "背景颜色"
    L["Color of the cast bar background"] = "施法条背景顏色"
    L["Border size"] = "边框大小"
    L["Status Bar border"] = "状态条边框"
    L["Status Bar border color"] = "状态条边框颜色"
    L["Icon"] = "图标"
    L["Icon size"] = "图标大小"
    L["Icon border"] = "图标边框"
    L["Icon border color"] = "图标边框颜色"
    L["If test is running, type \"/gladdy test\" again"] = "如果测试已经开始，调整此选项后请输入/gladdy test以重新开始测试"
    L["Spark"] = "尾部发亮"
    L["Spark enabled"] = "启用"
    L["Spark color"] = "颜色"
    L["Color of the cast bar spark"] = "计时条进度的尾部颜色"
    L["Font of the castbar"] = "施法条字体"
    L["Font size"] = "字体大小"
    L["Size of the text"] = "施法条字体大小"
    L["Format"] = "格式"
    L["Timer Format"] = "时间格式"
    L["Remaining"] = "剩余时间"
    L["Total"] = "总时间"
    L["Both"] = "两者"
    L["Castbar position"] = "施法条位置"
    L["Icon position"] = "图标位置"
    L["Offsets"] = "偏移"

    -- Classicon.lua
    L["Class Icon"] = "职业图标"
    L["Balance"] = "平衡"
    L["Feral"] = "野性"
    L["Restoration"] = "恢复"
    L["Beast Mastery"] = "兽王"
    L["Marksmanship"] = "射击"
    L["Survival"] = "生存"
    L["Arcane"] = "奥术"
    L["Fire"] = "火焰"
    L["Frost"] = "冰霜"
    L["Holy"] = "神圣"
    L["Protection"] = "防护"
    L["Retribution"] = "惩戒"
    L["Discipline"] = "戒律"
    L["Shadow"] = "暗影"
    L["Assassination"] = "刺杀"
    L["Combat"] = "战斗"
    L["Subtlety"] = "敏锐"
    L["Elemental"] = "元素"
    L["Enhancement"] = "增强"
    L["Affliction"] = "痛苦"
    L["Demonology"] = "恶魔"
    L["Destruction"] = "毁灭"
    L["Arms"] = "武器"
    L["Fury"] = "狂怒"
    L["Show Spec Icon"] = "显示天赋图标"
    L["Shows Spec Icon once spec is detected"] = "若侦测到天赋则显示天赋图标"
    L["Icon width factor"] = "图标宽度比例"
    L["This changes positions with trinket"] = "调整职业图标位置"
    L["Border color"] = "边框颜色"

    --CombatIndicator.lua
    L["Combat Indicator"] = "战斗指示器"
    L["Enable Combat Indicator icon"] = "显示是否进入战斗"
    L["Anchor"] = "锚点"
    L["This changes the anchor of the ci icon"] = "调整战斗指示器显示锚点"
    L["This changes position relative to its anchor of the ci icon"] = "调整战斗指示器位置"

    -- Constants.lua
    L["Physical"] = "物理" --Line 749
    L["Holy"] = "神圣" --Line 750
    L["Fire"] = "火焰" --Line 751
    L["Nature"] = "自然" --Line 752
    L["Frost"] = "冰霜" --Line 753
    L["Shadow"] = "暗影" --Line 754
    L["Arcane"] = "奥术" --Line 755
    L["Unknown"] = "未知" --Line 756

    -- Cooldowns.lua
    L["Cooldowns"] = "技能冷却监控"
    L["Enabled cooldown module"] = "启用冷却时间监控模块"
    L["Cooldown size"] = "大小"
    L["Size of each cd icon"] = "冷却时间图标"
    L["Icon Width Factor"] = "宽度"
    L["Max Icons per row"] = "每行图标数量"
    L["Scale of the font"] = "字体大小"
    L["Anchor of the cooldown icons"] = "冷却图标锚点"
    L["Grow Direction of the cooldown icons"] = "冷却图标延伸方向"
    L["Offset"] = "偏移"
    L["BloodElf"] = "血精灵"
    L["NightElf"] = "暗夜精灵"
    L["Scourge"] = "亡灵"

    -- Diminishings.lua
    L["Diminishings"] = "控场递减"
    L["Enabled DR module"] = "启用递减模块"
    L["DR Cooldown position"] = "递减冷却时间位置"
    L["Position of the cooldown icons"] = "递减冷却时间图标位置"
    L["DR Border Colors"] = "DR边框颜色"
    L["Dr Border Colors Enabled"] = "启用"
    L["Colors borders of DRs in respective DR-color below"] = "边框颜色依递减设定为以下颜色"
    L["Half"] = "二分之一"
    L["Quarter"] = "四分之一"
    L["Categories"] = "法术列表"
    L["Force Icon"] = "使用自定义图标"
    L["Icon of the DR"] = "选择此图标取代原始技能图标"

    -- ExportImport.lua
    L["Export Import"] = "导出/导入"
    L["Profile Export Import"] = "设置导出/导入"
    L["Export"] = "导出" --Line 138
    L["Export your current profile to share with others or your various accounts."] = "导出您目前的设置" --Line 139
    L["Import"] = "导入" --Line 162
    L["This will overwrite your current profile!"] = "这将会覆盖您目前的设置" --Line 163

    -- Healthbar.lua
    L["Health Bar"] = "血量条"
    L["DEAD"] = "死亡"
    L["LEAVE"] = "暂离"
    L["General"] = "一般"
    L["Color of the status bar background"] = "状态条背景颜色"
    L["Font of the bar"] = "字体"
    L["Name font size"] = "名称字体大小"
    L["Size of the name text"] = "设定名称字体大小"
    L["Health font size"] = "生命值字体大小"
    L["Size of the health text"] = "设定生命值字体大小"
    L["Size of the border"] = "边框大小"
    L["Health Bar Text"] = "血量条文字"
    L["Show name text"] = "显示名字"
    L["Show the units name"] = "显示单位名称"
    L["Show ArenaX"] = "显示编号"
    L["Show 1-5 as name instead"] = "使用编号1-5代替角色名字"
    L["Show the actual health"] = "显示目前生命值"
    L["Show the actual health on the health bar"] = "在血量条上显示目前生命值"
    L["Show max health"] = "显示最大生命值"
    L["Show max health on the health bar"] = "在血量条上显示最大生命值"
    L["Show health percentage"] = "显示百分比"
    L["Show health percentage on the health bar"] = "在血量条上显示生命值百分比"

    -- Highlight.lua
    L["Highlight"] = "高亮提示"
    L["Show Inside"] = "显示在框架內"
    L["Show Highlight border inside of frame"] = "將高亮边框显示于框架內侧"
    L["Colors"] = "边框颜色"
    L["Target border color"] = "目标"
    L["Color of the selected targets border"] = "目标的边框顏色"
    L["Focus border color"] = "焦点"
    L["Color of the focus border"] = "焦点目标边框顏色"
    L["Highlight target"] = "高亮目标"
    L["Toggle if the selected target should be highlighted"] = "是否高亮当前目标"
    L["Show border around target"] = "显示目标边框"
    L["Toggle if a border should be shown around the selected target"] = "是否显示当前目标的边框"
    L["Show border around focus"] = "显示焦点边框"
    L["Toggle of a border should be shown around the current focus"] = "是否显示当前焦点目标的边框"

    -- Pets.lua
    L["Pets"] = "宠物"
    L["Enables Pets module"] = "启用宠物模块"
    L["Width of the bar"] = "宠物条宽度"
    L["Health color"] = "生命值顏色"
    L["Color of the status bar"] = "状态条顏色"
    L["Portrait"] = "头像"
    L["Health Values"] = "生命值"

    -- Targets.lua
    L["Targets"] = "敌方目标"
    L["Enables Targets module"] = "启用敌方目标显示模块"

    -- Powerbar.lua
    L["Power Bar"] = "法力/能量条"
    L["Power Bar Text"] = "法力/能量条文字"
    L["Power Texts"] = "法力/能量条文字"
    L["Show race"] = "显示种族"
    L["Show spec"] = "显示天赋"
    L["Show the actual power"] = "显示目前法力/能量"
    L["Show the actual power on the power bar"] = "在计量条中显示目前法力/能量值"
    L["Show max power"] = "显示最大法力/能量值"
    L["Show max power on the power bar"] = "在计量条中显示最大法力/能量值"
    L["Show power percentage"] = "显示法力/能量百分比"
    L["Show power percentage on the power bar"] = "在计量条中显示目前法力/能量百分比"

    -- Racial.lua
    L["Racial"] = "种族"
    L["Enable racial icon"] = "启用种族图标"
    L["This changes the anchor of the racial icon"] = "调整种族图标锚点"
    L["This changes position relative to its anchor of the racial icon"] = "调整种族图标位置"

    -- TotemPlates.lua
    L["Totem Plates"] = "图腾栏"
    L["Customize Totems"] = "自定义图腾"
    L["Custom totem name"] = "自定义图腾名字"
    L["Totem General"] = "图腾通用设置"
    L["Turns totem icons instead of nameplates on or off. (Requires reload)"] = "是否显示图腾名字(需重新加载)"
    L["Show friendly"] = "显示右方图腾"
    L["Show enemy"] = "显示敌方图腾"
    L["Totem size"] = "图腾大小"
    L["Size of totem icons"] = "图腾图标大小"
    L["Font of the custom totem name"] = "自定义图腾字体"
    L["Apply alpha when no target"] = "图腾非目标时使用alpha值"
    L["Always applies alpha, even when you don't have a target. Else it is 1."] = "若图腾未被选为目标，其图标使用alpha值設定"
    L["Apply alpha when targeted"] = "图腾为目标时使用alpha值"
    L["Always applies alpha, even when you target the totem. Else it is 1."] = "图腾被选为目标时，其图标使用alpha值設定"
    L["All totem border alphas (configurable per totem)"] = "图腾 Alpha值 "
    L["Totem icon border style"] = "图腾边框样式"
    L["All totem border color"] = "图腾边框顏色"

    -- Trinket.lua
    L["Trinket"] = "饰品"
    L["Enable trinket icon"] = "启用饰品图标"
    L["This changes positions of the trinket"] = "调整饰品图标位置"

    -- XiconProfiles.lua
    L["Profile"] = "样式"
    L["XiconProfiles"] = "框架外观" --Line 4
    L[" No Pet"] = "(无宠物)" --Line 109, 119

    -- Frame.lua
    L["Gladdy - drag to move"] = "Gladdy - 拖拽移动"

    -- Gladdy.lua
    L["Welcome to Gladdy!"] = "欢迎使用 Gladdy!"
    L["First run has been detected, displaying test frame."] = "第一次使用时，显示此测试框架。"
    L["Valid slash commands are:"] = "可用的指令为:"
    L["If this is not your first run please lock or move the frame to prevent this from happening."] = "若非第一次使用，请移动或锁定框架以免此提示再次出现。"

    -- Clicks.lua
    L["Action #%d"] = "动作 #%d"
    L["Target"] = "目标" --Line 15
    L["Focus"] = "焦点" --Line 16
    L["Clicks"] = "点击动作"
    L["Left button"] = "左键"
    L["Right button"] = "右键"
    L["Middle button"] = "中键"
    L["Button 4"] = "鼠标按键4"
    L["Button 5"] = "鼠标按键5"
    L["Select what action this mouse button does"] = "设置输入按键后欲执行的动作"
    L["Modifier"] = "修饰键"
    L["Select which modifier to use"] = "设置欲使用的修饰键"
    L["Button"] = "按键"
    L["Select which mouse button to use"] = "设置欲使用的鼠标按键"
    L["Name"] = "名称"
    L["Select the name of the click option"] = "设置动作名称"
    L["Action"] = "动作"
    L["Cast Spell / Macro"] = "施放法术/宏"

    --RangeCheck.lua
    L["Range Check"] = "距离检测"
    L["Spells"] = "法术"
    L["Fade"] = "变暗"
    L["Out of Range Darkening Level"] = "超出距离时变暗程度"
    L["Higher is darker"] = "数值越高越暗"
    L["yards"] = " 码" --Line 366, 388
    L["Changing the spellID only applies to your player class!\n\nExample: If you are a Paladin and wish to change your range check spell to Repentance, edit the Paladin spellID to 20066."] = "对应您的职业修改欲使用监控距离的技能。\n\n例:若您为圣骑士且想以忏悔技能用于距离监控，请将圣骑士的法术ID改为20066。" --Line 352

    --ShadowsightTimer.lua
    L["Shadowsight Timer"] = "暗影视界计时"
    L["Locked"] = "锁定"
    L["Announce"] = "通报"
    L["Scale"] = "大小"
    L["Shadowsight up in %ds"] = "暗影视界於%d秒后就绪"
    L["Shadowsight up!"] = "暗影视界已就绪"

    -- Options.lua
    L["settings"] = "设置"
    L["Reset module"] = "重置模块"
    L["Reset module to defaults"] = "将模块重置为初始值"
    L["No settings"] = "无设置"
    L["Module has no settings"] = "模块没有设置"
    L["General settings"] = "通用设置"
    L["Lock frame"] = "锁定框架"
    L["Toggle if frame can be moved"] = "调整框架是否可移动"
    L["Grow frame upwards"] = "框架向上延伸"
    L["If enabled the frame will grow upwards instead of downwards"] = "开启次选项时框架向上延伸"
    L["Down"] = "下"
    L["Up"] = "上"
    L["Frame General"] = "框架"
    L["Frame scale"] = "框架大小"
    L["Scale of the frame"] = "框架的尺寸"
    L["Frame padding"] = "框架內距"
    L["Padding of the frame"] = "框架的內距"
    L["Frame width"] = "框架宽度"
    L["Margin"] = "框架间距"
    L["Margin between each button"] = "框架的间距"
    L["Cooldown General"] = "冷却"
    L["Font General"] = "字体"
    L["General Font"] = "通用字体"
    L["Font color text"] = "文字顏色"
    L["Font color timer"] = "计时器文字顏色"
    L["Color of the timers"] = "计时器顏色"
    L["Icons General"] = "图标"
    L["Icon border style"] = "图标边框样式"
    L["This changes the border style of all icons"] = "调整所有图标的边框样式"
    L["This changes the border color of all icons"] = "调整所有图标的边框顏色"
    L["Statusbar General"] = "状态条"
    L["Statusbar texture"] = "状态条材质"
    L["This changes the texture of all statusbar frames"] = "调整所有状态条的材质"
    L["Statusbar border style"] = "状态条边框样式"
    L["This changes the border style of all statusbar frames"] = "调整所有状态条的边框样式"
    L["Statusbar border offset divider (smaller is higher offset)"] = "状态条边框距离"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "调整状态条边框距离"
    L["Statusbar border color"] = "状态条边框顏色"
    L["This changes the border color of all statusbar frames"] = "调整所有状态条的边框颜色"
    L["Hide Blizzard"] = "隐藏暴雪框架"
    L["Grow Direction"] = "框架延伸方向"
    L["Arena only"] = "只在竞技场中"
    L["Never"] = "从不"
    L["Always"] = "一直"
    L["Load configuration"] = "设置选项" --Line 713
    L["Load configuration options"] = "加载设置选项" --Line 714
    L["Background Color of the frame"] = "框架的背景顏色"

    L["Gladdy"] = "Gladdy框架"  --Line 210, 709, 727
    L["Gladdy Tooltip round"] = "Gladdy 工具提示 圆形"
    L["Gladdy Tooltip squared"] = "Gladdy 工具提示 方形"
    L["Gloss (black border)"] = "光泽 (黑色边框)"
    L["Unlock frame"] = "解锁框架"
    L["Show Mover"] = "显示移动器"
    L["Hide Mover"] = "隐藏移动器"
    L["Toggle to show Mover Frames"] = "切换显示移动器框架"
    L["Test"] = "测试"
    L["Show Test frames"] = "显示测试框架"
    L["Hide"] = "隐藏"
    L["Hide frames"] = "隐藏框架"
    L["ReloadUI"] = "重新加载UI"
    L["Reloads the UI"] = "重新加载用户界面"
    L["Grow Middle"] = "从中心延伸"
    L["Frames expand along a centric anchor"] = "框架沿中心锚点扩展"
    L["Pixel Perfect Scale"] = "像素完美缩放"
    L["Enables Pixel Perfect Scale - disables manual "] = "启用像素完美缩放 - 停用手动 "
    L["OmniCC Enabled"] = "启用 OmniCC"
    L["Frame width"] = "框架宽度"
    L["Background color"] = "背景颜色"
    L["Background Color of the frame"] = "框架的背景颜色"
    L["Cooldown General"] = "冷却"
    L["Font General"] = "字体"
    L["General Font"] = "通用字体"
    L["Font color text"] = "文字顏色"
    L["Font color timer"] = "计时器文字顏色"
    L["Color of the timers"] = "计时器顏色"
    L["Icons General"] = "图标"
    L["Icon border style"] = "图标边框样式"
    L["This changes the border style of all icons"] = "调整所有图标的边框样式"
    L["Icon border color"] = "图标边框顏色"
    L["This changes the border color of all icons"] = "调整所有图标的边框顏色"
    L["Statusbar General"] = "状态条"
    L["Statusbar texture"] = "状态条材质"
    L["This changes the texture of all statusbar frames"] = "调整所有状态条的材质"
    L["Statusbar border style"] = "状态条边框样式"
    L["This changes the border style of all statusbar frames"] = "调整所有状态条的边框样式"
    L["Statusbar border offset divider (smaller is higher offset)"] = "状态条边框距离"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "调整状态条边框距离"
    L["Statusbar border color"] = "状态条边框顏色"
    L["This changes the border color of all statusbar frames"] = "调整所有状态条的边框颜色"
    L["Hide Blizzard"] = "隐藏暴雪框架"
    L["Arena only"] = "只在竞技场中"
    L["Never"] = "从不"
    L["Always"] = "一直"
    L["Grow Direction"] = "框架延伸方向"
    L["Down"] = "下"
    L["Up"] = "上"
    L["Load configuration"] = "设置选项"
    L["Load configuration options"] = "加载设置选项"

    -- Nameplates.lua
    L["Show important auras on nameplates"] = "在名条上显示重要光环"
    L["Zoomed Icon"] = "缩放图标"
    L["Zoomes the icon to remove borders"] = "缩放图标以移除边框"
    L["Icon size"] = "图标大小"
    L["Size of each aura icon"] = "每个光环图标大小"
    L["Icon Width Factor"] = "图标宽度比例"
    L["Stretches the icon"] = "图标宽度"
    L["Max Icons"] = "最大图标数"
    L["Maximum number of aura icons to show"] = "显示的最大光环图标数量"
    L["Icon Padding"] = "图标內距"
    L["Space between icons"] = "图标间距"
    L["Horizontal offset"] = "水平偏移"
    L["Vertical offset"] = "垂直偏移"
    L["Center Icons"] = "置中图标"
    L["Sort Order"] = "排序顺序"
    L["How to sort aura icons"] = "如何排序光环图标"
    L["Time"] = "时间"
    L["Time Left"] = "剩余时间"
    L["Frame Strata and Level"] = "框架层级和等级"
    L["Frame Strata"] = "框架层级"
    L["Frame Level"] = "框架等级"
    L["Aura List"] = "光环列表"
    L["Configure which auras to show on nameplates"] = "设置要在名条上显示哪些光环"
    L["Note: Uses the same aura list as the Auras module. Configure auras in the Auras module settings."] = "注意：使用与光环模块相同的光环列表。在光环模块设置中设置光环。"

    -- Diminishings.lua
    L["Show when Aura applied"] = "应用光环时显示"
    L["Group"] = "群组"
    L["DR Icon"] = "递减图标"
    L["Group direction"] = "群组方向"
    L["Cooldown Font"] = "冷却字体"
    L["Enable DR Colors as Font Color"] = "启用递减颜色作为字体颜色"
    L["Shows the current DR Level on the DR icon."] = "在递减图标上显示当前递减等级。"
    L["Cooldown number alpha"] = "冷却数字透明度"
    L["Font Outline"] = "字体轮廓"
    L["X Offset"] = "X 偏移"
    L["Y Offset"] = "Y 偏移"
    L["DR Font"] = "递减字体"
    L["Enable DR Font"] = "启用递减字体"
    L["DR Font color"] = "递减字体颜色"
    L["Color of the font"] = "字体颜色"
    L["Font of the DR Font"] = "递减字体的字体"
    L["DR Colors"] = "递减颜色"
    L["DR Texts"] = "递减文字"
    L["Text on half DR"] = "半递减文字"
    L["Text on quarter DR"] = "四分之一递减文字"
    L["Text on immune DR"] = "免疫递减文字"
    L["Enable DR Colors as Border Color"] = "启用递减颜色作为边框颜色"
    L["Colors borders of DRs in respective DR Colors"] = "以对应的递减颜色为递减边框着色"
    L["Border color"] = "边框颜色"
    L["DR Grow Direction"] = "递减延伸方向"
    L["Grow Direction of the dr icons"] = "递减图标的延伸方向"
    L["Change the DR Duration in seconds (DR is dynamic between 15-20s)"] = "以秒为单位变更递减持续时间（递减在15-20秒之间动态变化）"
    L["DR Duration"] = "递减持续时间"
    L["Space between Icons"] = "图标间距"
    L["Scale of the text"] = "设定字体大小"
    L["Scale of the font"] = "字体大小"

    -- Clicks.lua
    L["Action #%d"] = "动作 #%d"
    L["Left Click Enemy 1"] = "左键点击敌人 1"
    L["Left Click Enemy 2"] = "左键点击敌人 2"
    L["Left Click Enemy 3"] = "左键点击敌人 3"
    L["Left Click Enemy 4"] = "左键点击敌人 4"
    L["Left Click Enemy 5"] = "左键点击敌人 5"
    L["Right Click Enemy 1"] = "右键点击敌人 1"
    L["Right Click Enemy 2"] = "右键点击敌人 2"
    L["Right Click Enemy 3"] = "右键点击敌人 3"
    L["Right Click Enemy 4"] = "右键点击敌人 4"
    L["Right Click Enemy 5"] = "右键点击敌人 5"
    L["Middle Click Enemy 1"] = "中键点击敌人 1"
    L["Middle Click Enemy 2"] = "中键点击敌人 2"
    L["Middle Click Enemy 3"] = "中键点击敌人 3"
    L["Middle Click Enemy 4"] = "中键点击敌人 4"
    L["Middle Click Enemy 5"] = "中键点击敌人 5"
    L["Button4 Click Enemy 1"] = "按钮4点击敌人 1"
    L["Button4 Click Enemy 2"] = "按钮4点击敌人 2"
    L["Button4 Click Enemy 3"] = "按钮4点击敌人 3"
    L["Button4 Click Enemy 4"] = "按钮4点击敌人 4"
    L["Button4 Click Enemy 5"] = "按钮4点击敌人 5"
    L["Button5 Click Enemy 1"] = "按钮5点击敌人 1"
    L["Button5 Click Enemy 2"] = "按钮5点击敌人 2"
    L["Button5 Click Enemy 3"] = "按钮5点击敌人 3"
    L["Button5 Click Enemy 4"] = "按钮5点击敌人 4"
    L["Button5 Click Enemy 5"] = "按钮5点击敌人 5"
    L["Left button"] = "左键"
    L["Right button"] = "右键"
    L["Middle button"] = "中键"
    L["Button 4"] = "鼠标按键4"
    L["Button 5"] = "鼠标按键5"
    L["None"] = "无"
    L["All"] = "全部"
    L["CTRL"] = "CTRL"
    L["SHIFT"] = "SHIFT"
    L["ALT"] = "ALT"
    L["Cast Spell"] = "施放法术"
    L["Name"] = "名称"
    L["Select the name of the click option"] = "设置动作名称"
    L["Button"] = "按键"
    L["Select which mouse button to use"] = "设置欲使用的鼠标按键"
    L["Modifier"] = "修饰键"
    L["Select which modifier to use"] = "设置欲使用的修饰键"
    L["Action"] = "动作"
    L["Select what action this mouse button does"] = "设置输入按键后欲执行的动作"
    L["Cast Spell / Macro"] = "施放法术/宏"
    L["Target"] = "目标"
    L["Focus"] = "焦点"
    
    -- Additional missing keys for zhCN
    L["No Pet"] = "无宠物"
    L["0%"] = "0%"
    L["100%"] = "100%"
    L["50%"] = "50%"
    L["Above"] = "上方"
    L["Add Aura"] = "添加光环"
    L["Add Buff"] = "添加增益"
    L["Add Debuff"] = "添加减益"
    L["All enemy totem border color"] = "所有敌方图腾边框颜色"
    L["All friendly totem border color"] = "所有友方图腾边框颜色"
    L["Alpha of the icon when cooldown active"] = "冷却中图标的透明度"
    L["Announce Spell School"] = "通报法术分类"
    L["Announces Spell School like \"Holy\" instead of Spell Name"] = "通报法术分类如「神圣」而非法术名称"
    L["Announces when enemies' spells are interrupted"] = "当敌方法术被打断时通报"
    L["Attach To TotemPlate"] = "附加至图腾栏"
    L["Aura Detached"] = "光环分离"
    L["Aura X Offset"] = "光环 X 偏移"
    L["Aura Y Offset"] = "光环 Y 偏移"
    L["Aura size"] = "光环大小"
    L["Aura width factor"] = "光环宽度比例"
    L["Background"] = "背景"
    L["Bar Color"] = "计量条颜色"
    L["Bar horizontal"] = "水平计量条"
    L["Bar vertical"] = "垂直计量条"
    L["BeastMastery"] = "兽王"
    L["Below"] = "下方"
    L["Blood"] = "鲜血"
    L["Brewmaster"] = "酿酒大师"
    L["Buff Lists"] = "增益列表"
    L["By Arena"] = "依竞技场"
    L["By Order"] = "依顺序"
    L["COOLDOWN USED: %s (%s) used %s - %s sec. cooldown"] = "冷却已使用: %s (%s) 使用了 %s - %s 秒冷却"
    L["Class Colored Bars"] = "职业颜色计量条"
    L["Class Icon Enabled"] = "职业图标已启用"
    L["Class colored"] = "职业颜色"
    L["Colored trinket"] = "彩色饰品"
    L["Colored trinket CD"] = "彩色饰品冷却"
    L["Colored trinket No CD"] = "彩色饰品无冷却"
    L["Custom Colors"] = "自定义颜色"
    L["Custom Tags"] = "自定义标签"
    L["Custom Tags Enabled"] = "自定义标签已启用"
    L["Default Aura"] = "默认光环"
    L["Delete"] = "删除"
    L["Desaturate Icon"] = "图标去饱和"
    L["Detached Aura"] = "分离光环"
    L["Detached Interrupt"] = "分离打断"
    L["Detached Mode"] = "分离模式"
    L["Dialog"] = "对话框"
    L["Dispel Icon"] = "驱散图标"
    L["Dispel Icon Enabled"] = "驱散图标已启用"
    L["Dynamic Icon"] = "动态图标"
    L["Enable"] = "启用"
    L["Enable Aura to be displayed"] = "启用光环显示"
    L["Enable Custom Colors"] = "启用自定义颜色"
    L["Enable Interrupt Spell School Colors"] = "启用打断法术分类颜色"
    L["Enabled Racials"] = "种族技能已启用"
    L["Enabled showing racial"] = "已启用显示种族"
    L["Enables Targets module"] = "启用目标模块"
    L["Enemy"] = "敌方"
    L["Enrage"] = "狂怒"
    L["Export"] = "导出"
    L["Export your current profile to share with others or your various accounts."] = "导出您目前的设置以与他人或您的其他账号分享。"
    L["Fade"] = "淡出"
    L["Font Enabled"] = "字体已启用"
    L["Font of the custom totem name"] = "自定义图腾名字字体"
    L["Font size left text"] = "左侧文字字体大小"
    L["Font size right text"] = "右侧文字字体大小"
    L["Form"] = "形态"
    L["Friendly"] = "友方"
    L["Fullscreen"] = "全屏"
    L["Fullscreen Dialog"] = "全屏对话框"
    L["Group Class Icons"] = "群组职业图标"
    L["Group Mode"] = "群组模式"
    L["Group Pets"] = "群组宠物"
    L["Group Targets"] = "群组目标"
    L["Guardian"] = "守护者"
    L["Health Bar Text"] = "血量条文字"
    L["Health Values"] = "生命值"
    L["Height"] = "高度"
    L["Hide Disabled Totem Plates"] = "隐藏已停用的图腾栏"
    L["High"] = "高"
    L["Higher is darker"] = "数值越高越暗"
    L["Highlight mouseover"] = "鼠标指向高亮"
    L["Horizontal Grow Direction"] = "水平延伸方向"
    L["Horizontal Grow Direction of the cooldown icons"] = "冷却图标的水平延伸方向"
    L["Icon Enabled"] = "图标已启用"
    L["Icon border"] = "图标边框"
    L["If test is running, type \"/gladdy test\" again"] = "如果测试正在执行，请再次输入 \"/gladdy test\""
    L["Import"] = "导入"
    L["Interrupt Detached"] = "分离打断"
    L["Interrupt X Offset"] = "打断 X 偏移"
    L["Interrupt Y Offset"] = "打断 Y 偏移"
    L["Interrupt size"] = "打断大小"
    L["Interrupt width factor"] = "打断宽度比例"
    L["Interrupts"] = "打断"
    L["INTERRUPTED: %s (%s)"] = "已打断: %s (%s)"
    L["Layer Group"] = "图层群组"
    L["Layer Group and Level"] = "图层群组和等级"
    L["Layer Level"] = "图层等级"
    L["Left Font Outline"] = "左侧字体轮廓"
    L["Left Text"] = "左侧文字"
    L["Left Text Horizontal Offset"] = "左侧文字水平偏移"
    L["Left Text Vertical Offset"] = "左侧文字垂直偏移"
    L["Left font size"] = "左侧字体大小"
    L["Locked"] = "已锁定"
    L["Low"] = "低"
    L["Max"] = "最大"
    L["Medium"] = "中"
    L["Mistweaver"] = "织雾者"
    L["Nature"] = "自然"
    L["Cooldowncircle"] = "冷却圆圈"
    L["Only numbers, comma or space is allowed. \n Example: 124, 51243, 4121, 41111"] = "仅允许数字、逗号或空格。\n 例: 124, 51243, 4121, 41111"
    L["Out of Range Darkening Level"] = "超出距离时变暗程度"
    L["Power Bar Text"] = "法力/能量条文字"
    L["Power Texts"] = "法力/能量条文字"
    L["Power font size"] = "法力字体大小"
    L["Race font size"] = "种族字体大小"
    L["Range Check"] = "距离检测"
    L["Remaining"] = "剩余"
    L["Reset Glow"] = "重置发光"
    L["Reset Glow Color"] = "重置发光颜色"
    L["Reset Time"] = "重置时间"
    L["Reset time in seconds"] = "重置时间（秒）"
    L["Reverse"] = "反向"
    L["Right Font Outline"] = "右侧字体轮廓"
    L["Right Text"] = "右侧文字"
    L["Right Text Horizontal Offset"] = "右侧文字水平偏移"
    L["Right Text Vertical Offset"] = "右侧文字垂直偏移"
    L["Right font size"] = "右侧字体大小"
    L["Scale"] = "大小"
    L["Scourge"] = "亡灵"
    L["Show Arena1-5 as name instead"] = "显示 Arena1-5 作为名称"
    L["Show Spell Name"] = "显示法术名称"
    L["Show Time"] = "显示时间"
    L["Show border colors by spell school"] = "根据不同法术显示不同边框颜色"
    L["Show dynamic color on cooldown numbers"] = "冷却时间数字以动态颜色显示"
    L["Show name text"] = "显示名字文字"
    L["Show race"] = "显示种族"
    L["Show spec"] = "显示天赋"
    L["Show the units name"] = "显示单位名称"
    L["Show two timers"] = "显示两个计时器"
    L["Shows a solid colored icon when off/off CD."] = "当关闭/无冷却时显示实色图标。"
    L["Size"] = "大小"
    L["Size of each cd icon"] = "每个冷却图标大小"
    L["Size of the health text"] = "生命值文字大小"
    L["Size of the left text"] = "左侧文字大小"
    L["Size of the power text"] = "法力文字大小"
    L["Size of the race text"] = "种族文字大小"
    L["Size of the right text"] = "右侧文字大小"
    L["Spark"] = "尾部发亮"
    L["Spark enabled"] = "尾部发亮已启用"
    L["Spell ID used for texture"] = "用于材质的法术 ID"
    L["Spell IDs"] = "法术 ID"
    L["Spell School Colors"] = "法术种类颜色"
    L["Spell School Colors Enabled"] = "法术种类颜色已启用"
    L["Spells"] = "法术"
    L["Start Time"] = "开始时间"
    L["Start time in seconds"] = "开始时间（秒）"
    L["Status Bar border"] = "状态条边框"
    L["Status Bar border color"] = "状态条边框颜色"
    L["Stealth Color"] = "潜行颜色"
    L["Style"] = "样式"
    L["Test With Talents"] = "使用天赋测试"
    L["Test Without Talents"] = "不使用天赋测试"
    L["Text"] = "文字"
    L["Thick Outline"] = "粗轮廓"
    L["Thin Outline"] = "细轮廓"
    L["This changes positions of the trinket"] = "调整饰品图标位置"
    L["This changes positions with trinket"] = "调整与饰品的位置"
    L["This will overwrite your current profile!"] = "这将会覆盖您目前的设置！"
    L["Tooltip"] = "工具提示"
    L["Totem Pulse"] = "图腾脉冲"
    L["Totem Pulse Enabled"] = "图腾脉冲已启用"
    L["Turn on/off"] = "开启/关闭"
    L["Turns countdown before the start of an arena match on/off."] = "在竞技场开始前倒数剩余秒数"
    L["Turns totem icons instead of nameplates on or off."] = "开启/关闭图腾图标而非名条。"
    L["Unholy"] = "邪恶"
    L["Unknown"] = "未知"
    L["Vertical Bar"] = "垂直计量条"
    L["Vertical Grow Direction"] = "垂直延伸方向"
    L["Vertical Grow Direction of the cooldown icons"] = "冷却图标的垂直延伸方向"
    L["Width"] = "宽度"
    L["Will use Debuff Color if disabled"] = "若未启用则使用减益颜色"
    L["Windwalker"] = "踏风"
    L["XiconProfiles"] = "框架外观"
    L["yards"] = " 码"
    L["changes "] = "变更 "
elseif GetLocale() == "frFR" then
    -- Announcements.lua
    L["Announcements"] = "ANNONCE"
    L["RESURRECTING: %s (%s)"] = "EN TRAIN DE RESSUSCITER: %s (%s)"
    L["SPEC DETECTED: %s - %s (%s)"] = "SPEC DETECTÉ: %s - %s (%s)"
    L["LOW HEALTH: %s (%s)"] = "VIE BASSE: %s (%s)"
    L["TRINKET USED: %s (%s)"] = "TRINKET UTILISÉ: %s (%s)"
    L["TRINKET READY: %s (%s)"] = "TRINKET PRÊT: %s (%s)"
    L["DRINKING: %s (%s)"] = "EN TRAIN DE BOIRE: %s (%s)"
    L["Self"] = "Soi"
    L["Party"] = "Groupe"
    L["Raid Warning"] = "Avertissement de raid"
    L["Blizzard's Floating Combat Text"] = "Texte de combat flottant de Blizzard"
    L["Trinket used"] = "Trinket utilisé"
    L["Announce when an enemy's trinket is used"] = "Annonce quand un trinket ennemie est utilisé"
    L["Trinket ready"] = "Trinket prêt"
    L["Announce when an enemy's trinket is ready again"] = "Annonce quand un trinket ennemie est prêt de nouveau"
    L["Drinking"] = "En train de boire"
    L["Announces when enemies sit down to drink"] = "Annonce quand les ennemies s'assoient pour boire"
    L["Resurrection"] = "Resurrection"
    L["Announces when an enemy tries to resurrect a teammate"] = "Annonce quand un ennemie essaie de ressusciter un membre de son groupe"
    L["New enemies"] = "Nouveaux ennemis"
    L["Announces when new enemies are discovered"] = "Annonce quand les ennemies sont découverts"
    L["Spec Detection"] = "Détection des SPEC"
    L["Announces when the spec of an enemy was detected"] = "Annonce quand une spec d'un ennemie est détecté"
    L["Low health"] = "Vie Basse"
    L["Announces when an enemy drops below a certain health threshold"] = "Annonce quand un ennemie tombe en dessous d'un certain seuil de points de vie"
    L["Low health threshold"] = "Seuil de points de vie bas"
    L["Choose how low an enemy must be before low health is announced"] = "Choisissez à quel point un ennemi doit être bas avant d'annoncer une santé faible"
    L["Destination"] = "Destination"
    L["Choose how your announcements are displayed"] = "Choisissez comment vos annonces sont affichées"

    -- ArenaCountDown.lua
    L["Arena Countdown"] = "Compte à rebours de l'arène"
    L["Turn on/off"] = "Activer / désactiver"
    L["Turns countdown before the start of an arena match on/off."] = "Active / désactive le compte à rebours avant le début d'un match d'arène"
    L["Size"] = "Taille"

    -- Auras.lua
    L["Auras"] = "Auras"
    L["Frame"] = "Cadre"
    L["Cooldown"] = "Temps de recharge"
    L["No Cooldown Circle"] = "Pas de Balayage du temps de recharge"
    L["Cooldown circle alpha"] = "Alpha du Balayage du temps de recharge"
    L["Font"] = "Police de caractère"
    L["Font of the cooldown"] = "Police du temps de recharge"
    L["Font scale"] = "Échelle de police"
    L["Scale of the text"] = "Échelle du texte"
    L["Font color"] = "Couleur de la police"
    L["Color of the text"] = "Couleur du texte"
    L["Border"] = "Bordure"
    L["Border style"] = "Style de bordure"
    L["Buff color"] = "Couleur des buff"
    L["Debuff color"] = "Couleur des Debuff"
    L["Check All"] = "Vérifie tout"
    L["Uncheck All"] = "Décocher tout"
    L["Enabled"] = "Activé"
    L["Priority"] = "Priorité"

    -- BuffsDebuffs.lua
    L["Buffs and Debuffs"] = "Buffs et Debuffs"
    L["Enable"] = "Activé"
    L["Enabled Buffs and Debuffs module"] = "Module de buffs et debuffs activés"
    L["Show CC"] = "Montrer les CC"
    L["Shows all debuffs, which are displayed on the ClassIcon as well"] = "Montrer tout les debuffs, qui sont affichés sur l'icone de classe aussi"
    L["Buffs"] = "Buffs"
    L["Size & Padding"] = "Taille et remplissage"
    L["Icon Size"] = "Taille de l'icône"
    L["Size of the DR Icons"] = "Taille des icones de DR"
    L["Icon Width Factor"] = "Facteur de largeur d'icône"
    L["Stretches the icon"] = "Étire l'icône"
    L["Icon Padding"] = "Remplissage d'icônes"
    L["Space between Icons"] = "Espace entre les icônes"
    L["Position"] = "Position"
    L["Aura Position"] = "Position de l'aura"
    L["Position of the aura icons"] = "Position de l'icône de l'aura"
    L["Top"] = "Haut"
    L["Bottom"] = "Bas"
    L["Left"] = "Gauche"
    L["Right"] = "Droite"
    L["Grow Direction"] = "Direction de croissance"
    L["Grow Direction of the aura icons"] = "Direction de croissance de l'icônr de l'aura"
    L["Horizontal offset"] = "Décalage horizontal"
    L["Vertical offset"] = "Décalage verticale"
    L["Alpha"] = "Alpha"
    L["Debuffs"] = "Debuffs"
    L["Dynamic Timer Color"] = "Couleur de la minuterie dynamique"
    L["Show dynamic color on cooldown numbers"] = "Afficher la couleur dynamique sur les numéros de recharge"
    L["Color of the cooldown timer and stacks"] = "Couleur du temps de recharge et des piles"
    L["Spell School Colors"] = "Couleurs de l'école de sorts"
    L["Spell School Colors Enabled"] = "Couleurs de l'école de sorts"
    L["Show border colors by spell school"] = "Afficher les couleurs des bordures par école de sorts"
    L["Curse"] = "Malédiction"
    L["Color of the border"] = "Couleur de la bordure"
    L["Magic"] = "Magique"
    L["Poison"] = "Poison"
    L["Physical"] = "Physique"
    L["Immune"] = "Immunisé"
    L["Disease"] = "Maladie"
    L["Aura"] = "Aura"
    L["Form"] = "Forme"

    -- Castbar.lua
    L["Cast Bar"] = "Barre d'incantation"
    L["Bar"] = "Barre"
    L["Bar Size"] = "Taille de la barre"
    L["Bar height"] = "Hauteur de la barre"
    L["Height of the bar"] = "Hauteur de la barre"
    L["Bar width"] = "Largeur de la barre"
    L["Width of the bars"] = "Largeur de la barre"
    L["Texture"] = "Texture"
    L["Bar texture"] = "Texture de la barre"
    L["Texture of the bar"] = "Texture de la barre"
    L["Bar color"] = "Couleur de la barre"
    L["Color of the cast bar"] = "Couleur de la barre d'incantation"
    L["Background color"] = "Couleur de l'Arrière plan"
    L["Color of the cast bar background"] = "Couleur de l'Arrière plan de la barre d'incantation"
    L["Border size"] = "taille de la bordure"
    L["Status Bar border"] = "Bordure de la barre de statut"
    L["Status Bar border color"] = "Couleur de la bordure de la barre de statut"
    L["Icon"] = "Icone"
    L["Icon size"] = "Taille de l'icone"
    L["Icon border"] = "Bordure de l'icone"
    L["Icon border color"] = "Couleur de la bordure de l'icone"
    L["Spark"] = "Etincelle"
    L["Spark enabled"] = "Etincelle activé"
    L["Spark color"] = "Couleur de l'étincelle"
    L["Color of the cast bar spark"] = "Couleur de l'étincelle de la barre d'incantation"
    L["Font of the castbar"] = "Police de la barre d'incantation"
    L["Font size"] = "Taille de la police"
    L["Size of the text"] = "Taille du texte"
    L["Format"] = "Format"
    L["Timer Format"] = "Format du timer"
    L["Remaining"] = "Restant"
    L["Total"] = "Total"
    L["Both"] = "Les deux"
    L["Castbar position"] = "Position de la barre d'incantation"
    L["Icon position"] = "Position de l'icone"
    L["Offsets"] = "Décalage"

    -- Classicon.lua
    L["Class Icon"] = "Icone de la classe"
    L["Balance"] = "Equilibre"
    L["Feral"] = "Combat farouche"
    L["Restoration"] = "Restauration"
    L["Beast Mastery"] = "Maîtrise des bêtes"
    L["Marksmanship"] = "Précision"
    L["Survival"] = "Survie"
    L["Arcane"] = "Arcane"
    L["Fire"] = "Feu"
    L["Frost"] = "Givre"
    L["Holy"] = "Sacré"
    L["Retribution"] = "Vindicte"
    L["Protection"] = "Protection"
    L["Discipline"] = "Discipline"
    L["Shadow"] = "Ombre"
    L["Assassination"] = "Assassinat"
    L["Combat"] = "Combat"
    L["Subtlety"] = "Finesse"
    L["Elemental"] = "Elementaire"
    L["Enhancement"] = "Amélioration"
    L["Affliction"] = "Affliction"
    L["Demonology"] = "Démonologie"
    L["Destruction"] = "Destruction"
    L["Arms"] = "Armes"
    L["Fury"] = "Fureur"
    L["Show Spec Icon"] = "Afficher l'icone des spécialisations"
    L["Shows Spec Icon once spec is detected"] = "Afficher l'icone des spécialisations une fois que les spé sont détectés"
    L["Icon width factor"] = "Facteur de largeur d'icône"
    L["This changes positions with trinket"] = "Cela change de position avec le trinket"
    L["Border color"] = "Couleur de la bordure"

    --CombatIndicator.lua
    L["Combat Indicator"] = "Indicateur de combat"
    L["Enable Combat Indicator icon"] = "Activer l'icone de l'indicateur de combat"
    L["Anchor"] = "Ancre"
    L["This changes the anchor of the ci icon"] = "Cela change l'ancre de l'icône de l'indicateur de combat"
    L["This changes position relative to its anchor of the ci icon"] = "Cela change la position par rapport à son ancre de l'icône de l'indicateur de combat"

    -- Cooldowns.lua
    L["Cooldowns"] = "Temps de recharge (CD)"
    L["COOLDOWN USED: %s (%s) used %s - %s sec. cooldown"] = "CD utilisé : %s (%s) utilisé %s - %s sec. cd"
    L["Enabled cooldown module"] = "Module du temps de recharge activé"
    L["Cooldown size"] = "Taille du temps de recharge"
    L["Size of each cd icon"] = "Taille de chaque icone de CD"
    L["Max Icons per row"] = "Nombre maximal d'icônes par ligne"
    L["Scale of the font"] = "Échelle de la police"
    L["Anchor of the cooldown icons"] = "Ancre de l'icone du temps de recharge"
    L["Grow Direction of the cooldown icons"] = "Direction de croissance des icônes de temps de recharge"
    L["Offset"] = "Décalage"

    -- Diminishings.lua
    L["Diminishings"] = "Rendements décroissants (DR)"
    L["Enabled DR module"] = "Activer le module DR"
    L["DR Cooldown position"] = "Position du temps de recharge des DR"
    L["Position of the cooldown icons"] = "Position des icones de temps de recharge"
    L["DR Border Colors"] = "Couleur de la bordure des DR"
    L["Dr Border Colors Enabled"] = "Couleur de la bordure des DR activé"
    L["Colors borders of DRs in respective DR-color below"] = "Couleurs des bordures des DR dans la couleur DR respective ci-dessous"
    L["Half"] = "Demi"
    L["Quarter"] = "Quart"
    L["Categories"] = "Catégories"
    L["Force Icon"] = "Forcer les icones"
    L["Icon of the DR"] = "Icones des DR"

    -- ExportImport.lua
    L["Export Import"] = "Exporter Importer"
    L["Profile Export Import"] = "Profile de l'Exporter Importer"

    -- Healthbar.lua
    L["Health Bar"] = "Barre de vie"
    L["DEAD"] = "MORT"
    L["LEAVE"] = "QUITTER"
    L["General"] = "Général"
    L["Color of the status bar background"] = "Couleur de l'arrière-plan de la barre d'état"
    L["Font of the bar"] = "Police de la barre"
    L["Name font size"] = "Taille de la police du nom"
    L["Size of the name text"] = "Taille du texte du nom"
    L["Health font size"] = "Taille de la police de la santé"
    L["Size of the health text"] = "Taille du texte de la santé"
    L["Size of the border"] = "Taille de la bordure"
    L["Health Bar Text"] = "Texte de la barre de santé"
    L["Show name text"] = "Afficher le texte du nom"
    L["Show the units name"] = "Afficher le nom des unités"
    L["Show ArenaX"] = "Afficher ArenaX"
    L["Show Arena1-5 as name instead"] = "Afficher Arena1-5 comme nom à la place"
    L["Show the actual health"] = "Afficher la santé réelle"
    L["Show the actual health on the health bar"] = "Afficher la santé réelle sur la barre de santé"
    L["Show max health"] = "Afficher la santé maximale"
    L["Show max health on the health bar"] = "Afficher la santé maximale sur la barre de santé"
    L["Show health percentage"] = "Afficher le pourcentage de santé"
    L["Show health percentage on the health bar"] = "Afficher le pourcentage de santé sur la barre de santé"

    -- Highlight.lua
    L["Highlight"] = "Surbrillance"
    L["Show Inside"] = "Afficher dedans"
    L["Show Highlight border inside of frame"] = "Afficher la bordure de surbrillance à l'intérieur du cadre"
    L["Colors"] = "Couleurs"
    L["Target border color"] = "Couleur de la bordure cible"
    L["Color of the selected targets border"] = "Couleur de la bordure des cibles sélectionnées"
    L["Focus border color"] = "Couleur de la bordure du Focus"
    L["Color of the focus border"] = "Couleur de la bordure du Focus"
    L["Highlight target"] = "Mettre la cible en surbrillance"
    L["Toggle if the selected target should be highlighted"] = "Basculer si la cible sélectionnée doit être mise en surbrillance"
    L["Show border around target"] = "Afficher la bordure autour de la cible"
    L["Toggle if a border should be shown around the selected target"] = "Basculer si une bordure doit être affichée autour de la cible sélectionnée"
    L["Show border around focus"] = "Afficher la bordure autour du focus"
    L["Toggle of a border should be shown around the current focus"] = "La bascule d'une bordure doit être affichée autour du focus actuel"

    -- Pets.lua
    L["Pets"] = "Familier"
    L["Enables Pets module"] = "Activer le module familier (pet)"
    L["Width of the bar"] = "Largeur de la barre"
    L["Health color"] = "Couleur de la santé"
    L["Color of the status bar"] = "Couleur de la barre d'état"
    L["Portrait"] = "Portrait"
    L["Health Values"] = "Valeurs de santé"

    -- Powerbar.lua
    L["Power Bar"] = "Barre de puissance"
    L["Power Bar Text"] = "Texte de la barre de puissance"
    L["Power Texts"] = "Textes de puissance"
    L["Show race"] = "Afficher la race"
    L["Show spec"] = "Afficher la spé"
    L["Show the actual power"] = "Afficher la puissance actuelle"
    L["Show the actual power on the power bar"] = "Afficher la puissance actuelle sur la barre de puissance"
    L["Show max power"] = "Afficher la puissance maximale"
    L["Show max power on the power bar"] = "Afficher la puissance maximale sur la barre de puissance"
    L["Show power percentage"] = "Afficher le pourcentage de puissance"
    L["Show power percentage on the power bar"] = "Afficher le pourcentage de puissance sur la barre de puissance"

    -- Racial.lua
    L["Racial"] = "Raciale"
    L["Enable racial icon"] = "Activer l'icone du raciale"
    L["This changes the anchor of the racial icon"] = "Cela change l'ancre de l'icône du raciale"
    L["This changes position relative to its anchor of the racial icon"] = "Cela change de position par rapport à son ancre de l'icône du raciale"

    -- TotemPlates.lua
    L["Totem Plates"] = "Totem Plates"
    L["Customize Totems"] = "Personnaliser les totems"
    L["Custom totem name"] = "Personnaliser le nom des totems"
    L["Totem General"] = "Totem Général"
    L["Turns totem icons instead of nameplates on or off. (Requires reload)"] = "Active ou désactive les icônes totem au lieu des nameplates. (Nécessite un rechargement)"
    L["Show friendly"] = "Montrer les amis"
    L["Show enemy"] = "Montrer les ennemies"
    L["Totem size"] = "Taille du totem"
    L["Size of totem icons"] = "Taille de l'icone du totem"
    L["Font of the custom totem name"] = "Police du nom du totem personnalisé"
    L["Apply alpha when no target"] = "Appliquer l'alpha en l'absence de cible"
    L["Always applies alpha, even when you don't have a target. Else it is 1."] = "Applique toujours l'alpha, même lorsque vous n'avez pas de cible. Sinon c'est 1."
    L["Apply alpha when targeted"] = "Appliquer l'alpha lorsque ciblé"
    L["Always applies alpha, even when you target the totem. Else it is 1."] = "Applique toujours l'alpha, même lorsque vous ciblez le totem. Sinon c'est 1."
    L["All totem border alphas (configurable per totem)"] = "Tous les alphas de bordure de totem (configurables par totem)"
    L["Totem icon border style"] = "Style de bordure d'icône totem"
    L["All totem border color"] = "Toutes les couleurs de bordure de totem"

    -- Trinket.lua
    L["Trinket"] = "Trinket (Bijoux PvP)"
    L["Enable trinket icon"] = "Activer l'icone du trinket"
    L["This changes positions of the trinket"] = "Cela change les positions du trinket"

    -- XiconProfiles.lua
    L["Profile"] = "Profile"

    -- Frame.lua
    L["Gladdy - drag to move"] = "Gladdy - faites glisser pour déplacer"

    -- Gladdy.lua
    L["Welcome to Gladdy!"] = "Bienvenue chez Gladdy !"
    L["First run has been detected, displaying test frame."] = "La première exécution a été détectée, affichant la trame de test."
    L["Valid slash commands are:"] = "Les commandes de slash valides sont :"
    L["If this is not your first run please lock or move the frame to prevent this from happening."] = "S'il ne s'agit pas de votre première utilisation, veuillez verrouiller ou déplacer le cadre pour éviter que cela ne se produise."

    -- Options.lua
    L["settings"] = "paramètres"
    L["Reset module"] = "Réinitialiser le module"
    L["Reset module to defaults"] = "Réinitialiser le module"
    L["No settings"] = "Pas de paramètres"
    L["Module has no settings"] = "Le module n'a pas de paramètres"
    L["General settings"] = "Paramètres général"
    L["Lock frame"] = "Verouiller le cadre"
    L["Toggle if frame can be moved"] = "Cliquez si le cadre peut être bougé"
    L["Grow frame upwards"] = "Agrandir le cadre vers le haut"
    L["If enabled the frame will grow upwards instead of downwards"] = "Si activé, le cadre grandira vers le haut au lieu de vers le bas"
    L["Down"] = "Vers la bas"
    L["Up"] = "Vers le haut"
    L["Frame General"] = "Cadre général"
    L["Frame scale"] = "Taille du cadre"
    L["Scale of the frame"] = "Taille du cadre"
    L["Frame padding"] = "Remplissage du cadre"
    L["Padding of the frame"] = "Remplissage du cadre"
    L["Frame width"] = "Largeur du cadre"
    L["Margin"] = "Marge"
    L["Margin between each button"] = "Marge entre chaque bouton"
    L["Cooldown General"] = "Temps de recharge Général"
    L["Font General"] = "Police Général"
    L["General Font"] = "Police Général"
    L["Font color text"] = "Texte de la couleur de la police"
    L["Font color timer"] = "Texte de la couleur de la police"
    L["Color of the timers"] = "Couleur des minuteurs"
    L["Icons General"] = "Icônes Général"
    L["Icon border style"] = "Style de bordure d'icône"
    L["This changes the border style of all icons"] = "Cela change le style de bordure de toutes les icônes"
    L["This changes the border color of all icons"] = "Cela change la couleur de bordure de toutes les icônes"
    L["Statusbar General"] = "Barre d'état Général"
    L["Statusbar texture"] = "Texture de la Barre d'état"
    L["This changes the texture of all statusbar frames"] = "Cela change la texture de tous les cadres de la barre d'état"
    L["Statusbar border style"] = "Style de bordure de la barre d'état"
    L["This changes the border style of all statusbar frames"] = "Cela modifie le style de bordure de tous les cadres de la barre d'état"
    L["Statusbar border offset divider (smaller is higher offset)"] = "Diviseur de décalage de bordure de barre d'état (le plus petit est le décalage le plus élevé)"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "Décalage de la bordure par rapport à la barre d'état (au cas où la barre d'état s'afficherait au-delà de la bordure)"
    L["Statusbar border color"] = "Couleur de la bordure de la barre d'état"
    L["This changes the border color of all statusbar frames"] = "Cela change la couleur de la bordure de tous les cadres de la barre d'état"
    L["Gladdy Tooltip round"] = "Gladdy Tooltip rond"
    L["Gladdy Tooltip squared"] = "Gladdy Tooltip carré"
    L["Gloss (black border)"] = "Brillant (bordure noire)"
    L["Unlock frame"] = "Déverrouiller le cadre"
    L["Show Mover"] = "Afficher le déplacement"
    L["Hide Mover"] = "Masquer le déplacement"
    L["Toggle to show Mover Frames"] = "Basculer l'affichage des cadres de déplacement"
    L["Test"] = "Test"
    L["Show Test frames"] = "Afficher les cadres de test"
    L["Hide"] = "Masquer"
    L["Hide frames"] = "Masquer les cadres"
    L["ReloadUI"] = "Recharger l'UI"
    L["Reloads the UI"] = "Recharge l'interface utilisateur"
    L["Grow Middle"] = "Croître depuis le centre"
    L["Frames expand along a centric anchor"] = "Les cadres s'étendent le long d'une ancre centrique"
    L["Pixel Perfect Scale"] = "Échelle Pixel Perfect"
    L["Enables Pixel Perfect Scale - disables manual "] = "Active l'échelle Pixel Perfect - désactive manuel "
    L["OmniCC Enabled"] = "OmniCC activé"
    L["Frame width"] = "Largeur du cadre"
    L["Background color"] = "Couleur de fond"
    L["Background Color of the frame"] = "Couleur de fond du cadre"
    L["Cooldown General"] = "Temps de recharge Général"
    L["Font General"] = "Police Général"
    L["General Font"] = "Police Général"
    L["Font color text"] = "Couleur du texte de la police"
    L["Font color timer"] = "Couleur du minuteur de la police"
    L["Color of the timers"] = "Couleur des minuteurs"
    L["Icons General"] = "Icônes Général"
    L["Icon border style"] = "Style de bordure d'icône"
    L["This changes the border style of all icons"] = "Cela change le style de bordure de toutes les icônes"
    L["Icon border color"] = "Couleur de bordure d'icône"
    L["This changes the border color of all icons"] = "Cela change la couleur de bordure de toutes les icônes"
    L["Statusbar General"] = "Barre d'état Général"
    L["Statusbar texture"] = "Texture de la Barre d'état"
    L["This changes the texture of all statusbar frames"] = "Cela change la texture de tous les cadres de la barre d'état"
    L["Statusbar border style"] = "Style de bordure de la barre d'état"
    L["This changes the border style of all statusbar frames"] = "Cela modifie le style de bordure de tous les cadres de la barre d'état"
    L["Statusbar border offset divider (smaller is higher offset)"] = "Diviseur de décalage de bordure de barre d'état (le plus petit est le décalage le plus élevé)"
    L["Offset of border to statusbar (in case statusbar shows beyond the border)"] = "Décalage de la bordure par rapport à la barre d'état (au cas où la barre d'état s'afficherait au-delà de la bordure)"
    L["Hide Blizzard"] = "Masquer Blizzard"
    L["Arena only"] = "Arène uniquement"
    L["Never"] = "Jamais"
    L["Always"] = "Toujours"
    L["Grow Direction"] = "Direction de croissance"
    L["Down"] = "Vers le bas"
    L["Up"] = "Vers le haut"
    L["Load configuration"] = "Charger la configuration"
    L["Load configuration options"] = "Charger les options de configuration"

    -- Nameplates.lua
    L["Show important auras on nameplates"] = "Afficher les auras importantes sur les plaques de nom"
    L["Zoomed Icon"] = "Icône zoomée"
    L["Zoomes the icon to remove borders"] = "Zoome l'icône pour supprimer les bordures"
    L["Icon size"] = "Taille de l'icône"
    L["Size of each aura icon"] = "Taille de chaque icône d'aura"
    L["Icon Width Factor"] = "Facteur de largeur d'icône"
    L["Stretches the icon"] = "Étire l'icône"
    L["Max Icons"] = "Max Icônes"
    L["Maximum number of aura icons to show"] = "Nombre maximum d'icônes d'aura à afficher"
    L["Icon Padding"] = "Remplissage d'icônes"
    L["Space between icons"] = "Espace entre les icônes"
    L["Horizontal offset"] = "Décalage horizontal"
    L["Vertical offset"] = "Décalage verticale"
    L["Center Icons"] = "Centrer les icônes"
    L["Sort Order"] = "Ordre de tri"
    L["How to sort aura icons"] = "Comment trier les icônes d'aura"
    L["Time"] = "Temps"
    L["Time Left"] = "Temps restant"
    L["Frame Strata and Level"] = "Couche et niveau du cadre"
    L["Frame Strata"] = "Couche du cadre"
    L["Frame Level"] = "Niveau du cadre"
    L["Aura List"] = "Liste des auras"
    L["Configure which auras to show on nameplates"] = "Configurer quelles auras afficher sur les plaques de nom"
    L["Note: Uses the same aura list as the Auras module. Configure auras in the Auras module settings."] = "Note : Utilise la même liste d'auras que le module Auras. Configurez les auras dans les paramètres du module Auras."

    -- Diminishings.lua
    L["Show when Aura applied"] = "Afficher quand l'aura est appliquée"
    L["Group"] = "Groupe"
    L["DR Icon"] = "Icône DR"
    L["Group direction"] = "Direction du groupe"
    L["Cooldown Font"] = "Police du temps de recharge"
    L["Enable DR Colors as Font Color"] = "Activer les couleurs DR comme couleur de police"
    L["Shows the current DR Level on the DR icon."] = "Affiche le niveau DR actuel sur l'icône DR."
    L["Cooldown number alpha"] = "Alpha du nombre de temps de recharge"
    L["Font Outline"] = "Contour de la police"
    L["X Offset"] = "Décalage X"
    L["Y Offset"] = "Décalage Y"
    L["DR Font"] = "Police DR"
    L["Enable DR Font"] = "Activer la police DR"
    L["DR Font color"] = "Couleur de la police DR"
    L["Color of the font"] = "Couleur de la police"
    L["Font of the DR Font"] = "Police de la police DR"
    L["DR Colors"] = "Couleurs DR"
    L["DR Texts"] = "Textes DR"
    L["Text on half DR"] = "Texte sur demi DR"
    L["Text on quarter DR"] = "Texte sur quart DR"
    L["Text on immune DR"] = "Texte sur DR immunisé"
    L["Enable DR Colors as Border Color"] = "Activer les couleurs DR comme couleur de bordure"
    L["Colors borders of DRs in respective DR Colors"] = "Colore les bordures des DR dans les couleurs DR respectives"
    L["Border color"] = "Couleur de la bordure"
    L["DR Grow Direction"] = "Direction de croissance DR"
    L["Grow Direction of the dr icons"] = "Direction de croissance des icônes dr"
    L["Change the DR Duration in seconds (DR is dynamic between 15-20s)"] = "Changer la durée DR en secondes (DR est dynamique entre 15-20s)"
    L["DR Duration"] = "Durée DR"
    L["Space between Icons"] = "Espace entre les icônes"
    L["Scale of the text"] = "Échelle du texte"
    L["Scale of the font"] = "Échelle de la police"

    -- Clicks.lua
    L["Action #%d"] = "Action #%d"
    L["Left Click Enemy 1"] = "Clic gauche Ennemi 1"
    L["Left Click Enemy 2"] = "Clic gauche Ennemi 2"
    L["Left Click Enemy 3"] = "Clic gauche Ennemi 3"
    L["Left Click Enemy 4"] = "Clic gauche Ennemi 4"
    L["Left Click Enemy 5"] = "Clic gauche Ennemi 5"
    L["Right Click Enemy 1"] = "Clic droit Ennemi 1"
    L["Right Click Enemy 2"] = "Clic droit Ennemi 2"
    L["Right Click Enemy 3"] = "Clic droit Ennemi 3"
    L["Right Click Enemy 4"] = "Clic droit Ennemi 4"
    L["Right Click Enemy 5"] = "Clic droit Ennemi 5"
    L["Middle Click Enemy 1"] = "Clic molette Ennemi 1"
    L["Middle Click Enemy 2"] = "Clic molette Ennemi 2"
    L["Middle Click Enemy 3"] = "Clic molette Ennemi 3"
    L["Middle Click Enemy 4"] = "Clic molette Ennemi 4"
    L["Middle Click Enemy 5"] = "Clic molette Ennemi 5"
    L["Button4 Click Enemy 1"] = "Bouton4 Clic Ennemi 1"
    L["Button4 Click Enemy 2"] = "Bouton4 Clic Ennemi 2"
    L["Button4 Click Enemy 3"] = "Bouton4 Clic Ennemi 3"
    L["Button4 Click Enemy 4"] = "Bouton4 Clic Ennemi 4"
    L["Button4 Click Enemy 5"] = "Bouton4 Clic Ennemi 5"
    L["Button5 Click Enemy 1"] = "Bouton5 Clic Ennemi 1"
    L["Button5 Click Enemy 2"] = "Bouton5 Clic Ennemi 2"
    L["Button5 Click Enemy 3"] = "Bouton5 Clic Ennemi 3"
    L["Button5 Click Enemy 4"] = "Bouton5 Clic Ennemi 4"
    L["Button5 Click Enemy 5"] = "Bouton5 Clic Ennemi 5"
    L["Left button"] = "Bouton gauche"
    L["Right button"] = "Bouton droit"
    L["Middle button"] = "Bouton molette"
    L["Button 4"] = "Bouton 4"
    L["Button 5"] = "Bouton 5"
    L["None"] = "Aucun"
    L["All"] = "Tous"
    L["CTRL"] = "CTRL"
    L["SHIFT"] = "MAJ"
    L["ALT"] = "ALT"
    L["Cast Spell"] = "Lancer un sort"
    L["Name"] = "Nom"
    L["Select the name of the click option"] = "Sélectionner le nom de l'option de clic"
    L["Button"] = "Bouton"
    L["Select which mouse button to use"] = "Sélectionner quel bouton de souris utiliser"
    L["Modifier"] = "Modificateur"
    L["Select which modifier to use"] = "Sélectionner quel modificateur utiliser"
    L["Action"] = "Action"
    L["Select what action this mouse button does"] = "Sélectionner quelle action ce bouton de souris effectue"
    L["Cast Spell / Macro"] = "Lancer un sort / Macro"
    L["Target"] = "Cible"
    L["Focus"] = "Focus"

    -- Additional missing keys
    L["Frame"] = "Cadre"
    L["Enabled"] = "Activé"
    L["Icon"] = "Icône"
    L["Position"] = "Position"
    L["Cooldown"] = "Temps de recharge"
    L["Font"] = "Police de caractère"
    L["Font scale"] = "Échelle de police"
    L["Font color"] = "Couleur de la police"
    L["Border"] = "Bordure"
    L["Border style"] = "Style de bordure"
    L["Buff color"] = "Couleur des buff"
    L["Debuff color"] = "Couleur des Debuff"
    L["Check All"] = "Vérifie tout"
    L["Uncheck All"] = "Décocher tout"
    L["Force Icon"] = "Forcer les icones"
    L["Icon of the DR"] = "Icones des DR"
    L["Categories"] = "Catégories"
    L["Immune"] = "Immunisé"
    L["Half"] = "Demi"
    L["Quarter"] = "Quart"
    L["Top"] = "Haut"
    L["Bottom"] = "Bas"
    
    -- Additional missing keys for frFR
    L["No Pet"] = "Pas de familier"
    L["0%"] = "0%"
    L["100%"] = "100%"
    L["50%"] = "50%"
    L["Above"] = "Au-dessus"
    L["Add Aura"] = "Ajouter une aura"
    L["Add Buff"] = "Ajouter un buff"
    L["Add Debuff"] = "Ajouter un debuff"
    L["All enemy totem border color"] = "Couleur de la bordure de tous les totems ennemis"
    L["All friendly totem border color"] = "Couleur de la bordure de tous les totems amis"
    L["Alpha of the icon when cooldown active"] = "Alpha de l'icône lorsque le temps de recharge est actif"
    L["Announce Spell School"] = "Annoncer l'école de sorts"
    L["Announces Spell School like \"Holy\" instead of Spell Name"] = "Annonce l'école de sorts comme \"Sacré\" au lieu du nom du sort"
    L["Announces when enemies' spells are interrupted"] = "Annonce quand les sorts des ennemis sont interrompus"
    L["Attach To TotemPlate"] = "Attacher à la plaque de totem"
    L["Aura Detached"] = "Aura détachée"
    L["Aura X Offset"] = "Décalage X de l'aura"
    L["Aura Y Offset"] = "Décalage Y de l'aura"
    L["Aura size"] = "Taille de l'aura"
    L["Aura width factor"] = "Facteur de largeur de l'aura"
    L["Background"] = "Arrière-plan"
    L["Bar Color"] = "Couleur de la barre"
    L["Bar horizontal"] = "Barre horizontale"
    L["Bar vertical"] = "Barre verticale"
    L["BeastMastery"] = "Maîtrise des bêtes"
    L["Below"] = "En dessous"
    L["Blood"] = "Sang"
    L["Brewmaster"] = "Maître brasseur"
    L["Buff Lists"] = "Listes de buffs"
    L["By Arena"] = "Par arène"
    L["By Order"] = "Par ordre"
    L["COOLDOWN USED: %s (%s) used %s - %s sec. cooldown"] = "TEMPS DE RECHARGE UTILISÉ : %s (%s) a utilisé %s - %s sec. de temps de recharge"
    L["Class Colored Bars"] = "Barres colorées par classe"
    L["Class Icon Enabled"] = "Icône de classe activée"
    L["Class colored"] = "Coloré par classe"
    L["Colored trinket"] = "Bijou coloré"
    L["Colored trinket CD"] = "Bijou coloré avec CD"
    L["Colored trinket No CD"] = "Bijou coloré sans CD"
    L["Custom Colors"] = "Couleurs personnalisées"
    L["Custom Tags"] = "Tags personnalisés"
    L["Custom Tags Enabled"] = "Tags personnalisés activés"
    L["Default Aura"] = "Aura par défaut"
    L["Delete"] = "Supprimer"
    L["Desaturate Icon"] = "Désaturer l'icône"
    L["Detached Aura"] = "Aura détachée"
    L["Detached Interrupt"] = "Interruption détachée"
    L["Detached Mode"] = "Mode détaché"
    L["Dialog"] = "Dialogue"
    L["Dispel Icon"] = "Icône de dissipation"
    L["Dispel Icon Enabled"] = "Icône de dissipation activée"
    L["Dynamic Icon"] = "Icône dynamique"
    L["Enable"] = "Activer"
    L["Enable Aura to be displayed"] = "Activer l'affichage de l'aura"
    L["Enable Custom Colors"] = "Activer les couleurs personnalisées"
    L["Enable Interrupt Spell School Colors"] = "Activer les couleurs d'école de sorts pour les interruptions"
    L["Enabled Racials"] = "Raciales activées"
    L["Enabled showing racial"] = "Affichage des raciales activé"
    L["Enables Targets module"] = "Active le module des cibles"
    L["Enemy"] = "Ennemi"
    L["Enrage"] = "Enrager"
    L["Export"] = "Exporter"
    L["Export your current profile to share with others or your various accounts."] = "Exportez votre profil actuel pour le partager avec d'autres ou vos différents comptes."
    L["Fade"] = "Fondu"
    L["Font Enabled"] = "Police activée"
    L["Font of the custom totem name"] = "Police du nom de totem personnalisé"
    L["Font size left text"] = "Taille de la police du texte gauche"
    L["Font size right text"] = "Taille de la police du texte droit"
    L["Form"] = "Forme"
    L["Friendly"] = "Ami"
    L["Fullscreen"] = "Plein écran"
    L["Fullscreen Dialog"] = "Dialogue plein écran"
    L["Group Class Icons"] = "Icônes de classe de groupe"
    L["Group Mode"] = "Mode groupe"
    L["Group Pets"] = "Familiers de groupe"
    L["Group Targets"] = "Cibles de groupe"
    L["Guardian"] = "Gardien"
    L["Health Bar Text"] = "Texte de la barre de santé"
    L["Health Values"] = "Valeurs de santé"
    L["Height"] = "Hauteur"
    L["Hide Disabled Totem Plates"] = "Masquer les plaques de totem désactivées"
    L["High"] = "Élevé"
    L["Higher is darker"] = "Plus élevé est plus sombre"
    L["Highlight mouseover"] = "Surbrillance au survol"
    L["Horizontal Grow Direction"] = "Direction de croissance horizontale"
    L["Horizontal Grow Direction of the cooldown icons"] = "Direction de croissance horizontale des icônes de temps de recharge"
    L["Icon Enabled"] = "Icône activée"
    L["Icon border"] = "Bordure de l'icône"
    L["If test is running, type \"/gladdy test\" again"] = "Si le test est en cours, tapez \"/gladdy test\" à nouveau"
    L["Import"] = "Importer"
    L["Interrupt Detached"] = "Interruption détachée"
    L["Interrupt Spells School Colors"] = "Couleurs d'école de sorts pour les interruptions"
    L["Interrupt X Offset"] = "Décalage X de l'interruption"
    L["Interrupt Y Offset"] = "Décalage Y de l'interruption"
    L["Interrupt size"] = "Taille de l'interruption"
    L["Interrupt width factor"] = "Facteur de largeur de l'interruption"
    L["Interrupts"] = "Interruptions"
    L["INTERRUPTED: %s (%s)"] = "INTERROMPU : %s (%s)"
    L["Layer Group"] = "Groupe de couche"
    L["Layer Group and Level"] = "Groupe et niveau de couche"
    L["Layer Level"] = "Niveau de couche"
    L["Left Font Outline"] = "Contour de la police gauche"
    L["Left Text"] = "Texte gauche"
    L["Left Text Horizontal Offset"] = "Décalage horizontal du texte gauche"
    L["Left Text Vertical Offset"] = "Décalage vertical du texte gauche"
    L["Left font size"] = "Taille de la police gauche"
    L["Locked"] = "Verrouillé"
    L["Low"] = "Faible"
    L["Max"] = "Max"
    L["Medium"] = "Moyen"
    L["Mistweaver"] = "Tisse-brume"
    L["Nature"] = "Nature"
    L["No Cooldown Circle"] = "Pas de cercle de temps de recharge"
    L["Cooldowncircle"] = "Cercle de temps de recharge"
    L["Only numbers, comma or space is allowed. \n Example: 124, 51243, 4121, 41111"] = "Seuls les chiffres, la virgule ou l'espace sont autorisés. \n Exemple : 124, 51243, 4121, 41111"
    L["Out of Range Darkening Level"] = "Niveau d'assombrissement hors de portée"
    L["Power Bar Text"] = "Texte de la barre de puissance"
    L["Power Texts"] = "Textes de puissance"
    L["Power font size"] = "Taille de la police de puissance"
    L["Race font size"] = "Taille de la police de race"
    L["Range Check"] = "Vérification de portée"
    L["Remaining"] = "Restant"
    L["Reset Glow"] = "Réinitialiser la lueur"
    L["Reset Glow Color"] = "Réinitialiser la couleur de la lueur"
    L["Reset Time"] = "Réinitialiser le temps"
    L["Reset time in seconds"] = "Temps de réinitialisation en secondes"
    L["Reverse"] = "Inverser"
    L["Right Font Outline"] = "Contour de la police droite"
    L["Right Text"] = "Texte droit"
    L["Right Text Horizontal Offset"] = "Décalage horizontal du texte droit"
    L["Right Text Vertical Offset"] = "Décalage vertical du texte droit"
    L["Right font size"] = "Taille de la police droite"
    L["Scale"] = "Échelle"
    L["Scourge"] = "Fléau"
    L["Show Arena1-5 as name instead"] = "Afficher Arena1-5 comme nom à la place"
    L["Show Spell Name"] = "Afficher le nom du sort"
    L["Show Time"] = "Afficher le temps"
    L["Show border colors by spell school"] = "Afficher les couleurs des bordures par école de sorts"
    L["Show dynamic color on cooldown numbers"] = "Afficher la couleur dynamique sur les numéros de temps de recharge"
    L["Show name text"] = "Afficher le texte du nom"
    L["Show race"] = "Afficher la race"
    L["Show spec"] = "Afficher la spé"
    L["Show the units name"] = "Afficher le nom des unités"
    L["Show two timers"] = "Afficher deux minuteurs"
    L["Shows a solid colored icon when off/off CD."] = "Affiche une icône de couleur unie lorsqu'elle est désactivée/sans CD."
    L["Size"] = "Taille"
    L["Size of each cd icon"] = "Taille de chaque icône de CD"
    L["Size of the health text"] = "Taille du texte de santé"
    L["Size of the left text"] = "Taille du texte gauche"
    L["Size of the power text"] = "Taille du texte de puissance"
    L["Size of the race text"] = "Taille du texte de race"
    L["Size of the right text"] = "Taille du texte droit"
    L["Spark"] = "Étincelle"
    L["Spark enabled"] = "Étincelle activée"
    L["Spell ID used for texture"] = "ID de sort utilisé pour la texture"
    L["Spell IDs"] = "IDs de sorts"
    L["Spell School Colors"] = "Couleurs d'école de sorts"
    L["Spell School Colors Enabled"] = "Couleurs d'école de sorts activées"
    L["Spells"] = "Sorts"
    L["Start Time"] = "Heure de début"
    L["Start time in seconds"] = "Heure de début en secondes"
    L["Status Bar border"] = "Bordure de la barre d'état"
    L["Status Bar border color"] = "Couleur de la bordure de la barre d'état"
    L["Stealth Color"] = "Couleur de furtivité"
    L["Style"] = "Style"
    L["Test With Talents"] = "Test avec talents"
    L["Test Without Talents"] = "Test sans talents"
    L["Text"] = "Texte"
    L["Thick Outline"] = "Contour épais"
    L["Thin Outline"] = "Contour fin"
    L["This changes positions of the trinket"] = "Cela change les positions du trinket"
    L["This changes positions with trinket"] = "Cela change les positions avec le trinket"
    L["This will overwrite your current profile!"] = "Cela écrasera votre profil actuel !"
    L["Tooltip"] = "Info-bulle"
    L["Totem Pulse"] = "Pulsation de totem"
    L["Totem Pulse Enabled"] = "Pulsation de totem activée"
    L["Turn on/off"] = "Activer / désactiver"
    L["Turns countdown before the start of an arena match on/off."] = "Active / désactive le compte à rebours avant le début d'un match d'arène"
    L["Turns totem icons instead of nameplates on or off."] = "Active ou désactive les icônes totem au lieu des nameplates."
    L["Unholy"] = "Impie"
    L["Unknown"] = "Inconnu"
    L["Vertical Bar"] = "Barre verticale"
    L["Vertical Grow Direction"] = "Direction de croissance verticale"
    L["Vertical Grow Direction of the cooldown icons"] = "Direction de croissance verticale des icônes de temps de recharge"
    L["Width"] = "Largeur"
    L["Will use Debuff Color if disabled"] = "Utilisera la couleur Debuff si désactivé"
    L["Windwalker"] = "Marche-vent"
    L["XiconProfiles"] = "Profils Xicon"
    L["yards"] = "mètres"
    L["changes "] = "changements "
end



-- Superhack allowing use key as value if not present in table
LibStub("Gladdy").L = setmetatable(L, {
    __index = function(t, k)
        return k
    end
})