...
function RangeCheck:isAutoattackInRange(button)
    ...
    if UnitIsDeadOrGhost(button.unit) and not Gladdy:FindAuraByName(feignDeath, button.unit) then
        return nil
    end
...
function RangeCheck:CreateTimer(frame)
    if not frame.range then
        frame.range = CreateFrame("Frame")
        frame.range.elapsed = 0
        frame.range.parent = frame
        frame.range:SetScript("OnUpdate", function(self, elapsed)
            self.elapsed = self.elapsed + elapsed
            if self.elapsed >= 0.05 then
                self.elapsed = 0
                RangeCheck.CheckRange(self)
            end
        end)
    end
end
